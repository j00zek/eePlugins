...
class AsumanaksoyResolver(ResolveUrl):
    name = 'Asumanaksoy'
    domains = ['cdn.asumanaksoy.com']
    pattern = r'(?://|\.)(cdn\.asumanaksoy\.com)/video/([0-9a-zA-Z$:/.]+)'
    ...
    def get_media_url(self, host, media_id):
        ...
        if 'ideo not found' not in html:
            html = json.loads(html)
            source = html.get('videoSource')
            if source:
