...
def listM3U():
    ...
    if file_name == '' or path_m3u == '':
        xbmcgui.Dialog().notification('CDA', 'Podaj nazwę pliku oraz katalog docelowy.', xbmcgui.NOTIFICATION_ERROR)
        return
    xbmcgui.Dialog().notification('CDA', 'Generuję listę M3U.', xbmcgui.NOTIFICATION_INFO)
    #LIST
    url = 'https://api.cda.pl/tv/channels'
    response = getJson(url=url, auth=True)
    channels = response.get("channels",None)
    ...
    for c in channels:
        ...
        dataE2 += 'plugin.video.cdaplMB/main.py%3fmode=play_tv&cid=' + '%s:%s\n' % (cid, chName) #j00zek for E2 bouquets
    ...
    for c in channels:
        if c['number']==cid:
            ...
            if not manifest_dash:
                log,profil = LogowanieCda()
                response = getJson(url=url, auth=True)
                c=[ch for ch in response['channels'] if ch['number']==cid ][0]
                manifest_dash = c.get("manifest_dash",None)
                if not manifest_dash:
                    continue
            drm_widevine = c.get("drm_widevine",None)
            drm_header_value = c.get("drm_header_value",None)
            drm_header_name = c.get("drm_header_name",None)
            urlid = {'dash':manifest_dash, 'widevine':drm_widevine, 'drm_header_value': drm_header_value,'drm_header_name':drm_header_name}
            try:
                ab= json.dumps(urlid)
                PlayVid(ab,True)
...
def PlayVid(id,tv=False):
    ...
    abx = stream_url
    if not tv:
       # stream_url ='http://127.0.0.1:%s/dd='%(str(proxyport))+abx+'|User-Agent='    + quote('Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:98.0) Gecko/20100101 Firefox/98.0')
        stream_url = abx+'|User-Agent='    + quote('Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:98.0) Gecko/20100101 Firefox/98.0')

    else:
        stream_url +='|User-Agent='    + quote('Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:98.0) Gecko/20100101 Firefox/98.0')
