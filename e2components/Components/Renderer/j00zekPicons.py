...
DBG = False
try:
    from Components.j00zekComponents import j00zekDEBUG, j00zekEXCEPTION
except Exception:
    DBG = False
#####
...
def addPiconPath(mountpoint):
    ...
    try:
        ...
        j00zekEXCEPTION('[j00zekPicons] Exception:' + str(e))
...
def getPiconName(serviceName, selfPiconType):
    ...
    if DBG: j00zekDEBUG('[j00zekPicons:getPiconName("%s", "%s")] >>>' % (serviceName, selfPiconType))
    name = None
    sname = None
    pngname = None
    ...
    if selfPiconType == 'piconProv':
        ...
        if DBG: j00zekDEBUG('[j00zekPicons:getPiconName] looking piconProv for "%s"/"%s"' % (sName,serviceName))
        pngname = findPicon(sName, selfPiconType, serviceName)
        return pngname
    elif selfPiconType == 'piconSat':
        sName = serviceName.upper().replace('.', '').replace('°', '')
        if DBG: j00zekDEBUG('[j00zekPicons:getPiconName] looking piconSat for "%s"/"%s"' % (sName,serviceName))
        pngname = findPicon(sName, selfPiconType, serviceName)
        return pngname
        ...
        if fields:
            ...
            if len(fields) >= 10:
                ...
                if not pngname and fields[2] != '1':
                    ...
                    fields[2] = '1'
                    pngname = findPicon('_'.join(fields), selfPiconType, serviceName)
                if 0: #pngname and isVTI and os.path.islink(pngname): #to delete incorrect references
    ...
    if not pngname and sname:
        ...
        if not pngname:
            ...
            if len(name) > 0:
                ...
                if 0: #pngname and isVTI and not '1_' in pngname:
                    try:
                        os.symlink(pngname, '%s/%s.png' % (os.path.dirname(pngname), sname) )
                    except Exception as e:
                        j00zekEXCEPTION('[j00zekPicons:getPiconName] Exception %s' % str(e))
                if not pngname:
                    if name.endswith('hd'):
        ...
        for attrib, value in self.skinAttributes:
            if attrib == 'path':
                ...
                try:
                    ...
                    self.animationType = None
                    j00zekEXCEPTION('[j00zekPicons:applySkin] Exception=%s' % str(e))
        ...
        if self.instance:
            ...
            try:
                if what[0] == self.CHANGED_CLEAR: #we are expecting a real update soon. do not bother polling NOW, but clear data
                    ...
                    if DBG: j00zekDEBUG('[j00zekPicons]:[changed()] self.piconType = "%s", self.source.text = "%s"' %(self.piconType, str(self.source.text)))
                ...
                j00zekEXCEPTION('[j00zekPicons]:[changed] Exception:' + str(e))
