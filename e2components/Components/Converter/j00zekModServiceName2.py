...
#%cY yellow
    ...
    def getText(self):
        ...
        if self.type == self.NAME:
            ...
            if DBG: j00zekDEBUG('[j00zekModServiceName2:getText] if self.type==%s' % 'self.PROVIDER')
            tmpprov = tmpsat = ''
            if self.isStream:
                if DBG: j00zekDEBUG('[j00zekModServiceName2:getText] if self.isStream:%s' % '')
            ...
            if DBG: j00zekDEBUG('[j00zekModServiceName2:getText] tmpprov=%s' % tmpprov)
            ...
            if self.isStream:
                ...
                if self.ref and self.info:
                    tmpORBPOS = self.getTransponderInfo(self.info, self.ref, 'O')
                tmpORBPOS = self.getTransponderInfo(info, ref, 'O')
            if DBG: j00zekDEBUG('[j00zekModServiceName2:getText] if self.type == self.ORBPOS returns "%s"' % tmpORBPOS)
            return tmpORBPOS
