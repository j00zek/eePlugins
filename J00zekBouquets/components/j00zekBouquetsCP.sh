...
myTITLE="--- j00zekBouquets v.14-03-2016 ---"
...
myPath=/tmp
if ! `cat /proc/cpuinfo|grep -q 'cpu family.*sh4'`;then
	if `mount |grep -q '/media/data'`;then
		myPath=/media/data
	elif `mount |grep -q '/hdd'`;then
		myPath=`mount |grep -m 1 '/hdd'|cut -d ' ' -f3`
	fi
fi
...
SprawdzSystem(){
onValidSystem=0
for i in `ls /etc/opkg`
do
	if `cat /etc/opkg/$i|tr '[:upper:]' '[:lower:]'|egrep -q 'pkt|hdmu'`; then #Nie wspieramy PKT
		echo "_(j00zekBouquets does not work on this image)!!!!"
		exit 0
	fi
done

if [ ! -f /etc/opkg/opkg.conf ]; then #Nie wspieramy dziwnych systemow
	onValidSystem=0
elif `cat /etc/opkg/opkg.conf|tr '[:upper:]' '[:lower:]'|grep -q pkt`; then #Nie wspieramy PKT
	onValidSystem=0
elif [ -e /usr/lib/enigma2/python/Plugins/Extensions/PKT ]; then #PKT sprawdzone inaczej :P
	onValidSystem=0
else
	if `cat /proc/cpuinfo|grep -q 'cpu family.*sh4'`;then
		if `grep -q 'GOS_20' </usr/bin/enigma2`; then #na sh4, tylko GOS
			onValidSystem=1
		fi
	else
		onValidSystem=1
...
if [ ! -e /usr/bin/dvbsnoop ]; then
	...
	if [ $? -gt 1 ]; then
		echo "_(System is misconfigured)!!"
		exit 0
	fi
fi
...
if [ $? -eq 0 ]; then
	if ! `echo $LASTSERVICE| grep -q ":$2:"`; then
		...
		wget -q  http://127.0.0.1/web/zap?sRef=$1 -O /dev/null 2>/dev/null
                [ $? -gt 0 ] && echo "_(Error switching channel, check configuration of openwebif plugin)"
		sleep 10
	fi
fi
}
...
if [ ! -s $myPath/tids.log ];then
	echo "_(Error collecting transponders data!!!)"
	exit 0
fi
}
#
...
if [ $DATA == "CP" ]; then
	WAITMESSAGE="_(Syncing services in lamedb, wait a minute...)"
...
echo "_(Selected provider:) $PROVIDER"
#################### NIT ####################
echo "_(Collecting transponders data...)"
getNIT $ALLOWTRANSPONDERS
#################### Dumping dvb stream... ####################
echo "_(Dumping dvb stream packets...)"
DumpStream
#################### SDT ####################
echo "_(Reading SDT...)"
#trwa 6s
...
echo "_(Reading Channel Association Table...)"
...
echo "_(Syncing transponders in lamedb...)"
#NAMESPACE:TID:NID
#00820000:1b58:013e
...
if [ $ExcludeSIDs -eq 1 ];then
	echo "_(Excluding unwanted channels) ..."
...
if [ $doPROVIDER -eq 1 ];then
	echo "_(Creating file) $BOUQUETFILE.tv"
	...
	[ $DEBUG -eq 0 ] && mv -f $myPath/$BOUQUETFILE.tv /etc/enigma2/ || cp -f $myPath/$BOUQUETFILE.tv /etc/enigma2/
fi
...
if [ $doOWN -eq 1 ];then
	...
	if [ -z "$FirstBouquet" ];then
		echo "_(ERROR assigning 1st bouquet name. Is the structure of the bouquets.tv file standard?)"
	elif [ ! -f /etc/enigma2/$FirstBouquet ];then
		echo "/etc/enigma2/$FirstBouquet _(does not exists!!!)"
		...
		echo "_(Updating bouquet) $FirstBouquet"
		while IFS=";" read -r ID SID NAMESPACE NID TID TYP DVBS2 SATPROVIDER NAZWA; do
...
echo "_(Finished in) $runningTime s."
exit 0
