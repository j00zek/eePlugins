...
class emukodiConsole(Screen):
    ...
    def __init__(self, session, title = "emukodiConsole", cmdlist = None, finishedCallback = None, closeOnSuccess = False, endText = "\nKoniec :D"):
        ...
        self.executebuiltin = os.path.join(working_dir,"executebuiltin")
        if os.path.exists(self.executebuiltin):
            os.remove(self.executebuiltin)
    ...
    def runFinished(self, retval):
        ...
        if os.path.exists(self.executebuiltin): #z xbmc
            self.cmdlist.append(open(self.executebuiltin, "r").read().strip())
            os.remove(self.executebuiltin)
        self.run += 1
        #print('runFinished self.cmdlist', self.run, len(self.cmdlist), self.cmdlist)
        if self.run != len(self.cmdlist):
            with open("/proc/sys/vm/drop_caches", "w") as f: f.write("1\n")
            log(self.cmdlist[self.run])
            #print('runFinished doExec',self.cmdlist[self.run])
            if self.doExec(self.cmdlist[self.run]):  # Start of container application failed so we must call runFinished manually.
                self.runFinished(-1) # so we must call runFinished manual
        else:
            ...
            #if lastpage:
            self["text"].lastPage()
            with open('/tmp/e2kodi.log', 'a') as f:print('[E2Kodi]>[emukodiConsole.runFinished(errorOcurred=%s)] displayed text:\n %s' % (self.errorOcurred, myText), file=f)
