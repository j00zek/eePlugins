...
class IPLA(object):
    ...
    def newtime(self,ff):
        ...
        if dd=='25':
            dd='01'
            dzien='{:>02d}'.format(int(dzien)+1)
        ff = re.sub(r'(\d+)T(\d+)', '%sT%s' % (dzien, int(dd)), ff)
    ...
    def generate_m3u(self):
        path = '/tmp/.emukodi/' # j00zek 4 e2kodi xbmcgui.Dialog().browse(0, 'Wybierz miejsce zapisu playlisty', 'files')
        if path == '':
            return
...
if __name__ == '__main__':
    ...
    if not mode:
        ...
        yes = True # j00zek 4 e2kodi xbmcgui.Dialog().yesno("[COLOR orange]Uwaga[/COLOR]", 'Czy na pewno chcesz się wylogować?',yeslabel='TAK', nolabel='NIE')
        if yes:
