...
TYPES = [
    {
        "msg": _addon.getLocalizedString(30801),
        "code": "movies",
        "isMovie": True,
        "data": {"z": "movies4cat", "cs": "37178378331", "v": "4"},
    },
    {
        "msg": _addon.getLocalizedString(30802),
        "code": "series",
        "isMovie": False,
        "data": {"z": "series4cat", "cs": "1591", "v": "4"},
    },  # mask? 1 | 2 | 4 | 16 | 32 | 512 | 1024
]
CATEGORIES = [
...
def listOfItems(sl, ctype, category):
    ...
    if ctp is None or cat is None:
        xbmcgui.Dialog().ok(
            heading=_addon.getLocalizedString(30800),
    ...
    for item in items:
        ...
        link = get_url(
            library="play" if ctp["isMovie"] else "seasons", lid=item["id"], ctype=ctype
        )
...
def episodes(sl, lid, sz):
    ...
    xbmcplugin.setPluginCategory(
        _handle, data["title"] + " / " + _addon.getLocalizedString(30807) + " " + sz
    )
    ...
    for episode in episodes:
        ...
        list_item.setInfo(
            "video",
            {"title": episode["title"], "duration": int(episode["duration"]) * 60},
        )
...
def play(sl, lid):
    ...
    if "description" not in data or xbmcgui.Dialog().yesno(
        data["title"],
        data["description"],
        _addon.getLocalizedString(30804),
        _addon.getLocalizedString(30803),
    ):
        params = {}
        ...
        if info:
            ...
            if is_helper.check_inputstream():
                ...
                playitem.setProperty(
                    "inputstream.adaptive.manifest_type", info["protocol"]
                )
            ...
            return
        xbmcgui.Dialog().ok(
            heading=data["title"], line1=_addon.getLocalizedString(30805)
        )
