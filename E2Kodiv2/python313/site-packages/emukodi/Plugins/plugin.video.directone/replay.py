    ...
    if channels:
        for channel in channels:
            if channel["replay"]:
                ...
                list_item.setArt(
                    {"thumb": utils.get_logo(channel["title"], sl._api_url)}
                )
                link = get_url(
                    replay="days",
...
def days(sl, stationid, channel, askpin):
    ...
    xbmcplugin.setPluginCategory(
        _handle, _addon.getLocalizedString(30600) + " / " + channel
    )
    if askpin != "False":
    ...
    for day in range(0, 8):
        if day < 7 or time_until_end_of_day(now).seconds > REPLAY_LAST_GAP:
            ...
            title = (
                _addon.getLocalizedString(30601)
                if day == 0
                else _addon.getLocalizedString(30602)
                if day == 1
                else utils.dec_utf8(d.strftime("%d. %m."))
            )
            title = (
                _addon.getLocalizedString(int("3061" + str(d.weekday()))) + ", " + title
            )
...
def programs(sl, stationid, channel, day=0, first=False):
    ...
    if today:
        ...
        start_date = from_date
    epg = utils.call(
        sl, lambda: sl.epg([{"id": stationid, "title": channel}], from_date, to_date,  showprogress=False, as_dict=True, EnrichedEPG=True)
    )
    xbmcplugin.setPluginCategory(
        _handle, _addon.getLocalizedString(30600) + " / " + channel
    )
    if day < 6 or (day == 6 and time_until_end_of_day(now).seconds > REPLAY_LAST_GAP):
        ...
        link = get_url(
            replay="programs", stationid=stationid, channel=channel, day=day + 1
        )
        is_folder = True
        xbmcplugin.addDirectoryItem(_handle, link, list_item, is_folder)
    if epg:
        lastLocId = None
        for program in epg[stationid]:
            start = datetime.datetime.fromtimestamp(program["start"])
            show_item = (
                start.replace(hour=0, minute=0, second=0, microsecond=0) - start_date
            ).days == 0  # started today
            if today:
                show_item = show_item and (
                    start + datetime.timedelta(seconds=program["duration"] + REPLAY_GAP * 60)
                    < now
                )
            if lastday:
                show_item = show_item and (
                    start > then and (start - then).seconds > REPLAY_LAST_GAP
                )
            if show_item:
                ...
                list_item.setInfo(
                    "video", {"title": program["title"], "duration": duration}
                )
                if "cover" in program:
                    list_item.setArt(
                        {"thumb": program["cover"], "icon": program["cover"]}
                    )

                link = get_url(
                    replay="replay",
                    locId=program["catchup_id"],
                    duration=duration,
                    lastLocId=lastLocId,
                )
                lastLocId = program["catchup_id"]
                is_folder = False
...
def replay(sl, locId, duration, lastLocId):
    try:
        ...
        info = utils.call(sl, lambda: sl.channel_info(locId))
    ...
    if args:
        if args["replay"][0] == "programs":
            programs(
                ...
                args["stationid"][0],
                args["channel"][0],
                int(args["day"][0]) if "day" in args else 0,
                "first" in args,
            )
