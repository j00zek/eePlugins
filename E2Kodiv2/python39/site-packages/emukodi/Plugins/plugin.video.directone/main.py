...
def locId_from_time(sl, stationid, utc):
    ...
    epg = utils.call(
        sl, lambda: sl.epg([{"stationid": stationid}], from_date, now, False)
    )
...
def play(channel_id, askpin):
    ...
    if info:
        ...
        if is_helper.check_inputstream():
            ...
            playitem.setProperty("inputstream.adaptive.license_type", info["drm"])
            playitem.setProperty("inputstream.adaptive.license_key", info["key"])
            #playitem.setProperty("inputstream.adaptive.server_certificate", info["certificate"])
            xbmcplugin.setResolvedUrl(_id, True, playitem)
...
if __name__ == "__main__":
    ...
    if "id" in args:
        play(
            str(args["id"][0]), str(args["askpin"][0]) if "askpin" in args else "False"
        )
        ...
        replay.router(
            args,
            skylink.Skylink(
                _user_name, _password, _profile, _provider, _pin_protected_content
            ),
        )
    elif "live" in args:
        live.router(
        ...
        if _a_show_live:
            xbmcplugin.addDirectoryItem(
                _id,
                live.get_url(live="channels"),
                xbmcgui.ListItem(label=_addon.getLocalizedString(30700)),
                True,
            )
        xbmcplugin.addDirectoryItem(
            _id,
            replay.get_url(replay="channels"),
            xbmcgui.ListItem(label=_addon.getLocalizedString(30600)),
            True,
        )
        xbmcplugin.addDirectoryItem(
            _id,
            library.get_url(library="types"),
            xbmcgui.ListItem(label=_addon.getLocalizedString(30800)),
            True,
        )
