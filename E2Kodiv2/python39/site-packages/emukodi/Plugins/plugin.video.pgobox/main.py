...
class IPLA(object):
    ...
    def newtime(self,ff):
        ...
        now_UTC=datetime.datetime.now(datetime.UTC) #datetime.utcnow().replace(microsecond=0) depracated
        dif=now_loc-now_UTC
    ...
    def getChannels(self):
        ...
        for i in data['result']['results']:
            ...
            for j in myper:
                ...
                if 1: # j00zek, obecnie brak channelperms w danych >>> j in channelperms or i['title']=='Polsat' or i['title']=='TV4' or i['title']=='Ukraina 24 HD' or not channelperms:
        ...
        for entry in items:
            if not entry['url'] in dupes:
                ...
                listakanalow[entry['url']] = entry['title'] #j00zek
        addon.setSetting('kanaly', str(dupes))
        addon.setSetting('listakanalow', json.dumps(listakanalow)) #j00zek
    ...
    def getFullEPG(self, kanaly = None, startDate = None, NumberOfDays = 2): #j00zek
        if kanaly is None:
            kanaly = ["1561396","1561386","1560849","1560544","1566742","1566556","1566588","1566406","1565708",
                      "1565807","1565809","1565709","1559196","1560537","1560388","1559892","1565980","1558790"] # FASTY
        ...
        DatesList = []
        for i in range(NumberOfDays): #iterates 0 to X
            fromDate = startDate + datetime.timedelta(days=i)
            fromDate = fromDate.strftime('%Y-%m-%dT%H:%M:%S') + ('.%03dZ' % (startDate.microsecond / 10000))
            toDate = startDate + datetime.timedelta(days=i+1)
            toDate = toDate.strftime('%Y-%m-%dT%H:%M:%S') + ('.%03dZ' % (startDate.microsecond / 10000))
            DatesList.append((fromDate, toDate))
            
        #pobierz id kanału i jego nazwę
        listakanalow = addon.getSetting('listakanalow')
        listakanalow = json.loads(listakanalow)
        ...
        mojeEPG = {}
        for kanal in kanaly:
            nazwa = listakanalow.get(kanal, kanal)
            CzysteDane = []
            for getForDates in DatesList:
                POST_DATA={"jsonrpc":"2.0","method":"getChannelsProgram","id":1,
                           "params":{"channelIds":[kanal],"fromDate":getForDates[0],"toDate":getForDates[1],"ua":UAIPLA,
                           "authData":{"sessionToken":authdata},
                           "userAgentData":{"deviceType":"pc","application":"firefox","os":"windows","osInfo":OSINFO,"build":1,
                           "portal":"pbg","player":"html"},"clientId":self.CLIENT_ID,
                           "deviceId":{"type":"other","value":self.DEVICE_ID}}}
                response = getRequests(self.NAVIGATE, data = POST_DATA, headers=self.HEADERS)
                response = response['result']
                dane = response.get(kanal, None)
                for dana in dane:
                    dana.pop('cpid')
                    dana.pop('id')
                    dana.pop('reporting')
                    CzysteDane.append(dana)
            mojeEPG[nazwa] = CzysteDane
            
        open("/tmp/pgbox_EPG.json", "w").write(json.dumps(mojeEPG))
        print('Dane EPG zapisane w /tmp/pgbox_EPG.json')
