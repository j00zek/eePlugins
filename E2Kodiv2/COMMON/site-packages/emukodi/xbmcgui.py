...
#              if "plugin://" in path:
              path = path.replace("plugin://", "/usr/lib/enigma2/python/Plugins/Extensions/emukodi/Plugins/")
...
class ListItem:
    def __init__(self, label=None, label2=None, iconImage=None, thumbnailImage=None, path=None, offscreen = False):
        self.data = {}
    ...
    def getVideoInfoTag(self) -> 'xbmc.InfoTagVideo':
        ...
        """
        from emukodi.xbmc import InfoTagVideo
        return InfoTagVideo()

# xbmc/interfaces/python/xbmcmodule/dialog.cpp

class Dialog:
    def notification(self, heading, message, icon = 'info', time = 3, sound=None):
        """
        ...
        * xbmcgui.NOTIFICATION_ERROR
        ...
        Example::
            ...
            ..
        """
        if time >= 1000: # standardowo czas jest podawany w kodi w ms
            time = int(time // 1000)
        print("Dialog().Notification: %s\n" % [heading, message, icon, time])
        if icon == "warning": typ = 2
        elif icon == "error": typ = 3
        else: typ = 1 #info
        from urllib.parse import quote as urllib_quote
        msg = xbmcE2.cleanKodiText('%s: %s' % (heading, message))
        xbmcE2.webIfMessage(urllib_quote(msg), typ, time)
        
    def input(self, message, type= None, icon=None, time=None, sound=None):
        print("Dialog().input: %s\n" % message)

    def ok(self, heading, line1, line2=None, line3=None):
        """Show a dialog 'OK'."""
        from traceback import format_exc
        print(format_exc())
        print("*** dialog czekam 600s na naciśnięcie OK :%s" % [heading, line1, line2, line3])
        from urllib.parse import quote as urllib_quote
        import time
        msg = '%s:\n %s' % (heading, line1)
        ...
        if not line3 is None:
            msg += '\n %s' % line3
        msg = xbmcE2.cleanKodiText(msg)
        time_to_wait = 600
        xbmcE2.webIfMessage(urllib_quote(msg), 0, time_to_wait)
        while time_to_wait > 0:
            time_to_wait -=1
            time.sleep(1)
            webHeader,webContent = xbmcE2.getWebPage('http://127.0.0.1/web/messageanswer')
            if '<e2state>True</e2state>' in webContent:
                time_to_wait = -1
        return True
