#import sys
#for pTa in [r'c:\Users\akpar\AppData\Roaming\Kodi\addons\script.module.pywidevine\lib',
#           ]:
...
def download_google_drive_file_from_url(url, destination):
    # Extract the file ID from different types of Google Drive URLs
    match = re.search(r'/d/([a-zA-Z0-9_-]+)', url)  # format: /file/d/<ID>/
    if not match:
        match = re.search(r'id=([a-zA-Z0-9_-]+)', url)  # format: ?id=<ID>
    if not match:
        raise ValueError("No valid file ID found in the provided URL.")

    file_id = match.group(1)

    # Start the download process from Google Drive
    base_url = "https://drive.google.com/uc?export=download"
    session = requests.Session()
    response = session.get(base_url, params={'id': file_id}, stream=True)

    # Check for a confirmation token (used for large files or virus scan bypass)
...
if not os.path.exists(device_file):
  url = '' # URL to download the device.wvd file from
  if url:
    LOG('Downloading {}'.format(device_file))
    download_google_drive_file_from_url(url, device_file)
