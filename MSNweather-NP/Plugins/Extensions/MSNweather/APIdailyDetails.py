            ...
            for condition in dictTrend['conditions']:
                if condition['days'] > 0:
                    if condition['type'] == 'Rain':
                        ...
                        summaryDayInfo += clr['Gray'] + _(', maximum ') + str(dictTrend['recPrecip']) + ' w ' + str(dictTrend['precipDate'][:4]) + 'r'
                    elif condition['type'] == 'Snow':
                        summaryDayInfo += clr['Gray'] + '\n' + _('- was snowing %s days, in average %s') % (str(int(condition['days'])), str(dictTrend['avgSnow']))
                        summaryDayInfo += clr['Gray'] + _(', maximum ') + str(dictTrend['recSnow']) + ' w ' + str(dictTrend['snowDate'][:4]) + 'r'
                    elif condition['type'] == 'Thunder':
                        if int(condition['days']) > 1:
                            summaryDayInfo += clr['Gray'] + '\n' + _('- thunder %s days') % str(int(condition['days']))
                        else:
                            summaryDayInfo += clr['Gray'] + '\n' + _('- thunder %s day') % str(int(condition['days']))
                    else:
