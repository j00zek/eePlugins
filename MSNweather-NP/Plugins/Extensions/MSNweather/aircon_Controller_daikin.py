...
class DaikinController(Screen):
    def __init__(self, session, AC_IP, AC_PORT, AC_info):
        ...
        self.isACworking = False
        # Top info
        self["ac_config_info"] = Label("")
        self["ac_name"] = Label("")
        #middle part
    ...
    def getACstate(self):
        ...
        ac_retVal = self.AC.get_name()
        if ac_retVal == '': #name unavailable means no connection to daikin
            self["ac_config_info"].setText("%s%s %s%s" % (clr['Gray'], self.AC_ADDR, clr['R'], _('IS UNAVAILABLE !!!') ))
            return
        else:
            self["ac_config_info"].setText("%s%s%s (%s)" % (clr['G'], self.AC_info, clr['Gray'], self.AC_ADDR))
            self["ac_name"].setText( _("%sName:%s %s") % (clr['G'], clr['Gray'], str(ac_retVal) ) )
            self.isACworking = True
