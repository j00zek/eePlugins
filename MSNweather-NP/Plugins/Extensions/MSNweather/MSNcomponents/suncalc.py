# based on https://github.com/Broham/suncalcPy
# some changes by j00zek
...
def getMoonTimes(date, lat, lng):
    ...
    rise2 = 0
    sett2 = 0
    if 1: # j00zek krok 1 chyba lepiej oblicza moonrise, dla odmianystep 2 lepiej moonset
        for i in range(1,24,1):
            ...
            if roots == 1:
                ...
                rise2 = i + (x2 if ye < 0 else x1)
                sett2 = i + (x1 if ye < 0 else x2)
    ...
    #print('offset', offset)
    if (rise2):
        result["moonrise"] = hoursLater(t, rise2)
        result["moonriseUTC"] = hoursLater(t, rise2) - timedelta(hours=offset)
    elif (rise):
        result["moonrise"] = hoursLater(t, rise)
        result["moonriseUTC"] = hoursLater(t, rise) - timedelta(hours=offset)
    if (sett):
        result["moonset"] = hoursLater(t, sett)
        result["moonsetUTC"] = hoursLater(t, sett) - timedelta(hours=offset)
    elif (sett2):
        result["moonset"] = hoursLater(t, sett2)
        result["moonsetUTC"] = hoursLater(t, sett2) - timedelta(hours=offset)
...
if __name__ == '__main__':
    print(getMoonTimes(datetime.now(),  52.094000000000001, 20.805 ))
