...
def GiosInfo(webContent):
    ...
    try:
        dictTMP = json.loads(webContent)
        dictTMP = dictTMP['Lista stacji pomiarowych']
        for inst in dictTMP:
            myID = str(inst['Identyfikator stacji'])
        ...
        if defDBG: print(traceback.format_exc())
...
def Gios(webContent):
    ...
    paramsDict['dictGios'] = {}
    addSensorsDict = True
    if str(dictTMP["AqIndex"]['Identyfikator stacji pomiarowej']) == paramsDict['airGiosID']:
        paramsDict['dictGios']['sensors'] = {}
        sensorName = None
        sensorVal = None
        observationTime = None
        for item in dictTMP["AqIndex"]:
            #if defDBG: print(item)
            if item.startswith('Wartość indeksu dla wskaźnika '):
                sensorName = item.replace('Wartość indeksu dla wskaźnika ', '').strip()
                sensorVal = dictTMP["AqIndex"][item]
            if sensorName is not None:
                if item.startswith('Data danych źródłowych, z których policzono wartość indeksu dla wskaźnika ') and sensorName in item:
                    observationTime = dictTMP["AqIndex"][item]
                
                if observationTime is not None:
                    if defDBG: print(sensorName, sensorVal, observationTime)
                    paramsDict['dictGios']['sensors'][sensorName] = sensorVal
...
def msn_api(webContent, webFileName):
    ...
    try:
        if dataType == 'overview':
            ...
            if 0: #to jakaś dziwna dana aqi airquality (https://www.eea.europa.eu/)
                if currentDict.get('primaryPollutant', '') != '':
                    source = 'MSNAPI'
                    doUpdate = True
                    inList = True
                    name = currentDict['primaryPollutant'].split(' ')[0].replace('Ozone','O3').replace('NO₂','NO2').replace('o₃','o2')
    ...
    if 1:
        ...
        if paramsDict['entryType'] == 'client' and paramsDict['mainEntryADDR'] != '0.0.0.0':
            ...
            if paramsDict['airGiosID'] != '':
                url = 'https://api.gios.gov.pl/pjp-api/v1/rest/aqindex/getIndex/%s' % paramsDict['airGiosID']
                #url = 'https://api.gios.gov.pl/pjp-api/v1/rest/station/sensors/%s' % paramsDict['airGiosID']
                initThread(url, 'Gios')
    ...
    if len(activeSources) > 0:
        ...
        while idx < len(activeSources):
            ...
            if name in ('MSN', 'foreca'):
                ...
                observationpoints += '%s: %s' % (name, tmpDict['giosobservationpoint']['Nazwa stacji'])
