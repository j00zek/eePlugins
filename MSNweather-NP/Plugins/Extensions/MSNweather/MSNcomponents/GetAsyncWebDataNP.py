...
def analyze_ForecaMeteo(webFileName):
    ...
    DBGhourly = False
    DBGdaily = False
    FC = paramsDict['%s.FC' % webFileName]
    ...
    try:
        if dataType == 'overview':
            ...
            if len(overviewDict) == 0: 
                print('overviewDict valuse not found')
                return
            ...
            if 1: #alerty
                ...
                valInfo = ''
                units = ''
                if len(overviewDict['alerts']) > 0:
                    val = overviewDict['alerts'][0]['title']
                    for aColor in ['Żółty', 'Czerwony']:
                        if aColor in overviewDict['alerts'][0]['severity']:
                            colorCode = clr[aColor]
                    valInfo = colorCode + overviewDict['alerts'][0]['desc']
...
def mainProc():
    if 1:
        ...
        paramsDict['dictWeather'] = {'currentData': {}, 'hourlyData': {}, 'dailyData': {}}
        try:
        ...
        if paramsDict['entryType'] == 'client' and paramsDict['mainEntryADDR'] != '0.0.0.0':
            ...
            def useArly(override):
                if paramsDict['airlyAPIKEY'] != '' and paramsDict['airlyID'] != '':
                    ...
                    date, time = ISO3339toDATETIME(airlyData['current']['fromDateTime'], 1)
            ...
            def useOpenSense(override):
                if paramsDict['airOpenSenseID'] != '' and len(paramsDict.get('dictOpenSense','')) > 0:
                    ...
                    date, time = ISO3339toDATETIME(tmpDict['lastMeasurementAt'], 1)
