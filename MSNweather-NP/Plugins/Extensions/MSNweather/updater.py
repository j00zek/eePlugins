...
class WeatherMSN:
    ...
    def getData(self):
        ...
        if self.calibrateTimeLoop:
            tmpInt = datetime.now().minute
            if tmpInt in (1, 21, 41):
                tmpInt = self.TIMER_INTERVAL
            elif tmpInt < 21:
                tmpInt = (21 - tmpInt + 20) * 60
            elif tmpInt < 41:
                tmpInt = (41 - tmpInt + 20) * 60
            else:
                tmpInt = (61 - tmpInt + 20) * 60
            self.calibrateTimeLoop = True
            self.timer.startLongTimer(tmpInt)
        else:
