...
class MSNWeatherEntryConfigScreen(ConfigListScreen, Screen):
        ...
        if currItemCfg == self.current.airlyID:
            ...
            def GIOSinstallations(currentID):
                ...
                try:
                    ...
                    # wycofano webURL = 'http://api.gios.gov.pl/pjp-api/rest/station/findAll'
                    webURL = 'http://api.gios.gov.pl/pjp-api/v1/rest/station/findAll?size=500'
                    ...
                    installations = json.loads(ensure_str(webContent))
                    installations = installations['Lista stacji pomiarowych']
                    for inst in installations:
                        try:
                            myID = str(inst['Identyfikator stacji'])
                            myAddress = ensure_str(inst['Nazwa stacji']).strip()
            ...
            if len(InstList) > 0:
                ...
                return
            currItemNam = _('No GiOS installations within 20km distance, provide ID manually')
            self.session.openWithCallback(self.keyOKret, VirtualKeyBoard, title=currItemNam, text=currItemCfg.value)
    ...
    def xmlCallback(self, xmlstring, webContent):
        ...
        if xmlstring == '':
            xmlstring = 'SYSTEM RETURNED EMPTY PAGE!!!'
        if xmlstring:
            errormessage = ''
            try:
    ...
    def searchLocation(self):
        if self.current.city.value != '':
            ...
            open("/tmp/.MSNdata/searchLocation.xmlContent", "w").write(xmlContent)
            ...
            open("/tmp/.MSNdata/searchLocation.webContent", "w").write(webContent)
