...
config.mymemories.videoMode = NoSave(ConfigEnableDisable(default=False))
if pathExists('/usr/lib/enigma2/python/Plugins/Extensions/AdvancedFreePlayer/AFPtreeSelector.py'):
    config.mymemories.videoMode.value = True
...
if os.path.exists('/hdd/kamerki'):
    ...
    config.mymemories.webcamCleanFiles = ConfigInteger(default=0, limits=(0, 21))
...
if os.path.exists('/hdd/pictures'):
    config.mymemories.autoDir = ConfigDirectory(default='/hdd/pictures')
else:
...
def autorotate(path):
    def autoLog(tekst, tryb = 'a'):
        if tekst != '\n':
            tekst  += '\n'
        open("/tmp/MyMemories_Last_autorotate.log", tryb).write(tekst)
        ...
        if exif:
            ...
            if orientation_key in exif:
                ...
                if orientation in rotate_values:
                    autoLog('rotating %s by %s' % (path, rotate_values[orientation]), 'w')
                    ...
                    autoLog('image_size = %s image_ratio = %s' % (image_size,image_ratio))
                    if image_ratio < 1.4:
                        new_im = Image.new("RGB", (int(image_size[1] * 1.778), image_size[1]))
                        new_im.paste(image, ( int((new_im.size[0] - image_size[0]) / 2) , 0))
                    elif image_ratio > 1.8:
                        new_im = Image.new("RGB", (image_size[0], int(image_size[0] / 1.778)))
                        new_im.paste(image, ( 0, int((new_im.size[1] - image_size[1]) / 2 )))
    ...
    if config.mymemories.autoWakeUp.value:
        ...
        if os.path.exists(config.mymemories.currWakeUpPic.value):
            ...
            if config.mymemories.webcamMode.value == True:
                ...
                os.system("( chmod 755 /tmp/manageWebcams.sh;/tmp/manageWebcams.sh;rm -f /tmp/manageWebcams.sh ) &")
        else:
...
def Plugins(**kwargs):
    if config.mymemories.webcamMode.value == True:
        return [PluginDescriptor(name="Moje wspomnienia i kamerki", icon="MyMemories.png", where = PluginDescriptor.WHERE_PLUGINMENU, needsRestart = False, fnc=main),
...
class picshow(Screen):
    ...
    def __init__(self, session):
        ...
        if config.mymemories.videoMode.value == True:
            self.filelist = FileList(currDir, matchingPattern = "(?i)^.*\.(jpeg|jpg|jpe|png|bmp|gif|mp4)")
        else:
    ...
    def KeyOk(self):
        ...
        #self.filelist.getFileList(), self.filelist.getSelectionIndex(), self.filelist.getCurrentDirectory()
        #filelist                   , index                            , path
        elif self.filelist.getFilename().lower().endswith('.mp4'):
            openmovie = '%s/%s' % (self.filelist.getCurrentDirectory(), self.filelist.getFilename())
            if DBG: printDEBUG("switching to AFP to play %s" % openmovie)
            from Plugins.Extensions.AdvancedFreePlayer.PlayWithdmnapi import AdvancedFreePlayer
            self.session.openWithCallback(self.callbackView, AdvancedFreePlayer, openmovie, '', '4097', None, 'Fake URLlinkName To Block Delete Question')
        else:
...
class MyMemoriesSetup(Screen, ConfigListScreen):
    ...
    def createSetup(self):
        self.list = []
        self.list.append(getConfigListEntry("Wyświetlaj również nagrane filmy", config.mymemories.videoMode))
        ...
        self.list.append(getConfigListEntry("Obsługa nagrań z kamer", config.mymemories.webcamMode))
        if config.mymemories.webcamMode.value == True:
            self.list.append(getConfigListEntry("Główny katalog z plikami z kamer", config.mymemories.webcamRootDir))
            self.list.append(getConfigListEntry("Kasuj pliki po X dni (0-nie kasuj)", config.mymemories.webcamCleanFiles))
        #
        self.list.append(getConfigListEntry(" ", config.mymemories.separator))
...
class Pic_Full_View(Screen):
    ...
    def start_decode(self):
        ...
        if selItem.lower().endswith('.mp4'):
            ...
            if config.mymemories.autoRotate.value and autorotate(selItem):
                self.picload.startDecode('/tmp/rotated.jpg')
            else:
                self.picload.startDecode(selItem)
            self["point"].show()
...
class MyMemoriesWakeUpPic(Screen):
    ...
    def start_decode(self):
        ...
        if os.path.exists(config.mymemories.currWakeUpPic.value):
            ...
            if DBG: printDEBUG("\t %s does not exists, end" % config.mymemories.currWakeUpPic.value )
