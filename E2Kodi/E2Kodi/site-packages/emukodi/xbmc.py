...
def getUserAgent():
    return "Kodi/22.0-ALPHA1 (Windows NT 10.0.22000.2538; Win64; x64) App_Bitness/64 Version/22.0-ALPHA1-(21.90.700)-Git:20250702-943fffad41"
...
def executebuiltin(function, wait = False):
    ...
    """
    doRun = True
    if "Container.Update" in function: #Update current listing. Send Container.Update(path,replace) to reset the path history.
        log("xbmc.executebuiltin('Container.Update') requested")
    elif "Container.Refresh" in function: #Refresh current listing
    ...
    if doRun:    
        ...
        n3 = url.find("?", 0)
        mode = url[n3:].replace(',replace','')
        scriptPath = sys.argv[0]
        #arg2 = sys.argv[1]  
        cmd = "/usr/bin/python %s '1' '%s' 'resume:false'" % (scriptPath, mode)
        log("xbmc.executebuiltin(%s) cmd = '%s'" % (function, cmd) )
        open(os.path.join(xbmcE2.working_dir,"executebuiltin"), "w").write(cmd)
        #os.system(cmd)
...
def executeJSONRPC(jsonrpccommand):
    ...
    Example::
        ...
        ..
    """
    jsonDict = json.loads(jsonrpccommand)
    #{'jsonrpc': '2.0', 'method': 'Settings.GetSettingValue', 'params': {'setting': 'locale.audiolanguage'}, 'id': 1}
    if jsonDict['method'] == 'Settings.GetSettingValue':
        params = jsonDict['params']
        if params['setting'] == 'locale.audiolanguage':
            return json.dumps({'id': 1, 'jsonrpc': '2.0', 'result': {'value': 'mediadefault'}})
    return json.dumps({'error': '?!? %s' % cjsonrpccommand})
...
def getLanguage(ISO_639 = 1, someBool = False):
...
def translatePath(url, asURL=False):
    ...
    if url.startswith('/'):
        return url
    elif "special://home/addons/" in url:
...
class Player:
    def __init__(self, core=None):
        ...
        #dataA = listitem.data
    def play(self, url=None, listitem=None, playlist=None, windowed=None, item=None): # play([item, listitem, windowed, startpos])
