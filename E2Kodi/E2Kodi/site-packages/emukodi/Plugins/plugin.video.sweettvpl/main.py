...
if __name__ == '__main__':
    #SweetTV()
    if sys.argv[2].startswith('plugin.video.sweettvpl/playvid/'):
        #print('durny link odtwarzania z wtyczki ')
        exlink = sys.argv[2].replace('plugin.video.sweettvpl/playvid/','')
        mode = 'playtvs'
    else:
    ...
    #print('sweettvpl ENTER: mode=%r, name=%r, exlink=%r' % (mode, name, exlink))
    CreateDatas()
    if mode is None:
        mainpage('live')
    elif mode == 'loginTV':
