...
def playvid(id):
    if not helper.get_setting('logged'):
        ...
        if jsdata.get("result", None) == 'OK':
            ...
            if PROTOCOL == 'hls': #base on sss feedback
            ...
            if helper.get_setting('playerType')=='ffmpeg' and DRM==None:
                ...
                helper.PlayVid(stream_url, lic_url, PROTOCOL, DRM, flags=False, subs = subs,vod=vod, headers=headers)
