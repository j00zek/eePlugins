...
dataPath = '/etc/E2Kodi/pilot.wp/' #xbmcvfs.translatePath(addonInfo('profile'))
...
def play(id):
    if 0: #j00zek
        ...
        if channelDict is None or len(channelDict) == 0:
            return
        if channelDict.get('drms', None) is None and addon.getSetting('streamTvProtocol') != 'DASH':
    ...
    play_item.setProperty("pathHeader", headers)
