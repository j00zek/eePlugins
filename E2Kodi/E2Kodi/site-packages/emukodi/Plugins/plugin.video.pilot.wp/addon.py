...
netvicaptcha =  addon.getSetting('netvicaptcha')
...
def login():
    if mode == "0":
        ...
        netvicaptcha =  addon.getSetting('netvicaptcha')
        if len(netviapisessval) > 0 and len(netviapisessid) > 0:
            if len(netvicaptcha) > 0:
                cookies = {'netviapisessid': netviapisessid, 'netviapisessval': netviapisessval, 'netvicaptcha': netvicaptcha}
            else:
                cookies = {'netviapisessid': netviapisessid, 'netviapisessval': netviapisessval}
            saveToDB('wppilot_cache', cookiesToString(cookies))
            return cookiesToString(cookies)
...
def route():
    ...
    if action == 'BUILD_M3U':
        ...
        play(id)
    elif action == 'loginInfo':
        xbmcgui.Dialog().notification('WP Pilot', 'Aby się zalogować pobierz i uruchom na PC ExtractCookie4e2Kodi i postępuj zgodnie z wyświetlanymi komunikatami', 
