...
import sys
import six
from emukodi import xbmcaddon, xbmcgui, xbmcplugin, xbmc
...
ADDON = xbmcaddon.Addon(id='plugin.video.skylinecctv')
...
def addDir(name, url, mode, iconimage, fanart, description):
    u = sys.argv[0] + "?url=" + quote_plus(url) + "&mode=" + str(mode) + "&name=" + quote_plus(
        name) + "&iconimage=" + quote_plus(str(iconimage)) + "&description=" + quote_plus(description)
    ok = True
