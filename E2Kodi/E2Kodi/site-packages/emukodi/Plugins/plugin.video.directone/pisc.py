...
def set_pisc():
    ...
    if not xbmcgui.Dialog().yesno(
        _self.getAddonInfo("name"), _self.getLocalizedString(30394)
    ):
        return

    if _playlist_generate:
        path = os.path.join(
            _self.getSetting("playlist_folder"), _self.getSetting("playlist_file")
        )
    ...
    if _epg_generate:
        path = os.path.join(
            _self.getSetting("epp_folder"), _self.getSetting("epg_file")
        )
