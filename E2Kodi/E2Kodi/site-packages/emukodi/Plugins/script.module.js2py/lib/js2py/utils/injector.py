...
LOAD_ATTR = opcode.opmap['LOAD_ATTR'] # added to fix for p3.12 >>> https://github.com/PiotrDabkowski/Js2Py/commit/fd7df4a91fb08060914c7b1d9e94583d18f3371b by felixonmars
...
def append_arguments(code_obj, new_locals):
    ...
    is_new_load_attr = sys.version_info >= (3, 12) # added to fix for p3.12
    ...
    for inst in instructions(code_obj):
        ...
        drop_future_cache = False
        if inst.opcode == LOAD_GLOBAL or (is_new_load_attr and inst.opcode == LOAD_ATTR): # fix for p3.12 if inst.opcode == LOAD_GLOBAL:
            idx = inst.arg
            if is_new_bytecode:
                idx = idx // 2
