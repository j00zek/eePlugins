...
class IPLA(object):
    ...
    def PlayIpla(self,id_,cpid=0):
        ...
        if acc:
            ...
            if not cc:
                xbmc.log("[j00zek pgobox] if not cc:", level=xbmc.LOGINFO)

                UAcp=  'Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:91.0) Gecko/20100101 Firefox/91.0'
                stream_url = mediaSources['url']
                xbmc.log("[j00zek pgobox] stream_url: %s" % stream_url, level=xbmc.LOGINFO)
                ...
                if is_helper.check_inputstream():
                    ...
                    xbmc.log("[j00zek pgobox] inputstream.adaptive.stream_headers: %s" % 'Referer: https://polsatboxgo.pl&User-Agent=' + quote(UAcp), level=xbmc.LOGINFO)
                    ...
                    xbmc.log("[j00zek pgobox] inputstream.adaptive.license_key: %s" % LICENSE_URL + '|Content-Type=application%2Fjson&Referer=https://polsatboxgo.pl/&User-Agent=' + quote(UAcp) + '|'+data+'|JBlicense', level=xbmc.LOGINFO)
                    #xbmc.log("[j00zek pgobox] inputstream.adaptive.stream_headers: %s" % 'Referer: https://polsatboxgo.pl&User-Agent=' + quote(UAcp), level=xbmc.LOGINFO)
                    play_item.setProperty('inputstream.adaptive.license_flags', "persistent_storage")
            else:
                xbmc.log("[j00zek pgobox] if cc:", level=xbmc.LOGINFO)
