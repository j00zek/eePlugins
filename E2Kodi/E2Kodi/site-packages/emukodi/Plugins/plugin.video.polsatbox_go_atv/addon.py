...
UA='pbg_tv_android_native_cpplayer/22300'
userAgentData={
    "application": "native",
    "build": 22300,
        ...
        if now-sessUpdateTime>=24*60*60 or afterErr:
            ...
            if 'result' in resp:
                ...
                if addon.getSetting('isCodeLogged')!='true':
                    ...
                    addon.setSetting('isCodeLogged','')
                    xbmc.log('@@@Nie można odświeżyć sesji-wylogowanie (logowanie kodem)', level=xbmc.LOGINFO)
...
def getCategories():
    ...
    if 'errors' not in resp and 'result' in resp:
        ...
        xbmc.executebuiltin('Container.Update(plugin://plugin.video.polsatbox_go_atv/?mode=start,replace)')
        return False
