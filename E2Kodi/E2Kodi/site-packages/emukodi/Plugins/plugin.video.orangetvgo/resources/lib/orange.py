...
def listtv():
    ...
    helper.headers.update(helper.extra_hea())
    jsdata = helper.request_sess(helper.api_url.format('live/channel-list?hhTech=%s&deviceCat=otg'%(helper.get_setting('hh_tech'))), 'get', headers=helper.headers, json=True)#hhTech=iptv
...
def CreateBlad2(id):
    ...
    if 'erminal is not registered' in jsdata.text:
        ...
        if jsdata.status_code == 201:
            ...
            url = (helper.chan_url).format(id)+'&hhTech='+helper.get_setting('hh_tech')
            if '|' in id:
                id,subid = id.split('|')
                url = ('https://tvgo.orange.pl/gpapi/live/channel/{0}/{1}/catchup?hhTech=%s&deviceCat=otg'%(helper.get_setting('hh_tech'))).format(id,subid)#stov
...
def playtv(id):
    ...
    if not 'errCode' in jsdata:
        ...
        dt='User-Agent='+quote(helper.UA)+'&Origin='+helper.main_url#"&Cookie="+cookies
        
        cert_data = base64.b64encode(requests.get('https://cps.purpledrm.com/wv_certificate/cert_license_widevine_com.bin',headers={'User-Agent':helper.UA}).content).decode('utf-8')
    
        lic_url = license_url+'|'+dt+'|R{SSM}|'
        proxy_lic='http://127.0.0.1:%s/licensetv='%(proxyport)+lic_url

        subs=[]
        helper.PlayVid(mpdurl, lic_url, PROTOCOL, DRM, certificate=cert_data, flags=False, subs = subs)
    else:
...
def playvid(id):
    ...
    if 'playReadyToken' in jsdata:
        ...
        heaLic={
            'User-Agent':helper.UA,
            'Origin':helper.main_url
        }
        lic_url = '%s|%s|R{SSM}|'%(license_url,heaLic)
        cert_data = base64.b64encode(requests.get('https://cps.purpledrm.com/wv_certificate/cert_license_widevine_com.bin',headers={'User-Agent':helper.UA}).content).decode('utf-8')
...
def loguj():
    ...
    if helper.login and helper.password:
        ...
        if 'refreshToken' in jsdata:
            ...
            helper.set_setting('currentProfileId',jsdata.get('userInfo',None).get('currentProfileId',None))
            helper.set_setting('householdExtId',jsdata.get('householdInfo',None).get('householdExtId',None))
            helper.set_setting('hh_tech',jsdata.get('serviceInfo',None).get('hhTech',None))
            
            xbmc.log('@@@hh_tech: '+helper.get_setting('hh_tech'), level=xbmc.LOGINFO)
            ...
            xbmc.log('@@@Login_error: '+str(jsdata), level=xbmc.LOGINFO)
...
def listM3U():
    ...
    for ch in channelList:
        ...
        dataE2 += 'plugin.video.orangetvgo/main.py%3fmode=playtv/' + '%s:%s\n' % (id, title) #j00zek for E2 bouquets
    ...
    f.write(dataE2)
    f.close()
    helper.notification('Orange TV GO', 'Lista kanałów przygotowana do importu')
