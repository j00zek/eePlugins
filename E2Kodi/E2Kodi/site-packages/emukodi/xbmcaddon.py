# coding: utf-8 
import json, os, os.path, sys, re
import xml.etree.cElementTree 

from emukodi import xbmcE2

#https://romanvm.github.io/Kodistubs/_modules/xbmcaddon.html#Addon.getSettingBool
...
class Addon:
    def __init__(self, id=None):
        ...
        if id is not None:## requested from addons or scripts
            ...
            self.callingID = sys.argv[0].split('/')[-2]
            self.id = getaddonpath_params("addon_id")
            self.path = getaddonpath_params("addons_path") + "/" + str(self.callingID) + "/"
        self.pluginSetingsSLPath = '/etc/E2Kodi/%s/' % self.callingID.replace('plugin.video.', '').replace('script.module.', '')
        self.poDict = {}
    ...
    def getLocalizedString(self, idx=" "):
        idx = str(idx)
        stringPoJson = os.path.join(self.pluginSetingsSLPath, 'lang.json')
        if os.path.exists(stringPoJson):
            with open(stringPoJson,'r') as f:
                self.poDict = json.load(f)
        else:
            self.poDict = {}
        if self.poDict == {}:
            locFile = ''
            locPath = ''
            for locFile in [self.getSetting('e2k_po_file'), 'resources/language/resource.language.pl_pl/strings.po', 'resources/language/resource.language.en_gb/strings.po']:
                if locFile != '':
                    locPath = os.path.join(self.path, locFile)
            ...
            if locFile != '' and os.path.exists(locPath):
                xbmcE2.log('getLocalizedString EXISTS %s' % (locPath))
                with open(locPath, 'r') as file:
                    msgctxt = ''
                    msgid = ''
                    msgstr = ''
                    for line in file:
                        ...
                        if msgstr != '':
                            if msgctxt != '':
                                self.poDict[str(msgctxt)] = str(msgstr)
                                #xbmcE2.log('self.poDict[%s] = %s' % (msgctxt, self.poDict[msgctxt]))
                            if msgid != '':
                                self.poDict[str(msgid)] = str(msgstr)
                                #xbmcE2.log('self.poDict[%s] = %s' % (msgid, self.poDict[msgctxt]))
                            msgctxt = ''
                            msgid = ''
                            msgstr = ''
                if len(self.poDict) > 0:
                    #stringPoJson = os.path.join(self.pluginSetingsSLPath, locFile.split('language.')[1].split('/')[0] + '.json')
                    jsonData = json.dumps(self.poDict, indent=1, ensure_ascii=False)
                    with open(stringPoJson, 'w') as f:
                        f.write(jsonData)
            else:
                xbmcE2.log('getLocalizedString NOT EXISTS %s' % (locPath))
        return self.poDict.get(idx,idx)
    ...
    def getSettingBool(self, id): 
        retVal = self.getSetting(id)
        if str(retVal).lower() == 'false':
            return False
        else:
            return True
            ...
            if str(item) in line:
                ...
                if 'type="folder"' in line:
                    ...
                    if 'source="working_dir"' in line or 'source="auto"' in line:
                        xtxt = os.path.join(xbmcE2.working_dir, "")
                        #print('source=',xtxt)
                        return str(xtxt)
                    elif 'source="' in line:
                        return line.split('source="')[1].split('"')[0]
                    elif 'default="addon_path/':
                        return os.path.join(self.path, line.split('default="')[1].split('"')[0].replace('addon_path/',''))
                    elif 'default="':
    ...
    def getAddonInfo(self, item):
        ...
        if item == "profile":
            if not os.path.exists(self.pluginSetingsSLPath):
                os.system("mkdir -p " + self.pluginSetingsSLPath)
            return self.pluginSetingsSLPath
