...
import emukodislink.utils.args
import emukodislink.utils.times
from emukodislink.cache import Cache
from emukodislink.exceptions import FatalPluginError, NoStreamsError, PluginError
from emukodislink.options import Argument, Arguments, Options
from emukodislink.user_input import UserInputRequester


if TYPE_CHECKING:  # pragma: no cover
    from emukodislink.session.session import Streamlink
...
_PLUGINARGUMENT_TYPE_REGISTRY: Mapping[str, Callable[[Any], Any]] = {
    "int": int,
    "float": float,
    "bool": emukodislink.utils.args.boolean,
    "comma_list": emukodislink.utils.args.comma_list,
    "comma_list_filter": emukodislink.utils.args.comma_list_filter,
    "filesize": emukodislink.utils.args.filesize,
    "keyvalue": emukodislink.utils.args.keyvalue,
    "num": emukodislink.utils.args.num,
    "hours_minutes_seconds": emukodislink.utils.times.hours_minutes_seconds,
    "hours_minutes_seconds_float": emukodislink.utils.times.hours_minutes_seconds_float,
}
...
class Plugin(metaclass=PluginMeta):
    ...
    #: with access to its :attr:`HTTPSession <emukodislink.session.Streamlink.http>`.
    session: Streamlink
    ...
    #: The plugin's :class:`Arguments <emukodislink.options.Arguments>` collection.
    #:
    ...
    def streams(self, stream_types=None, sorting_excludes=None):
        ...
        being a :class:`Stream <emukodislink.stream.Stream>` instance.
        ...
        :returns: A :class:`dict` of stream names and :class:`Stream <emukodislink.stream.Stream>` instances
        """
    ...
    def _get_streams(self):
        ...
        Needs to return either a dict of :class:`Stream <emukodislink.stream.Stream>` instances mapped by stream name,
        or needs to act as a generator which yields tuples of stream names and :class:`Stream <emukodislink.stream.Stream>`
        instances.
        """
...
def pluginmatcher(
    ...
    :meth:`Streamlink.resolve_url() <emukodislink.session.Streamlink.resolve_url>` if multiple plugins match the input URL.
    ...
    .. code-block:: python
        ...
        from emukodislink.plugin import HIGH_PRIORITY, Plugin, pluginmatcher
...
def pluginargument(
    ...
    """
    Decorator for plugin arguments. Takes the same arguments as :class:`Argument <emukodislink.options.Argument>`.
    ...
    .. code-block:: python
        ...
        from emukodislink.plugin import Plugin, pluginargument
