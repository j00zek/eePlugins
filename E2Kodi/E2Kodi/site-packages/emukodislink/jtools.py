...
try:
    from emukodislink.e2config import getE2config
except Exception:
    from e2config import getE2config

import base64, os, sys, time, warnings

os.environ["XDG_CONFIG_HOME"] = "/etc" #aby config emukodislinka dzialal
...
def killSRVprocess(KeepPID):
    ...
    #ubicie emukodislinkSRV ale tylko starego
    CMDs.append("if [ `ps -ef|grep -v grep|grep -v %s|grep -c emukodislinkSRV` -gt 0 ];then" % KeepPID)
    CMDs.append("     (ps -ef|grep -v grep|grep -v %s|grep emukodislinkSRV|awk '{print $2}'|xargs kill)" % KeepPID)
    CMDs.append('fi')
    os.system('\n'.join(CMDs))
...
def cleanCMD(forceKill = True, KeepPID = 0): #czyszczenie smieci
    ...
    CMDs.append(r' rm -f /tmp/emukodislinkpipe-*')
    CMDs.append(r'fi')
    ...
    CMDs.append(r"find /tmp/ -maxdepth 1 -mmin +180 -name 'emukodislinkpipe-*' -exec rm -- '{}' \;")
    os.system('\n'.join(CMDs))
...
def LogToFile():
    ...
    if retVal == False or getE2config('ClearLogFile', True) == True:
            for logPath in ('/home/root','/tmp','/hdd'):
                if os.path.exists(logPath + '/emukodislinkSRV.log'):
                    os.system('rm -f /%s/emukodislinkSRV.log' % logPath)
    return retVal
    
def GetLogFileName():
    return getE2config('logPath', '/tmp') + '/emukodislinkSRV.log'
