...
import emukodislink.plugins
from emukodislink.options import Argument, Arguments

# noinspection PyProtectedMember
from emukodislink.plugin.plugin import _PLUGINARGUMENT_TYPE_REGISTRY, NO_PRIORITY, NORMAL_PRIORITY, Matcher, Matchers, Plugin
from emukodislink.utils.module import exec_module, get_finder
...
class StreamlinkPlugins:
    ...
    See the :attr:`Streamlink.plugins <emukodislink.session.Streamlink.plugins>` attribute.
    ...
    :func:`pluginmatcher <emukodislink.plugin.pluginmatcher>` and :func:`pluginargument <emukodislink.plugin.pluginargument>`
    ...
    def iter_arguments(self) -> Iterator[tuple[str, Arguments]]:
        """Iterate through all plugins and their :class:`Arguments <emukodislink.options.Arguments>`"""
        yield from (
    ...
    def iter_matchers(self) -> Iterator[tuple[str, Matchers]]:
        """Iterate through all plugins and their :class:`Matchers <emukodislink.plugin.plugin.Matchers>`"""
        yield from (
    ...
    def _load_plugin_from_finder(name: str, finder: PathEntryFinderProtocol) -> tuple[ModuleType, type[Plugin]] | None:
        try:
            ...
            mod = exec_module(finder, f"emukodislink.plugins.{name}", override=True)
