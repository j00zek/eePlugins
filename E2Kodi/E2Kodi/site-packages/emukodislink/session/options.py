...
from emukodislink.session.http import TLSNoDHAdapter
...
class StreamlinkOptions(Options):
    ...
    The following options can be accessed using the :meth:`Streamlink.get_option() <emukodislink.session.Streamlink.get_option>`
    and :meth:`Streamlink.set_option() <emukodislink.session.Streamlink.set_option>` methods, as well as the regular
    :meth:`get` and :meth:`set` methods of this :class:`Options <emukodislink.options.Options>` subclass.
