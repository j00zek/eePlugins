...
from emukodislink import __version__ as emukodislink_version
...
def get_latest() -> str:
    try:
        res = requests.get("https://pypi.python.org/pypi/emukodislink/json")
...
def check_version(force: bool = False) -> bool:
    ...
    if _parse_version(latest_version) > _parse_version(emukodislink_version):
        log.info(f"A new version of Streamlink ({latest_version}) is available!")
        cache.set("version_info_printed", True, 60 * 60 * 6)
        return False
    elif force:
        log.info(f"Your Streamlink version ({emukodislink_version}) is up to date!")
