...
import emukodislink.logger as logger
from emukodislink import NoPluginError, PluginError, StreamError, Streamlink, __version__ as emukodislink_version
from emukodislink.exceptions import FatalPluginError, StreamlinkDeprecationWarning
from emukodislink.plugin import Plugin
from emukodislink.stream.stream import Stream, StreamIO
from emukodislink.utils.named_pipe import NamedPipe
from emukodislink.utils.times import LOCAL as LOCALTIMEZONE
from emukodislink_cli.argparser import (
    ArgumentParser,
    build_parser,
    setup_plugin_args,
...
from emukodislink_cli.compat import stdout
from emukodislink_cli.console import ConsoleOutput, ConsoleUserInputRequester
from emukodislink_cli.constants import CONFIG_FILES, DEFAULT_STREAM_METADATA, LOG_DIR, PLUGIN_DIRS, STREAM_SYNONYMS
from emukodislink_cli.exceptions import StreamlinkCLIError
from emukodislink_cli.output import FileOutput, HTTPOutput, PlayerOutput
from emukodislink_cli.show_matchers import show_matchers
from emukodislink_cli.streamrunner import StreamRunner
from emukodislink_cli.utils import Formatter, datetime
from emukodislink_cli.utils.versioncheck import check_version
...
emukodislink: Streamlink = None  # type: ignore[assignment]


log = logging.getLogger("emukodislink.cli")
    ...
    while not player or player.running:
        ...
        if stream_fd and prebuffer:
            ...
            try:
                ...
                if args.player_external_http and not "emukodislinkProxy" in sys.argv[0]: sys.exit() #j00zek_patch 2
            except OSError as err:
...
def output_stream(stream, formatter: Formatter):
    ...
    try:
        with closing(output):
            ...
            # TODO: finally clean up the global variable mess and refactor the emukodislink_cli package
...
def handle_url():
    ...
    try:
        pluginname, pluginclass, resolved_url = emukodislink.resolve_url(args.url)
        options = setup_plugin_options(emukodislink, args, pluginname, pluginclass)
        plugin = pluginclass(emukodislink, resolved_url, options)
...
def print_plugins():
    ...
    pluginlist = emukodislink.plugins.get_names()
...
def can_handle_url() -> int:
    ...
    try:
        emukodislink.resolve_url(url, follow_redirect=follow_redirect)
        return 0
    except NoPluginError:
        return 1
    except KeyboardInterrupt:
...
def load_plugins(dirs: list[Path], showwarning: bool = True):
    ...
    for directory in dirs:
        if directory.is_dir():
            emukodislink.plugins.load_path(directory)
        elif showwarning:
...
def setup_config_args(parser, ignore_unknown=False):
    ...
    if emukodislink and args.url:
        # Only load first available plugin config
        with suppress(NoPluginError):
            pluginname, _pluginclass, _resolved_url = emukodislink.resolve_url(args.url)
...
def setup_emukodislink():
    """Creates the Streamlink session."""
    global emukodislink

    emukodislink = Streamlink({"user-input-requester": ConsoleUserInputRequester(console)})


def log_root_warning():
    if hasattr(os, "geteuid"):  # pragma: no branch
        if os.geteuid() == 0:
            log.info("emukodislink is running as root! Be careful!")
...
def log_current_versions() -> None:
    ...
    log.debug(f"Streamlink: {emukodislink_version}")

    return
#log.debug("Dependencies:")
    # https://peps.python.org/pep-0508/#names
    re_name = re.compile(r"[A-Z\d](?:[A-Z\d._-]*[A-Z\d])?", re.IGNORECASE)
    dependencies: list[str] = importlib.metadata.requires("emukodislink") or []
...
def setup(parser: ArgumentParser) -> None:
    ...
    setup_plugin_args(emukodislink, parser)
    ...
    log_current_arguments(emukodislink, parser)

    setup_session_options(emukodislink, args)
...
def run(parser: ArgumentParser) -> int:
    ...
    if args.version_check:
        ...
        show_matchers(emukodislink, console, args.show_matchers)
        ...
        console.msg(f"{usage}\nUse -h/--help to see the available options or read the manual at https://emukodislink.github.io/")
