...
#                   IPTV components
####################################################
from Plugins.Extensions.IPTVPlayerMario.components.iptvconfigmenu import ConfigMenu, GetMoviePlayer, GetListOfHostsNames, IsUpdateNeededForHostsChangesCommit
from Plugins.Extensions.IPTVPlayerMario.components.confighost import ConfigHostMenu, ConfigHostsMenu
from Plugins.Extensions.IPTVPlayerMario.components.configgroups import ConfigGroupsMenu

from Plugins.Extensions.IPTVPlayerMario.components.iptvfavouriteswidgets import IPTVFavouritesAddItemWidget, IPTVFavouritesMainWidget

from Plugins.Extensions.IPTVPlayerMario.iptvdm.iptvdownloadercreator import IsUrlDownloadable
from Plugins.Extensions.IPTVPlayerMario.libs.pCommon import CParsingHelper
...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import FreeSpace as iptvtools_FreeSpace, \
...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvhostgroups import IPTVHostsGroups
from Plugins.Extensions.IPTVPlayerMario.iptvdm.iptvdh import DMHelper
from Plugins.Extensions.IPTVPlayerMario.iptvdm.iptvbuffui import E2iPlayerBufferingWidget
...
from Plugins.Extensions.IPTVPlayerMario.components.iptvplayerinit import TranslateTXT as _, IPTVPlayerNeedInit, GetIPTVPlayerLastHostError, GetIPTVNotify, GetIPTVSleep

from Plugins.Extensions.IPTVPlayerMario.setup.iptvsetupwidget import IPTVSetupMainWidget
from Plugins.Extensions.IPTVPlayerMario.components.iptvplayer import IPTVStandardMoviePlayer, IPTVMiniMoviePlayer
from Plugins.Extensions.IPTVPlayerMario.components.iptvextmovieplayer import IPTVExtMoviePlayer
from Plugins.Extensions.IPTVPlayerMario.components.iptvpictureplayer import IPTVPicturePlayerWidget
from Plugins.Extensions.IPTVPlayerMario.components.iptvlist import IPTVMainNavigatorList
from Plugins.Extensions.IPTVPlayerMario.components.articleview import ArticleView
from Plugins.Extensions.IPTVPlayerMario.components.iptvarticlerichvisualizer import IPTVArticleRichVisualizer
from Plugins.Extensions.IPTVPlayerMario.components.ihost import IHost, CDisplayListItem, RetHost, CUrlItem, ArticleContent, CFavItem
...
from Plugins.Extensions.IPTVPlayerMario.p2p3.pVer import isPY2, pVersion
if not isPY2():
    basestring = str
...
class E2iPlayerWidget(Screen):
    ...
    def __init__(self, session):
        printDBG("!!!!! E2iPlayerWidget.__init__ IPTV_VERSION[%s], CPU:%s, PYTHON:%s !!!!!\n" % (E2iPlayerWidget.IPTV_VERSION,
                                                                                                 config.plugins.IPTVPlayerMario.plarform.value,
                                                                                                 pVersion()
                                                                                                ))
        ...
        selSkin = config.plugins.IPTVPlayerMario.skin.value
        ...
        if config.plugins.IPTVPlayerMario.disable_live.value:
        ...
        if config.plugins.IPTVPlayerMario.showcover.value:
            if not os_path.exists(config.plugins.IPTVPlayerMario.SciezkaCache.value):
                iptvtools_mkdirs(config.plugins.IPTVPlayerMario.SciezkaCache.value)

            if iptvtools_FreeSpace(config.plugins.IPTVPlayerMario.SciezkaCache.value, 10):
        ...
        self.visible = True
        self.bufferSize = config.plugins.IPTVPlayerMario.requestedBuffSize.value * 1024 * 1024
        ...
        if None == gDownloadManager:
            from Plugins.Extensions.IPTVPlayerMario.iptvdm.iptvdmui import GetIPTVDMNotification
            GetIPTVDMNotification().dialogInit(session)
            printDBG('============Initialize Download Menager============')
            gDownloadManager = IPTVDMApi(2, int(config.plugins.IPTVPlayerMario.IPTVDMMaxDownloadItem.value), GetIPTVDMNotification)
            if config.plugins.IPTVPlayerMario.IPTVDMRunAtStart.value:
            ...
            if self.spinnerEnabled and None != self.workThread:
                if self.workThread.isAlive():
                    ...
                    if self.hostName not in GetHostsList(fromList=True, fromHostFolder=False):
                        ...
                        if config.plugins.IPTVPlayerMario.preferredupdateserver.value == '3': #private sss repository
    ...
    def reportHostCrash(self, ret):
        try:
            if ret and config.plugins.IPTVPlayerMario.preferredupdateserver.value == '3': #private sss repository
                try:
    ...
    def blue_pressed(self):
        # For Keyboard test
        #if False:
        #    from Plugins.Extensions.IPTVPlayerMario.components.e2ivksuggestion import AutocompleteSearch
        #    from Plugins.Extensions.IPTVPlayerMario.suggestions.google import SuggestionsProvider
        #    self.session.open(GetVirtualKeyboard(), additionalParams={'autocomplete':AutocompleteSearch(SuggestionsProvider(True))})
        #    return

        # For subtitles test
        if False:
            from Plugins.Extensions.IPTVPlayerMario.components.iptvsubdownloader import IPTVSubDownloaderWidget
    ...
    def stopAutoPlaySequencer(self):
        if self.autoPlaySeqStarted:
            if not config.plugins.IPTVPlayerMario.disable_live.value:
                self.session.nav.playService(self.currentService)

            if config.plugins.IPTVPlayerMario.autoplay_start_delay.value == 0:
                self.showWindow()
    ...
    def sequencerPressOK(self):
        self.autoPlaySeqTimerValue = config.plugins.IPTVPlayerMario.autoplay_start_delay.value
    ...
    def runIPTVDM(self, callback=None):
        global gDownloadManager
        if None != gDownloadManager:
            from Plugins.Extensions.IPTVPlayerMario.iptvdm.iptvdmui import IPTVDMWidget
            if None == callback:
    ...
    def displayIcon(self, ret=None, doDecodeCover=False):
        ...
        if not config.plugins.IPTVPlayerMario.showcover.value or None == self.iconMenager:
            return
            ...
            if sel is None:
                ...
                if item.type in [CDisplayListItem.TYPE_VIDEO, CDisplayListItem.TYPE_AUDIO, CDisplayListItem.TYPE_PICTURE, CDisplayListItem.TYPE_DATA]:
                    if CDisplayListItem.TYPE_AUDIO == item.type:
                        self.bufferSize = config.plugins.IPTVPlayerMario.requestedAudioBuffSize.value * 1024
                    else:
                        self.bufferSize = config.plugins.IPTVPlayerMario.requestedBuffSize.value * 1024 * 1024
    ...
    def checkDirPin(self, callbackFun, arg1, arg2, arg3, pinCode, pin=None):
        if pin != None:
            if 4 != len(pinCode):
                pinCode = config.plugins.IPTVPlayerMario.pin.value # use default pin code if custom has wrong length
            if pin == pinCode:
    ...
    def showArticleContent(self, ret):
        ...
        if None != artItem:
            if len(artItem.images) and artItem.images[0]['url'].startswith('http'):
                self.session.openWithCallback(self.leaveArticleView, IPTVArticleRichVisualizer, artItem, {'buffering_path': config.plugins.IPTVPlayerMario.bufferingPath.value})
            else:
    ...
    def __requestCheckUpdate(self):
        ...
        if config.plugins.IPTVPlayerMario.autoCheckForUpdate.value:
    ...
    def askUpdateAvailable(self, NoUpdateCallback):
        ...
        if config.plugins.IPTVPlayerMario.autoCheckForUpdate.value \
            ...
            and self.lastPluginVersion != config.plugins.IPTVPlayerMario.updateLastCheckedVersion.value:
            ...
            config.plugins.IPTVPlayerMario.updateLastCheckedVersion.value = self.lastPluginVersion
            config.plugins.IPTVPlayerMario.updateLastCheckedVersion.save()
            configfile.save()
    ...
    def selectHost(self, arg1=None):
        ...
        if (config.plugins.IPTVPlayerMario.group_hosts.value == False or 0 == GetAvailableIconSize()):
    ...
    def selectGroup(self):
        ...
        if config.plugins.IPTVPlayerMario.AktualizacjaWmenu.value == True:
            if config.plugins.IPTVPlayerMario.preferredupdateserver.value != '4': #4 = managed by opkg
    ...
    def selectHostFromSingleList(self):
        ...
        for hostName in sortedList:
            if IsHostEnabled(hostName):
                ...
                if (config.plugins.IPTVPlayerMario.ListaGraficzna.value == False or 0 == GetAvailableIconSize()) and title[:4] == 'http':
                    try:
    ...
    def displayListOfHosts(self, arg=None):
        if config.plugins.IPTVPlayerMario.ListaGraficzna.value == False or 0 == GetAvailableIconSize():
    ...
    def selectItemCallback(self, ret, type):
        ...
        if ret:
            if ret[1] == "config":
                ...
                protectedByPin = config.plugins.IPTVPlayerMario.configProtectedByPin.value
            ...
            if self.showMessageNoFreeSpaceForIcon and hasIcon:
                self.showMessageNoFreeSpaceForIcon = False
                self.session.open(MessageBox, (_("There is no free space on the drive [%s].") % config.plugins.IPTVPlayerMario.SciezkaCache.value) + "\n" + _("New icons will not be available."), type=MessageBox.TYPE_INFO, timeout=10)
    ...
    def selectLinkForCurrVideo(self, customUrlItems=None):
        if not self.visible and not (self.autoPlaySeqStarted and
           config.plugins.IPTVPlayerMario.autoplay_start_delay.value == 0):
            self.setStatusTex("")
            self.showWindow()
    ...
    def checkBuffering(self, url):
        ...
        if protocol in ['f4m', 'uds']:
            ...
            return config.plugins.IPTVPlayerMario.buforowanie.value
        elif 'rtmp' == protocol:
            return config.plugins.IPTVPlayerMario.buforowanie_rtmp.value
        elif protocol in ['m3u8', 'em3u8']:
            return config.plugins.IPTVPlayerMario.buforowanie_m3u8.value

    def isUrlBlocked(self, url, type):
        protocol = url.meta.get('iptv_proto', '')
        if ".wmv" == self.getFileExt(url, type) and config.plugins.IPTVPlayerMario.ZablokujWMV.value:
    ...
    def writeCurrentTitleToFile(self, title):
        titleFilePath = config.plugins.IPTVPlayerMario.curr_title_file.value
        if "" != titleFilePath:
            try:
                ...
                printExc()
        if config.plugins.IPTVPlayerMario.set_curr_title.value:
            try:
    ...
    def playVideo(self, ret):
        ...
        if url != '' and CDisplayListItem.TYPE_PICTURE == self.currItem.type:
            self.session.openWithCallback(self.leavePicturePlayer, IPTVPicturePlayerWidget, url, config.plugins.IPTVPlayerMario.bufferingPath.value, self.currItem.name, {'seq_mode': self.autoPlaySeqStarted})
            ...
            bufferingPath = config.plugins.IPTVPlayerMario.bufferingPath.value
            downloadingPath = config.plugins.IPTVPlayerMario.NaszaSciezka.value
            ...
            if recorderMode:
                if None != gDownloadManager:
                    ...
                    if ret:
                        if not self.checkAutoPlaySequencer():
                            if config.plugins.IPTVPlayerMario.IPTVDMShowAfterAdd.value:
                                self.runIPTVDM()
                            else:
                ...
                if self.currItem.type == CDisplayListItem.TYPE_AUDIO:
                    gstAdditionalParams['show_iframe'] = config.plugins.IPTVPlayerMario.show_iframe.value
                    gstAdditionalParams['iframe_file_start'] = config.plugins.IPTVPlayerMario.iframe_file.value
                    gstAdditionalParams['iframe_file_end'] = config.plugins.IPTVPlayerMario.clear_iframe_file.value
                ...
                if isBufferingMode:
                    ...
                    if "mini" == player.value:
                        ...
                        if "extgstplayer" == player.value:
                            ...
                            if 'sh4' == config.plugins.IPTVPlayerMario.plarform.value: # use default value, due to small amount of RAM
    ...
    def leaveMoviePlayer(self, answer=None, lastPosition=None, clipLength=None, *args, **kwargs):
        ...
        if not config.plugins.IPTVPlayerMario.disable_live.value and not self.autoPlaySeqStarted:
            self.session.nav.playService(self.currentService)

        if 'favourites' == self.hostName and lastPosition != None and clipLength != None:
            try:
                if config.plugins.IPTVPlayerMario.favourites_use_watched_flag.value and (lastPosition * 100 / clipLength) > 80:
                    currSelIndex = self["list"].getCurrentIndex()
    ...
    def doSearchWithVirtualKeyboard(self):
        ...
        if caps.get('has_additional_params'):
            try:
                additionalParams = {}
                if caps.get('has_suggestions') and config.plugins.IPTVPlayerMario.osk_allow_suggestions.value:
                    ...
                    if suggestionsProvider == None:
                        providerAlias = config.plugins.IPTVPlayerMario.osk_default_suggestions.value
                        ...
                        if providerAlias == 'filmweb':
                            from Plugins.Extensions.IPTVPlayerMario.suggestions.filmweb import SuggestionsProvider as filmweb_Provider
                            suggestionsProvider = filmweb_Provider()
                        elif providerAlias == 'imdb':
                            from Plugins.Extensions.IPTVPlayerMario.suggestions.imdb import SuggestionsProvider as imdb_Provider
                            suggestionsProvider = imdb_Provider()
                        elif providerAlias == 'google':
                            from Plugins.Extensions.IPTVPlayerMario.suggestions.google import SuggestionsProvider as google_Provider
                            suggestionsProvider = google_Provider()
                        elif providerAlias == 'filmstarts':
                            from Plugins.Extensions.IPTVPlayerMario.suggestions.filmstarts import SuggestionsProvider as filmstarts_Provider
    ...
    def reloadList(self, params):
        ...
        if config.plugins.IPTVPlayerMario.showcover.value and self.iconMenager:
    ...
    def canByAddedToFavourites(self):
        try:
            favouritesHostActive = config.plugins.IPTVPlayerMario.hostfavourites.value
        except Exception:
