...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, GetIPTVPlayerVerstion, GetIconDir, GetAvailableIconSize
...
class PlayerSelectorWidget(Screen):
    ...
    def __init__(self, session, inList, outList, numOfLockedItems=0, groupName='', groupObj=None):
        ...
        try:
            confNumOfRow = int(config.plugins.IPTVPlayerMario.numOfRow.value)
            confNumOfCol = int(config.plugins.IPTVPlayerMario.numOfCol.value)
            # 0 - means AUTO
            if confNumOfRow > 0:
        ...
        for y in range(1, self.numOfRow + 1):
            for x in range(1, self.numOfCol + 1):
                ...
                if idx < self.numOfItems:
                    #self[strIndex].updateIcon( resolveFilename(SCOPE_PLUGINS, 'Extensions/IPTVPlayerMario/icons/PlayerSelector/' + self.currList[idx][1] + '.png'))
                    self[strIndex].setPixmap(self.pixmapList[idx])
                    self[strIndex].show()
                    idx += 1
                else:
