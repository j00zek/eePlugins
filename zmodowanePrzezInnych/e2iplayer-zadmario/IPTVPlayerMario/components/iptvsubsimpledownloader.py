...
# LOCAL import
###################################################
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, GetDefaultLang, GetTmpDir, GetSubtitlesDir, GetIconDir, RemoveDisallowedFilenameChars, iptv_system, MapUcharEncoding
from Plugins.Extensions.IPTVPlayerMario.components.iptvplayerinit import TranslateTXT as _
from Plugins.Extensions.IPTVPlayerMario.components.ihost import CDisplayListItem
...
class IPTVSubSimpleDownloaderWidget(Screen):
    ...
    def downloadFinished(self, status):
        if status != DMHelper.STS.DOWNLOADED:
            ...
            cmd = '%s "%s"' % (config.plugins.IPTVPlayerMario.uchardetpath.value, self.downloader.getFullFileName())
            printDBG("cmd[%s]" % cmd)
