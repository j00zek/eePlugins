...
from Plugins.Extensions.IPTVPlayerMario.components.cover import Cover3
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, GetIPTVDMImgDir, GetBinDir, GetSubtitlesDir, eConnectCallback, \
...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvsubtitles import IPTVSubtitlesHandler, IPTVEmbeddedSubtitlesHandler
from Plugins.Extensions.IPTVPlayerMario.tools.iptvmoviemetadata import IPTVMovieMetaDataHandler
...
from Plugins.Extensions.IPTVPlayerMario.components.iptvsubsimpledownloader import IPTVSubSimpleDownloaderWidget
from Plugins.Extensions.IPTVPlayerMario.components.iptvchoicebox import IPTVChoiceBoxWidget, IPTVChoiceBoxItem
from Plugins.Extensions.IPTVPlayerMario.components.iptvdirbrowser import IPTVFileSelectorWidget
from Plugins.Extensions.IPTVPlayerMario.components.configextmovieplayer import ConfigExtMoviePlayerBase, ConfigExtMoviePlayer
...
from Plugins.Extensions.IPTVPlayerMario.p2p3.manipulateStrings import iterDictItems, iterDictKeys, ensure_str
###################################################
# FOREIGN import
###################################################
...
class IPTVExtMoviePlayer(Screen):
    ...
    def openSubtitlesFromFileCallback(self, filePath=None):
        ...
        if None != filePath:
            ...
            cmd = '%s "%s"' % (config.plugins.IPTVPlayerMario.uchardetpath.value, filePath)
    ...
    def saveLastPlaybackTime(self):
        ...
        if config.plugins.IPTVPlayerMario.remember_last_position.value and lastPosition > 0 and self.playback['Length'] > (config.plugins.IPTVPlayerMario.remember_last_position_time.value * 60):
                self.metaHandler.setLastPosition(lastPosition)

    def loadLastPlaybackTime(self):
        if config.plugins.IPTVPlayerMario.remember_last_position.value and self.lastPosition < 1:
    ...
    def iptvDoStart(self):
        ...
        if 'gstplayer' == self.player:
            ...
            gstplayerPath = config.plugins.IPTVPlayerMario.gstplayerpath.value
            #'export GST_DEBUG="*:6" &&' +
            cmd = gstplayerPath + ' "%s"' % self.fileSRC

            # active audio track
            audioTrackIdx = self.metaHandler.getAudioTrackIdx()
            if config.plugins.IPTVPlayerMario.GSTplayer_no_IFD.value == False:
                cmd += ' %d ' % audioTrackIdx
            ...
            exteplayer3path = config.plugins.IPTVPlayerMario.exteplayer3path.value
            cmd = exteplayer3path
            ...
            if self.fileSRC.startswith('merge://') and 'audio_url' in tmpUri.meta and 'video_url' in tmpUri.meta:
                ...
                if not sts:
                    ...
                    self.showMessage(msg, MessageBox.TYPE_ERROR)
            if "://" in self.fileSRC:
                ramBufferSizeMB = config.plugins.IPTVPlayerMario.rambuffer_sizemb_network_proto.value
                ...
                ramBufferSizeMB = config.plugins.IPTVPlayerMario.rambuffer_sizemb_files.value

            if ramBufferSizeMB > 0:
                cmd += ' -b %s ' % ramBufferSizeMB

            if config.plugins.IPTVPlayerMario.stereo_software_decode.value:
                cmd += ' -s '

            if config.plugins.IPTVPlayerMario.dts_software_decode.value:
                cmd += ' -d '

            if config.plugins.IPTVPlayerMario.plarform.value in ('sh4', 'mipsel', 'armv7', 'armv5t'):
                if config.plugins.IPTVPlayerMario.wma_software_decode.value:
                    cmd += ' -w '
                if config.plugins.IPTVPlayerMario.mp3_software_decode.value:
                    cmd += ' -m '
                if config.plugins.IPTVPlayerMario.eac3_software_decode.value:
                    cmd += ' -e '
                if config.plugins.IPTVPlayerMario.ac3_software_decode.value:
                    cmd += ' -3 '
            if 'lpcm' == config.plugins.IPTVPlayerMario.software_decode_as.value:
                cmd += ' -l '

            if config.plugins.IPTVPlayerMario.aac_software_decode.value:
                cmd += ' -a 3 -p 10'
            elif config.plugins.IPTVPlayerMario.plarform.value in ('sh4', 'mipsel', 'armv7', 'armv5t'):
                cmd += ' -p 2'
                if None != self.downloader:
                    cmd += ' -o 1 '
    ...
    def createSummary(self):
        summary = config.plugins.IPTVPlayerMario.extplayer_summary.value
        create = False
        if summary == 'yes':
            create = True
        elif summary == 'auto':
