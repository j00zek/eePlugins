...
# LOCAL import
###################################################
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, GetHostsList, IsHostEnabled, SaveHostsOrderList, SortHostsList, GetHostsAliases
from Plugins.Extensions.IPTVPlayerMario.components.configbase import ConfigBaseWidget
    ...
    def __preparHostsConfigs(self, listOfHostsNames):
        ...
        for hostName in sortedList:
            try:
                optionEntry = None
                if isPY2():
                    exec('optionEntry = config.plugins.IPTVPlayerMario.host' + hostName)
                else:
                    optionEntry = eval('config.plugins.IPTVPlayerMario.host' + hostName)
