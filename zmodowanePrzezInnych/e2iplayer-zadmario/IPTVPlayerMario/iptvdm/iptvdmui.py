...
# LOCAL import
###################################################
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, eConnectCallback, GetIconDir, GetBinDir, E2PrioFix
...
from Plugins.Extensions.IPTVPlayerMario.components.iptvconfigmenu import ConfigMenu, GetMoviePlayer
from Plugins.Extensions.IPTVPlayerMario.components.iptvplayerinit import TranslateTXT as _

from Plugins.Extensions.IPTVPlayerMario.iptvdm.iptvdh import DMHelper, DMItemBase
from Plugins.Extensions.IPTVPlayerMario.iptvdm.iptvlist import IPTVDownloadManagerList
###################################################
    ...
    def refreshNewData(self, data):
        ...
        for item in newFiles:
            ...
            if len(params[0]) > 3 and params[0].lower()[-4:] in ['.flv', '.mp4']:
                fileName = os_path.join(config.plugins.IPTVPlayerMario.NaszaSciezka.value, params[0])
                skip = False
    ...
    def yellow_pressed(self):
        ...
        if not self.underRefreshing:
            ...
            try:
                ...
                printExc()
            cmd = '%s "%s" rl r' % (lsdirPath, config.plugins.IPTVPlayerMario.NaszaSciezka.value)
            printDBG("cmd[%s]" % cmd)
            ...
            if ret[1] == "play":
                ...
                if "mini" == player:
                    ...
                    if item.fileName.split('.')[-1] in ['mp3', 'm4a', 'ogg', 'wma', 'fla', 'wav', 'flac']:
                        additionalParams['show_iframe'] = config.plugins.IPTVPlayerMario.show_iframe.value
                        additionalParams['iframe_file_start'] = config.plugins.IPTVPlayerMario.iframe_file.value
                        additionalParams['iframe_file_end'] = config.plugins.IPTVPlayerMario.clear_iframe_file.value
