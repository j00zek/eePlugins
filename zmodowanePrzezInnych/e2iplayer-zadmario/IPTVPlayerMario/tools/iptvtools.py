...
from Plugins.Extensions.IPTVPlayerMario.p2p3.UrlLib import urllib2_urlopen, urllib2_Request, urllib2_URLError, urllib2_HTTPError
from Plugins.Extensions.IPTVPlayerMario.p2p3.manipulateStrings import strDecode, iterDictItems, ensure_str
###################################################
...
def GetServerKey(serverNum=None):
    if serverNum == None:
        serverNum = config.plugins.IPTVPlayerMario.preferredupdateserver.value
...
def UsePyCurl():
    return config.plugins.IPTVPlayerMario.usepycurl.value


def GetIconsHash():
    iconsHashFile = resolveFilename(SCOPE_PLUGINS, 'Extensions/IPTVPlayerMario/icons/PlayerSelector/hash.txt')
...
def GetGraphicsHash():
    graphicsHashFile = resolveFilename(SCOPE_PLUGINS, 'Extensions/IPTVPlayerMario/icons/hash.txt')
...
def E2PrioFix(cmd, factor=2):
    if '/duk' not in cmd and config.plugins.IPTVPlayerMario.plarform.value in ('mipsel', 'armv7', 'armv5t'):
...
def IsHttpsCertValidationEnabled():
    return config.plugins.IPTVPlayerMario.httpssslcertvalidation.value
...
def IsWebInterfaceModuleAvailable(chekInit=False):
    if chekInit:
        ...
        file = 'initiator'
    if (fileExists(resolveFilename(SCOPE_PLUGINS, 'Extensions/IPTVPlayerMario/Web/%s.py' % file)) or
        fileExists(resolveFilename(SCOPE_PLUGINS, 'Extensions/IPTVPlayerMario/Web/%s.pyo' % file)) or
        fileExists(resolveFilename(SCOPE_PLUGINS, 'Extensions/IPTVPlayerMario/Web/%s.pyc' % file))):
        return True
    else:
        return False
...
def GetAvailableIconSize(checkAll=True):
    iconSizes = [config.plugins.IPTVPlayerMario.IconsSize.value]
    if checkAll:
        iconSizes.extend(['135', '120', '100'])
    confirmedIconSize = 0
    for size in iconSizes:
        try:
            file = resolveFilename(SCOPE_PLUGINS, 'Extensions/IPTVPlayerMario/icons/PlayerSelector/marker{0}.png').format(int(size) + 45)
            if fileExists(file):
...
def GetLogoDir(file=''):
    return resolveFilename(SCOPE_PLUGINS, 'Extensions/IPTVPlayerMario/icons/logos/') + file


def GetPyScriptCmd(name):
    cmd = ''
    baseName = resolveFilename(SCOPE_PLUGINS, 'Extensions/IPTVPlayerMario/scripts/') + name
...
def GetJSScriptFile(file):
    return resolveFilename(SCOPE_PLUGINS, 'Extensions/IPTVPlayerMario/jsscripts/') + file


def GetUchardetPath():
    return config.plugins.IPTVPlayerMario.uchardetpath.value


def GetCmdwrapPath():
    return config.plugins.IPTVPlayerMario.cmdwrappath.value


def GetDukPath():
    return config.plugins.IPTVPlayerMario.dukpath.value
...
def GetCookieDir(file='', forceFromConfig=False):
    ...
    if gE2iPlayerTempCookieDir == None or forceFromConfig:
        cookieDir = os.path.join(config.plugins.IPTVPlayerMario.SciezkaCache.value, 'cookies/')
    else:
...
def GetJSCacheDir(fileName='', forceFromConfig=False):
    ...
    if gE2iPlayerTempJSCache == None or forceFromConfig:
        cookieDir = os.path.join(config.plugins.IPTVPlayerMario.SciezkaCache.value, 'JSCache/')
    else:
...
def GetTmpDir(fileName=''):
    path = config.plugins.IPTVPlayerMario.NaszaTMP.value
    path = path.replace('//', '/')
    mkdirs(path)
...
def GetCacheSubDir(dirName, fileName=''):
    path = os.path.join(config.plugins.IPTVPlayerMario.SciezkaCache.value, dirName)
    mkdirs(path)
    return os.path.join(path, fileName)
...
def GetIPTVDMImgDir(fileName=''):
    return os.path.join(resolveFilename(SCOPE_PLUGINS, 'Extensions/IPTVPlayerMario/icons/'), fileName)
...
def GetBinDir(fileName='', platform=None):
    ...
    return os.path.join(resolveFilename(SCOPE_PLUGINS, 'Extensions/IPTVPlayerMario/bin/'), platform, fileName)


def GetPluginDir(fileName=''):
    return os.path.join(resolveFilename(SCOPE_PLUGINS, 'Extensions/IPTVPlayerMario/'), fileName)
...
def GetSkinsDir(path=''):
    return os.path.join(resolveFilename(SCOPE_PLUGINS, 'Extensions/IPTVPlayerMario/skins/'), path)
...
def getDebugMode():
    ...
    try:
        from Components.config import config
        DBG = config.plugins.IPTVPlayerMario.debugprint.value
    except Exception:
        file = open(resolveFilename(SCOPE_CONFIG, "settings"))
        for line in file:
            if line.startswith('config.plugins.IPTVPlayerMario.debugprint='):
...
def __getHostsPath(fileName=''):
    return os.path.join(resolveFilename(SCOPE_PLUGINS), 'Extensions/IPTVPlayerMario/hosts/', fileName)
...
def GetHostsAliases():
    ...
    ret = {}
    try:
        HOST_PATH = resolveFilename(SCOPE_PLUGINS, 'Extensions/IPTVPlayerMario/hosts/')
...
def GetSkinsList():
    ...
    skins = []
    SKINS_PATH = resolveFilename(SCOPE_PLUGINS, 'Extensions/IPTVPlayerMario/skins/')
...
class CSearchHistoryHelper():
    ...
    def getHistoryList(self):
        ...
        historyList = historyList[:config.plugins.IPTVPlayerMario.search_history_size.value]
        uniqHistoryList = []
    ...
    def addHistoryItem(self, itemValue, itemType=None):
        ...
        try:
            if config.plugins.IPTVPlayerMario.search_history_size.value > 0:
...
class CMoviePlayerPerHost():
    ...
    def save(self):
        from Plugins.Extensions.IPTVPlayerMario.libs.e2ijson import dumps as json_dumps
        sts = False
        try:
...
def GetIPTVPlayerVerstion():
    try:
        from Plugins.Extensions.IPTVPlayerMario.version import IPTV_VERSION
    except Exception:
        try:
            from Plugins.Extensions.IPTVPlayerMario.version import Version as IPTV_VERSION #from opkg
        except Exception:
...
def GetIPTVPlayerComitStamp():
    try:
        from Plugins.Extensions.IPTVPlayerMario.version import COMMIT_STAMP
    except Exception:
        COMMIT_STAMP = ""
    return COMMIT_STAMP
...
def IsFPUAvailable():
    try:
        ...
        if IsFPUAvailable.available == False and config.plugins.IPTVPlayerMario.plarformfpuabi.value == 'hard_float':
            return True
    except Exception:
        printExc()
...
def IsSubtitlesParserExtensionCanBeUsed():
    try:
        if config.plugins.IPTVPlayerMario.useSubtitlesParserExtension.value:
            from Plugins.Extensions.IPTVPlayerMario.libs.iptvsubparser import _subparser as subparser
...
def defaultToolPath(fileName):
    toolPaths = []
    if isPY2():
        toolPaths.append(os.path.join(resolveFilename(SCOPE_PLUGINS, 'Extensions/IPTVPlayerMario/bin/'), config.plugins.IPTVPlayerMario.plarform.value))
        toolPaths.append(resolveFilename(SCOPE_PLUGINS, 'Extensions/IPTVPlayerMario/bin/'))
        ...
        if os.path.exists(myPath):
            ...
            if os.path.exists(cfgPath):
                ...
                with open('/etc/enigma2/settings', 'r') as f:
                    for line in f:
                        line = line.strip()
                        if line.startswith('config.plugins.IPTVPlayerMario.%s=' % cfgName):
                            defVal = line.split('=')[1]
