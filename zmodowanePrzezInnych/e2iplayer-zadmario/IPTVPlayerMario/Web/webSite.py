...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import GetPluginDir, printDBG
        ...
        if len(req.args.keys()) > 0:
            ...
            try:
                if key is None or arg is None:
                    ...
                    exec('config.plugins.IPTVPlayerMario.%s.setValue(True)\nconfig.plugins.IPTVPlayerMario.%s.save()' % (arg[3:], arg[3:]))
                    settings.configsHTML = {}
                    settings.activeHostsHTML = {}
                    return util.redirectTo("/iptvplayer/settings", req)
                elif key == 'cmd' and arg[:4] == 'OFF:':
                    print('config.plugins.IPTVPlayerMario.%s.setValue(False)\nconfig.plugins.IPTVPlayerMario.%s.save()' % (arg[4:], arg[4:]))
                    exec('config.plugins.IPTVPlayerMario.%s.setValue(False)\nconfig.plugins.IPTVPlayerMario.%s.save()' % (arg[4:], arg[4:]))
                    settings.activeHostsHTML.pop(arg[4:], None)
                    settings.activeHostsHTML.pop(arg[8:], None)
                    settings.configsHTML = {}
                    return util.redirectTo("/iptvplayer/settings", req)
                elif key[:4] == "CFG:":
                    exec('config.plugins.IPTVPlayerMario.%s.setValue("%s")\nconfig.plugins.IPTVPlayerMario.%s.save()' % (key[4:], arg, key[4:]))
                    settings.configsHTML = {}
                    return util.redirectTo("/iptvplayer/settings", req)
                elif key[:4] == "INT:":
                    exec('config.plugins.IPTVPlayerMario.%s.setValue("%s")\nconfig.plugins.IPTVPlayerMario.%s.save()' % (key[4:], arg, key[4:]))
