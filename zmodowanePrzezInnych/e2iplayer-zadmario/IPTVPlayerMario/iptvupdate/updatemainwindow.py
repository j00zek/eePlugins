...
# LOCAL import
###################################################
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, mkdirs, rmtree, FreeSpace, formatBytes, iptv_system, \
...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtypes import enum
from Plugins.Extensions.IPTVPlayerMario.iptvupdate.iptvlist import IPTVUpdateList
from Plugins.Extensions.IPTVPlayerMario.iptvdm.iptvdownloadercreator import UpdateDownloaderCreator
...
from Plugins.Extensions.IPTVPlayerMario.components.ihost import ArticleContent
...
from Plugins.Extensions.IPTVPlayerMario.p2p3.manipulateStrings import iterDictKeys
###################################################
# FOREIGN import
###################################################
...
class UpdateMainAppImpl(IUpdateObjectInterface):
    ...
    def __init__(self, session, allowTheSameVersion=False):
        ...
        self.list = []
        self.user = config.plugins.IPTVPlayerMario.iptvplayer_login.value
        self.password = config.plugins.IPTVPlayerMario.iptvplayer_password.value
    ...
    def getStepsList(self):
        self.list = []
        if config.plugins.IPTVPlayerMario.gitlab_repo.value and config.plugins.IPTVPlayerMario.preferredupdateserver.value == '2':
    ...
    def stepGetGitlab(self):
        ...
        if not sts:
            self.stepFinished(-1, msg)
            return
        serverUrl = "https://gitlab.com/{0}/e2iplayer/raw/master/IPTVPlayer/version.py".format(config.plugins.IPTVPlayerMario.gitlab_repo.value)
        self.downloader = UpdateDownloaderCreator(serverUrl)
    ...
    def stepDecryptArchive(self):
        ...
        try:
            ...
            with open(self.decKeyFilePath, "wb") as f:
                f.write(self.decKey)
            cmd = GetPyScriptCmd('decrypt') + ' "%s" "%s" "%s" ' % (resolveFilename(SCOPE_PLUGINS, 'Extensions/IPTVPlayerMario/libs/'), self.sourceArchive, self.decKeyFilePath)
        except Exception:
            printExc()
            cmd = 'fake'
    ...
    def stepGetIconsArchive(self):
        if '' == self.serversList[self.currServIdx]['icons_url'] or \
            not config.plugins.IPTVPlayerMario.ListaGraficzna.value:
            self.stepFinished(0, _('Skipped.'))
            return

        packageName = 'icons%s.tar.gz' % config.plugins.IPTVPlayerMario.IconsSize.value
    ...
    def stepCheckFiles(self):
        code, msg = self.checkVersionFile(os_path.join(self.ExtensionTmpPath, 'IPTVPlayerMario'))
    ...
    def stepRemoveUnnecessaryFiles(self):
        ...
        cmds = []
        iconSize = int(config.plugins.IPTVPlayerMario.IconsSize.value)
        if not config.plugins.IPTVPlayerMario.ListaGraficzna.value:
            iconSize = 0
        ...
        if not config.plugins.IPTVPlayerMario.IPTVWebIterface.value:
    ...
    def stepCopyGraphicsWithoutIcons(self):
        ...
        oldIconsDir = os_path.join(self.ExtensionPath, 'IPTVPlayerMario', 'icons')
        newIconsDir = os_path.join(self.ExtensionTmpPath, 'IPTVPlayerMario', 'icons')
    ...
    def stepCopyOnlyIcons(self):
        ...
        cmd = 'rm -rf "%s"/*' % (os_path.join(self.ExtensionPath, 'IPTVPlayerMario'))
        oldPlayerSelectorDir = os_path.join(self.ExtensionPath, 'IPTVPlayerMario', 'icons', 'PlayerSelector')
        newPlayerSelectorDir = os_path.join(self.ExtensionTmpPath, 'IPTVPlayerMario', 'icons', 'PlayerSelector')
    ...
    def stepCopyPostInatalledBinaries(self, init=True, code=0, msg=''):
        ...
        if init:
            ...
            if fileExists("%s/libs/iptvsubparser/_subparser.so" % os_path.join(self.ExtensionPath, 'IPTVPlayerMario')):
                self.copyBinariesCmdList.append('cp -f "%s/libs/iptvsubparser/_subparser.so" "%s/libs/iptvsubparser/_subparser.so"  2>&1 ' % (os_path.join(self.ExtensionPath, 'IPTVPlayerMario'), os_path.join(self.ExtensionTmpPath, 'IPTVPlayerMario')))

            if fileExists("%s/libs/e2icjson/e2icjson.so" % os_path.join(self.ExtensionPath, 'IPTVPlayerMario')):
                self.copyBinariesCmdList.append('cp -f "%s/libs/e2icjson/e2icjson.so" "%s/libs/e2icjson/e2icjson.so"  2>&1 ' % (os_path.join(self.ExtensionPath, 'IPTVPlayerMario'), os_path.join(self.ExtensionTmpPath, 'IPTVPlayerMario')))

            binPath = "%s/bin/" % (os_path.join(self.ExtensionPath, 'IPTVPlayerMario'))
            binariesTab = [('exteplayer3', config.plugins.IPTVPlayerMario.exteplayer3path.value),
                           ('gstplayer', config.plugins.IPTVPlayerMario.gstplayerpath.value),
                           ('wget', config.plugins.IPTVPlayerMario.wgetpath.value),
                           ('hlsdl', config.plugins.IPTVPlayerMario.hlsdlpath.value),
                           ('cmdwrap', config.plugins.IPTVPlayerMario.cmdwrappath.value),
                           ('duk', config.plugins.IPTVPlayerMario.dukpath.value),
                           ('f4mdump', config.plugins.IPTVPlayerMario.f4mdumppath.value),
                           ('uchardet', config.plugins.IPTVPlayerMario.uchardetpath.value)]
            for binItem in binariesTab:
                if binPath in binItem[1]:
                    self.copyBinariesCmdList.append('cp -f "%s/%s" "%s/bin/"  2>&1 ' % (binPath, binItem[0], os_path.join(self.ExtensionTmpPath, 'IPTVPlayerMario')))
    ...
    def stepInstallNewVersion(self):
        ...
        try:
            ...
            cmd = '%s "%s" -t 1 -T 10 -O - > /dev/null 2>&1; ' % (config.plugins.IPTVPlayerMario.wgetpath.value, url)
        except Exception:
            printExc()

        cmd += 'cp -rf "%s"/* "%s"/ 2>&1' % (os_path.join(self.ExtensionTmpPath, 'IPTVPlayerMario'), os_path.join(self.ExtensionPath, 'IPTVPlayerMario'))
        ...
        if DMHelper.STS.DOWNLOADED != status:
            ...
            if os_path.isfile(newVerFile):
                try:
                    ...
                    printExc()
                if 13 == len(newVerNum):
                    sourceUrl = "https://gitlab.com/{0}/e2iplayer/-/archive/master/e2iplayer-master.tar.gz".format(config.plugins.IPTVPlayerMario.gitlab_repo.value)
                    ...
                    self.stepFinished(0, _("GitLab version from {0} was downloaded successfully.".format(config.plugins.IPTVPlayerMario.gitlab_repo.value)))
                else:
    ...
    def __serversListDownloadFinished(self, arg, status):
        ...
        if DMHelper.STS.DOWNLOADED != status:
            ...
            try:
                ...
                return
            if config.plugins.IPTVPlayerMario.hiddenAllVersionInUpdate.value:
            ...
            if 0 < len(serversList):
                ...
                for idx in range(len(serversList)):
                    ...
                    if not config.plugins.IPTVPlayerMario.hiddenAllVersionInUpdate.value:
                        try:
                            newVerNum = int(server['version'].replace('.', ''))
                        except Exception:
                            continue
                        #printDBG("newVerNum[%s], currVerNum[%s]" % (newVerNum, currVerNum))
                        if newVerNum < currVerNum and not config.plugins.IPTVPlayerMario.downgradePossible.value:
                            continue
                        if newVerNum == currVerNum and not self.allowTheSameVersion:
                            continue
                        if 'X.X' != server['pyver'] and pythonVer != server['pyver']:
                            continue
                        if config.plugins.IPTVPlayerMario.possibleUpdateType.value not in [server['packagetype'], 'all']: #"sourcecode", "precompiled"
                            continue
                ...
                if 1 == len(options) and not config.plugins.IPTVPlayerMario.downgradePossible.value:
            ...
            if 'graphics_url' in self.serversList[self.currServIdx]:
                ...
                if self.localIconsHash == '' or self.serverIconsHash == '' or \
                   self.localIconsHash != self.serverIconsHash:
                    ...
                    if config.plugins.IPTVPlayerMario.ListaGraficzna.value:
                    ...
                    if oldGraphics:
                        if config.plugins.IPTVPlayerMario.ListaGraficzna.value:
                            ...
                            list.append(self.__getStepDesc(title=_("Copy graphics without icons."), execFunction=self.stepCopyGraphicsWithoutIcons))
                    elif config.plugins.IPTVPlayerMario.ListaGraficzna.value:
    ...
    def __doSyncCallBack(self, status, outData):
        self.cmd = None
        code, msg = self.checkVersionFile(os_path.join(self.ExtensionPath, 'IPTVPlayerMario'))
        self.stepFinished(code, msg)
