...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, GetCookieDir, GetPyScriptCmd, MergeDicts, GetDukPath, rm
...
from Plugins.Extensions.IPTVPlayerMario.tools.e2ijs import duktape_execute
...
###################################################
config.plugins.IPTVPlayerMario.bilasportpw_port = ConfigInteger(8193, (1024, 65535))


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_('PORT') + ": ", config.plugins.IPTVPlayerMario.bilasportpw_port))
    return optionList
...
class BilaSportPwApi(CBaseHostClass):
    ...
    def getResolvedVideoLink(self, videoUrl):
        ...
        if len(keyUrl):
            ...
            pyCmd = GetPyScriptCmd('livesports') + ' "%s" "%s" "%s" "%s" "%s" "%s" "%s" ' % (config.plugins.IPTVPlayerMario.bilasportpw_port.value, videoUrl, baseUrl, scriptUrl, self.HTTP_HEADER['User-Agent'], self.COOKIE_FILE, GetDukPath())
            meta = {'iptv_proto': 'em3u8'}
            meta['iptv_m3u8_key_uri_replace_old'] = '%s://' % proto
            meta['iptv_m3u8_key_uri_replace_new'] = 'http://127.0.0.1:{0}/{1}/'.format(config.plugins.IPTVPlayerMario.bilasportpw_port.value, proto)
