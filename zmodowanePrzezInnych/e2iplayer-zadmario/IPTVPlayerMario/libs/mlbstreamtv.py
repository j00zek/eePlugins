...
# LOCAL import
###################################################
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, byteify, MergeDicts, GetPyScriptCmd
...
###################################################
config.plugins.IPTVPlayerMario.mlbstreamtv_port = ConfigInteger(8193, (1024, 65535))


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_('PORT') + ": ", config.plugins.IPTVPlayerMario.mlbstreamtv_port))
    return optionList
...
class MLBStreamTVApi(CBaseHostClass):
    ...
    def getResolvedVideoLink(self, videoUrl):
        ...
        if len(keyUrl):
            ...
            pyCmd = GetPyScriptCmd('livesports') + ' "%s" "%s" "%s" "%s" "%s" ' % (config.plugins.IPTVPlayerMario.mlbstreamtv_port.value, videoUrl, baseUrl, scriptUrl, self.HTTP_HEADER['User-Agent'])
            meta = {'iptv_proto': 'em3u8'}
            meta['iptv_m3u8_key_uri_replace_old'] = '%s://' % proto
            meta['iptv_m3u8_key_uri_replace_new'] = 'http://127.0.0.1:{0}/{1}/'.format(config.plugins.IPTVPlayerMario.mlbstreamtv_port.value, proto)
