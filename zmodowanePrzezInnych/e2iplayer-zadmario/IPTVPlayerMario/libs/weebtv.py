...
###################################################
from Plugins.Extensions.IPTVPlayerMario.p2p3.UrlLib import urllib_unquote_plus
...
###################################################
config.plugins.IPTVPlayerMario.weebtv_premium = ConfigYesNo(default=True)
config.plugins.IPTVPlayerMario.weebtv_login = ConfigText(default="", fixed_size=False)
config.plugins.IPTVPlayerMario.weebtv_password = ConfigText(default="", fixed_size=False)
config.plugins.IPTVPlayerMario.weebtv_videoquality = ConfigSelection(default="1", choices=[("0", _("Low")), ("1", _("Standard")), ("2", _("High (or HD)"))])


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_("Premium user"), config.plugins.IPTVPlayerMario.weebtv_premium))
    if config.plugins.IPTVPlayerMario.weebtv_premium.value:
        optionList.append(getConfigListEntry(_("Username:"), config.plugins.IPTVPlayerMario.weebtv_login))
        optionList.append(getConfigListEntry(_("Password:"), config.plugins.IPTVPlayerMario.weebtv_password))
    optionList.append(getConfigListEntry(_("Preferred video quality:"), config.plugins.IPTVPlayerMario.weebtv_videoquality))
    return optionList
###################################################
...
class WeebTvApi:
    ...
    def _getJsonFromAPI(self, url):
        ...
        try:
            if config.plugins.IPTVPlayerMario.weebtv_premium.value:
                username = config.plugins.IPTVPlayerMario.weebtv_login.value
                password = config.plugins.IPTVPlayerMario.weebtv_password.value
            else:
                username = ''
                password = ''
    ...
    def getVideoLink(self, url):
        ...
        try:
            ...
            if sts:
                ...
                video_quality = config.plugins.IPTVPlayerMario.weebtv_videoquality.value
