...
# LOCAL import
###################################################
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, CSelOneLink, GetCookieDir
...
from Plugins.Extensions.IPTVPlayerMario.p2p3.UrlParse import urlparse, parse_qsl
...
###################################################
config.plugins.IPTVPlayerMario.moonwalk_format = ConfigSelection(default="m3u8", choices=[("hls/m3u8", "m3u8"), ("f4m", "f4m/hds")])
config.plugins.IPTVPlayerMario.moonwalk_df_format = ConfigSelection(default=9999, choices=[(0, _("the worst")), (360, "360p"), (480, "480p"), (720, "720"), (9999, _("the best"))])
config.plugins.IPTVPlayerMario.moonwalk_use_df = ConfigYesNo(default=False)
    ...
    def getDirectLinks(self, url):
        ...
        try:
            ...
            try:
                ...
                maxRes = config.plugins.IPTVPlayerMario.moonwalk_df_format.value
                tmpTab = CSelOneLink(tmpTab, __getLinkQuality, maxRes).getSortedLinks()
                if config.plugins.IPTVPlayerMario.moonwalk_use_df.value:
                    tmpTab = [tmpTab[0]]
                for item in tmpTab:
            ...
            if 'm3u8' == config.plugins.IPTVPlayerMario.moonwalk_format.value:
