...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, GetCookieDir, byteify, rm, CSelOneLink
...
from Plugins.Extensions.IPTVPlayerMario.components.ihost import CBaseHostClass
...
config.plugins.IPTVPlayerMario.videostar_streamprotocol = ConfigSelection(default="2", choices=[("1", "rtmp"), ("2", "HLS - m3u8"), ("3", "DASHS - mpd"), ("4", "DASH - mpd")])
config.plugins.IPTVPlayerMario.videostar_defquality = ConfigSelection(default="9999999999", choices=[("0", _("the worst")), ("400000", _("low")), ("950000", _("average")), ("1600000", _("high")), ("9999999999", _("the best"))])
config.plugins.IPTVPlayerMario.videostar_use_defquality = ConfigYesNo(default=True)
config.plugins.IPTVPlayerMario.videostar_show_all_channels = ConfigYesNo(default=False)
config.plugins.IPTVPlayerMario.videostar_login = ConfigText(default="", fixed_size=False)
config.plugins.IPTVPlayerMario.videostar_password = ConfigText(default="", fixed_size=False)


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_('Show all channels') + ": ", config.plugins.IPTVPlayerMario.videostar_show_all_channels))
    optionList.append(getConfigListEntry(_('Preferred streaming protocol') + ": ", config.plugins.IPTVPlayerMario.videostar_streamprotocol))
    optionList.append(getConfigListEntry(_('Preferred quality') + ": ", config.plugins.IPTVPlayerMario.videostar_defquality))
    optionList.append(getConfigListEntry(_('Use preferred quality') + ": ", config.plugins.IPTVPlayerMario.videostar_use_defquality))
    optionList.append(getConfigListEntry(_("Login") + ": ", config.plugins.IPTVPlayerMario.videostar_login))
    optionList.append(getConfigListEntry(_("Password") + ": ", config.plugins.IPTVPlayerMario.videostar_password))
    return optionList
###################################################
...
#        rm(self.COOKIE_FILE)
        ...
        login = config.plugins.IPTVPlayerMario.videostar_login.value
        password = config.plugins.IPTVPlayerMario.videostar_password.value
...
#            self.doLogin('guest', 'guest')
        ...
        try:
            ...
            for item in data[jsonChannels]:
                ...
                if not config.plugins.IPTVPlayerMario.videostar_show_all_channels.value and (item['access_status'] == 'unsubscribed' or (not self.loggedIn and guestTimeout == '0')):
                    continue
    ...
    def getVideoLink(self, cItem):
        ...
        formatId = config.plugins.IPTVPlayerMario.videostar_streamprotocol.value
        tries = 0
        while True:
            tries += 1
            try:
...
#                    self.doLogin('guest', 'guest')
        ...
        if len(urlsTab):
            maxBitrate = int(config.plugins.IPTVPlayerMario.videostar_defquality.value) * 1.3
            ...
            if config.plugins.IPTVPlayerMario.videostar_use_defquality.value:
                urlsTab = [urlsTab[0]]
