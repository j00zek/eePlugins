...
# LOCAL import
###################################################
from Plugins.Extensions.IPTVPlayerMario.libs.youtube_dl.extractor.youtube import YoutubeIE
...
from Plugins.Extensions.IPTVPlayerMario.libs.urlparserhelper import decorateUrl
...
###################################################
from Plugins.Extensions.IPTVPlayerMario.p2p3.UrlLib import urllib_urlencode
from Plugins.Extensions.IPTVPlayerMario.p2p3.UrlParse import urlparse, urlunparse, parse_qsl
...
###################################################
config.plugins.IPTVPlayerMario.ytformat = ConfigSelection(default="mp4", choices=[("flv, mp4", "flv, mp4"), ("flv", "flv"), ("mp4", "mp4")])
config.plugins.IPTVPlayerMario.ytDefaultformat = ConfigSelection(default="720", choices=[("0", _("the worst")), ("144", "144p"), ("240", "240p"), ("360", "360p"), ("720", "720p"), ("1080", "1080p"), ("1440", "1440p"), ("2160", "2160p"), ("9999", _("the best"))])
config.plugins.IPTVPlayerMario.ytUseDF = ConfigYesNo(default=True)
config.plugins.IPTVPlayerMario.ytAgeGate = ConfigYesNo(default=False)
config.plugins.IPTVPlayerMario.ytVP9 = ConfigYesNo(default=False)
config.plugins.IPTVPlayerMario.ytShowDash = ConfigSelection(default="auto", choices=[("auto", _("Auto")), ("true", _("Yes")), ("false", _("No"))])
config.plugins.IPTVPlayerMario.ytSortBy = ConfigSelection(default="A", choices=[("A", _("Relevance")), ("I", _("Upload date")), ("M", _("View count")), ("E", _("Rating"))])
...
class YouTubeParser():
    ...
    @staticmethod
    def isDashAllowed():
        value = config.plugins.IPTVPlayerMario.ytShowDash.value
        printDBG("ALLOW DASH: >> %s" % value)
        if value == "true" and IsExecutable('ffmpeg'):
            return True
        elif value == "auto" and IsExecutable('ffmpeg') and IsExecutable(config.plugins.IPTVPlayerMario.exteplayer3path.value):
            return True
        else:
            return False
    ...
    @staticmethod
    def isVP9Allowed():
        value = config.plugins.IPTVPlayerMario.ytVP9.value
    ...
    def isAgeGateAllowed():
        value = config.plugins.IPTVPlayerMario.ytAgeGate.value
    ...
    def getThumbnailUrl(self, thumbJson, maxWidth=1000, hq=False):
        ...
        try:
            ...
            if hq or (not config.plugins.IPTVPlayerMario.allowedcoverformats.value) or config.plugins.IPTVPlayerMario.allowedcoverformats.value != 'all':
#                if 'hqdefault' in url:
