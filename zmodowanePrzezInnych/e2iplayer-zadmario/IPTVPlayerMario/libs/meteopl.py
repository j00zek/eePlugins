...
# LOCAL import
###################################################
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, GetCookieDir
...
###################################################
config.plugins.IPTVPlayerMario.meteopl_locality = ConfigText(default="", fixed_size=False)


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry("Miejscowość:", config.plugins.IPTVPlayerMario.meteopl_locality))
    return optionList
...
class MeteoPLApi(CBaseHostClass):
    ...
    def getList(self, cItem):
        ...
        if initList:
            if '' != config.plugins.IPTVPlayerMario.meteopl_locality.value:
                try:
                    params = dict(cItem)
                    params.update({'init_list': False, 'meteo_cat': 'name', 'meteo_name': config.plugins.IPTVPlayerMario.meteopl_locality.value.decode('utf-8').encode('iso-8859-2'), 'url': self.getFullUrl('um/php/gpp/next.php'), 'title': config.plugins.IPTVPlayerMario.meteopl_locality.value})
