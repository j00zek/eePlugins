...
# LOCAL import
###################################################
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, GetBinDir, GetTmpDir, GetPyScriptCmd, IsFPUAvailable, ReadGnuMIPSABIFP, GetResourcesServerUri
from Plugins.Extensions.IPTVPlayerMario.setup.iptvsetuphelper import CBinaryStepHelper, CCmdValidator, SetupDownloaderCmdCreator
...
class IPTVSetupImpl:
    def __init__(self, finished, chooseQuestion, showMessage, setInfo):
        ...
        self.subparserPaths = [resolveFilename(SCOPE_PLUGINS, 'Extensions/IPTVPlayerMario/libs/iptvsubparser/_subparser.so')]

        # e2icjson
        self.e2icjsonVersion = 10202 #'1.2.2' int(z[0]) * 10000 + int(z[1]) * 100 + int(z[2])
        self.e2icjsonPaths = [resolveFilename(SCOPE_PLUGINS, 'Extensions/IPTVPlayerMario/libs/e2icjson/e2icjson.so')]

        # hlsdl
        self.hlsdlVersion = 0.21
        self.hlsdlPaths = [resolveFilename(SCOPE_PLUGINS, 'Extensions/IPTVPlayerMario/bin/hlsdl'), "/usr/bin/hlsdl"]

        # cmdwrap
        self.cmdwrapVersion = 2
        self.cmdwrapPaths = [resolveFilename(SCOPE_PLUGINS, 'Extensions/IPTVPlayerMario/bin/cmdwrap'), "/usr/bin/cmdwrap"]

        # duk
        self.dukVersion = 6 # "2.1.99 [experimental]" # real version
        self.dukPaths = [resolveFilename(SCOPE_PLUGINS, 'Extensions/IPTVPlayerMario/bin/duk'), "/usr/bin/duk"]
    ...
    def platformDetect(self):
        ...
        for platform in self.supportedPlatforms:
            platformtesterPath = resolveFilename(SCOPE_PLUGINS, "Extensions/IPTVPlayerMario/bin/%s/platformtester" % platform)
            try:
    ...
    def platformDetectFinished(self, stsTab, dataTab):
        ...
        def _saveConfig(platform):
            ...
            config.plugins.IPTVPlayerMario.plarform.value = self.platform
            printDBG("IPTVSetupImpl.platformDetectFinished platform[%s]" % self.platform)
            config.plugins.IPTVPlayerMario.plarform.save()
            configfile.save()
    ...
    def detectFPU(self):
        ...
        if config.plugins.IPTVPlayerMario.plarform.value == 'mipsel':
            ...
            if abiFP not in [-1, 0]:
                if abiFP == 3:
                    ...
                    val = "hard_float"
                if config.plugins.IPTVPlayerMario.plarformfpuabi.value != val:
                    config.plugins.IPTVPlayerMario.plarformfpuabi.value = val
                    config.plugins.IPTVPlayerMario.plarformfpuabi.save()
                    configfile.save()
                self.hasAbiFlags = hasAbiFlags
                self.abiFP = val
            printDBG(">> detectFPU hasAbiFlags[%s] abiFP[%s] -> [%s]" % (self.hasAbiFlags, self.abiFP, IsFPUAvailable()))

        if config.plugins.IPTVPlayerMario.plarform.value != 'mipsel' or IsFPUAvailable() or config.plugins.IPTVPlayerMario.plarformfpuabi.value != '':
    ...
    def detectFPUFinished(self, stsTab, dataTab):
        ...
        def _saveConfig(isHardFloat):
            if isHardFloat:
                config.plugins.IPTVPlayerMario.plarformfpuabi.value = "hard_float"
            else:
                config.plugins.IPTVPlayerMario.plarformfpuabi.value = "soft_float"
...
#            url = server + 'bin/' + platform + ('/%s%s' % (binName, old)) + '_openssl' + openSSLVersion
        ...
        self.stepHelper = CBinaryStepHelper("wget", self.platform, self.openSSLVersion, config.plugins.IPTVPlayerMario.wgetpath)
    ...
    def rtmpdumpStep(self, ret=None):
        ...
        self.stepHelper = CBinaryStepHelper("rtmpdump", self.platform, self.openSSLVersion, config.plugins.IPTVPlayerMario.rtmpdumppath)
    ...
    def uchardetStep(self, ret=None):
        ...
        self.stepHelper = CBinaryStepHelper("uchardet", self.platform, self.openSSLVersion, config.plugins.IPTVPlayerMario.uchardetpath)
    ...
    def subparserStep(self, ret=None):
        ...
        def _detectCmdBuilder(path):
            cmd = GetPyScriptCmd('modver') + ' "%s" iptvsubparser _subparser ' % resolveFilename(SCOPE_PLUGINS, 'Extensions/IPTVPlayerMario/libs/')
            return cmd
    ...
    def e2icjsonStep(self, ret=None):
        ...
        def _detectCmdBuilder(path):
            cmd = GetPyScriptCmd('modver') + ' "%s" e2icjson e2icjson ' % resolveFilename(SCOPE_PLUGINS, 'Extensions/IPTVPlayerMario/libs/')
            return cmd
    ...
    def hlsdlStep(self, ret=None):
        ...
        self.stepHelper = CBinaryStepHelper("hlsdl", self.platform, self.openSSLVersion, config.plugins.IPTVPlayerMario.hlsdlpath)
    ...
    def cmdwrapStep(self, ret=None):
        ...
        self.stepHelper = CBinaryStepHelper("cmdwrap", self.platform, self.openSSLVersion, config.plugins.IPTVPlayerMario.cmdwrappath)
        msg1 = _("cmdwrap tool")
        msg2 = _("\nFor more info please ask samsamsam@o2.pl")
    ...
    def dukStep(self, ret=None):
        ...
        self.stepHelper = CBinaryStepHelper("duk", self.platform, self.openSSLVersion, config.plugins.IPTVPlayerMario.dukpath)
        msg1 = _("duktape")
    ...
    def f4mdumpStep(self, ret=None):
        ...
        self.stepHelper = CBinaryStepHelper("f4mdump", self.platform, self.openSSLVersion, config.plugins.IPTVPlayerMario.f4mdumppath)
    ...
    def exteplayer3Step(self, ret=None):
        ...
        def _downloadCmdBuilder(ffmpegVersion, binName, platform, openSSLVersion, server, tmpPath):
            ...
            cmd = SetupDownloaderCmdCreator(url, tmpFile) + ' > /dev/null 2>&1'
            return cmd
        self.stepHelper = CBinaryStepHelper("exteplayer3", self.platform, self.openSSLVersion, config.plugins.IPTVPlayerMario.exteplayer3path)
    ...
    def gstplayerStep(self, ret=None):
        ...
        if self.gstreamerVersion == "0.10" and self.platform in ['armv5t']:
            ...
            def _downloadCmdBuilder(binName, platform, openSSLVersion, server, tmpPath):
                ...
                cmd = SetupDownloaderCmdCreator(url, tmpFile) + ' > /dev/null 2>&1'
                return cmd
            self.stepHelper = CBinaryStepHelper("gstplayer", self.platform, self.openSSLVersion, config.plugins.IPTVPlayerMario.gstplayerpath)
