...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, byteify, rm
...
###################################################
config.plugins.IPTVPlayerMario.bbc_login = ConfigText(default="", fixed_size=False)
config.plugins.IPTVPlayerMario.bbc_password = ConfigText(default="", fixed_size=False)


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_("Use web-proxy (it may be illegal):"), config.plugins.IPTVPlayerMario.bbc_use_web_proxy))
    # there is no support to login when web-proxy option is used
    if not config.plugins.IPTVPlayerMario.bbc_use_web_proxy.value:
        optionList.append(getConfigListEntry(_("e-mail") + ":", config.plugins.IPTVPlayerMario.bbc_login))
        optionList.append(getConfigListEntry(_("password") + ":", config.plugins.IPTVPlayerMario.bbc_password))

    optionList.append(getConfigListEntry(_("Default video quality:"), config.plugins.IPTVPlayerMario.bbc_default_quality))
    optionList.append(getConfigListEntry(_("Use default video quality:"), config.plugins.IPTVPlayerMario.bbc_use_default_quality))
    optionList.append(getConfigListEntry(_("Preferred format:"), config.plugins.IPTVPlayerMario.bbc_prefered_format))
    return optionList
###################################################
...
class BBCSport(CBaseHostClass):
    ...
    def tryTologin(self):
        ...
        if config.plugins.IPTVPlayerMario.bbc_use_web_proxy.value:
            if False != self.loggedIn:
                rm(self.COOKIE_FILE)
                self.loggedIn = False
            return False

        if None == self.loggedIn or self.login != config.plugins.IPTVPlayerMario.bbc_login.value or\
            self.password != config.plugins.IPTVPlayerMario.bbc_password.value:

            self.login = config.plugins.IPTVPlayerMario.bbc_login.value
            self.password = config.plugins.IPTVPlayerMario.bbc_password.value
