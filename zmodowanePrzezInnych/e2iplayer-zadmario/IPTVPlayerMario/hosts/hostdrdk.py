...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, GetLogoDir, byteify
from Plugins.Extensions.IPTVPlayerMario.libs.urlparserhelper import getDirectM3U8Playlist, getF4MLinksWithMeta
from Plugins.Extensions.IPTVPlayerMario.libs.dk_channels import TV2RChannel
###################################################
...
###################################################
config.plugins.IPTVPlayerMario.drdk_myip = ConfigText(default="213.173.226.190", fixed_size=False)


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_("Geolocation IP:"), config.plugins.IPTVPlayerMario.drdk_myip))
    return optionList
###################################################
...
class DRDK(CBaseHostClass):
    ...
    def __init__(self):
        ...
        if '' != config.plugins.IPTVPlayerMario.drdk_myip.value:
            self.cm.HEADER = {'X-Forwarded-For': config.plugins.IPTVPlayerMario.drdk_myip.value}
                ...
                for serv in cItem["StreamingServers"]:
                    ...
                    for qual in serv["Qualities"]:
                        for stream in qual["Streams"]:
                            ...
                            ip = config.plugins.IPTVPlayerMario.drdk_myip.value
                            if '' != ip:
