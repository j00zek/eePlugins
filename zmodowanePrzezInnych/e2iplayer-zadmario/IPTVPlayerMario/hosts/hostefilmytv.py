...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, GetTmpDir, byteify, rm
...
###################################################
config.plugins.IPTVPlayerMario.efilmytv_login = ConfigText(default="", fixed_size=False)
config.plugins.IPTVPlayerMario.efilmytv_password = ConfigText(default="", fixed_size=False)


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_("login") + ":", config.plugins.IPTVPlayerMario.efilmytv_login))
    optionList.append(getConfigListEntry(_("password") + ":", config.plugins.IPTVPlayerMario.efilmytv_password))
    return optionList
###################################################
...
#            item = item.split('</div>', 1)
    ...
    def tryTologin(self):
        ...
        if self.login == config.plugins.IPTVPlayerMario.efilmytv_login.value and \
           self.password == config.plugins.IPTVPlayerMario.efilmytv_password.value:
           return

        self.login = config.plugins.IPTVPlayerMario.efilmytv_login.value
        self.password = config.plugins.IPTVPlayerMario.efilmytv_password.value
