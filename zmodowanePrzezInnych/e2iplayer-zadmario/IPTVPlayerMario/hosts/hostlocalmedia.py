...
from Plugins.Extensions.IPTVPlayerMario.components.ihost import CHostBase, CBaseHostClass, RetHost, CUrlItem, ArticleContent
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, ReadTextFile, GetBinDir, \
                                                          formatBytes, GetTmpDir, mkdirs
from Plugins.Extensions.IPTVPlayerMario.libs.urlparserhelper import getDirectM3U8Playlist, getF4MLinksWithMeta
from Plugins.Extensions.IPTVPlayerMario.libs.urlparser import urlparser
from Plugins.Extensions.IPTVPlayerMario.libs.m3uparser import ParseM3u
...
from Plugins.Extensions.IPTVPlayerMario.components.iptvchoicebox import IPTVChoiceBoxItem
...
# E2 GUI COMMPONENTS
###################################################
from Plugins.Extensions.IPTVPlayerMario.components.asynccall import iptv_execute
...
###################################################
config.plugins.IPTVPlayerMario.local_alphasort = ConfigSelection(default="alphabetically", choices=[("alphabetically", _("Alphabetically")), ("none", _("None"))])
config.plugins.IPTVPlayerMario.local_showfilesize = ConfigYesNo(default=True)
config.plugins.IPTVPlayerMario.local_showhiddensdir = ConfigYesNo(default=False)
config.plugins.IPTVPlayerMario.local_showhiddensfiles = ConfigYesNo(default=False)
config.plugins.IPTVPlayerMario.local_maxitems = ConfigInteger(1000, (10, 1000000))


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_("Sort alphabetically"), config.plugins.IPTVPlayerMario.local_alphasort))
    optionList.append(getConfigListEntry(_("Show file size"), config.plugins.IPTVPlayerMario.local_showfilesize))
    optionList.append(getConfigListEntry(_("Show hiddens files"), config.plugins.IPTVPlayerMario.local_showhiddensfiles))
    optionList.append(getConfigListEntry(_("Show hiddens catalogs"), config.plugins.IPTVPlayerMario.local_showhiddensdir))
    optionList.append(getConfigListEntry(_("Max items per page"), config.plugins.IPTVPlayerMario.local_maxitems))
    return optionList
###################################################
...
class LocalMedia(CBaseHostClass):
    ...
    def prepareCmd(self, path, start, end):
        ...
        try:
            ...
            printExc()
        if config.plugins.IPTVPlayerMario.local_showhiddensdir.value:
            dWildcards = '[^.]*|.[^.]*|..[^.]*'
        else:
            dWildcards = '[^.]*'
        ...
        for ext in extensions:
            if config.plugins.IPTVPlayerMario.local_showhiddensfiles.value:
                wilcard = '*'
            else:
                wilcard = '[^.]*'
            insensitiveExt = ''
        ...
        if config.plugins.IPTVPlayerMario.local_showfilesize.value:
            cmd += " 1 "
        return cmd
    ...
    def listsMainMenu(self, cItem):
        ...
        # list mount points
        predefined = [{'title': _('Downloads'), 'path': config.plugins.IPTVPlayerMario.NaszaSciezka.value}, {'title': _('rootfs'), 'path': '/'}]
    ...
    def listDir(self, cItem):
        ...
        end = start + config.plugins.IPTVPlayerMario.local_maxitems.value
    ...
    def addFromTab(self, params, tab, path, category='', need_resolve=0):
        ...
        if config.plugins.IPTVPlayerMario.local_alphasort.value == 'alphabetically':
            try:
