...
from Plugins.Extensions.IPTVPlayerMario.components.ihost import CHostBase, CBaseHostClass, CDisplayListItem, RetHost, CUrlItem
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, GetLogoDir, GetCookieDir, byteify, CSelOneLink, rm
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtypes import strwithmeta
from Plugins.Extensions.IPTVPlayerMario.libs.urlparserhelper import getDirectM3U8Playlist
###################################################
from Plugins.Extensions.IPTVPlayerMario.p2p3.UrlLib import urllib_quote_plus, urllib_urlencode
###################################################
# FOREIGN import
###################################################
import datetime
...
###################################################
config.plugins.IPTVPlayerMario.laola1tv_defquality = ConfigSelection(default="1000000", choices=[("0", _("The worst")), ("500000", _("Low")), ("1000000", _("Mid")), ("1500000", _("High")), ("9000000", _("The best"))])
config.plugins.IPTVPlayerMario.laola1tv_onelink = ConfigYesNo(default=False)
config.plugins.IPTVPlayerMario.laola1tv_portal = ConfigSelection(default="int", choices=[("at", "AT"), ("de", "DE"), ("int", "INT")])
config.plugins.IPTVPlayerMario.laola1tv_language = ConfigSelection(default="en", choices=[("en", _("English")), ("de", _("Deutsch"))])
config.plugins.IPTVPlayerMario.laola1tv_myip1 = ConfigText(default="146.0.32.8", fixed_size=False)
config.plugins.IPTVPlayerMario.laola1tv_myip2 = ConfigText(default="85.128.142.29", fixed_size=False)


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_("Video default quality:"), config.plugins.IPTVPlayerMario.laola1tv_defquality))
    optionList.append(getConfigListEntry(_("Use default quality:"), config.plugins.IPTVPlayerMario.laola1tv_onelink))
    optionList.append(getConfigListEntry(_("Portal:"), config.plugins.IPTVPlayerMario.laola1tv_portal))
    optionList.append(getConfigListEntry(_("Language:"), config.plugins.IPTVPlayerMario.laola1tv_language))
    optionList.append(getConfigListEntry(_("Alternative geolocation IP 1:"), config.plugins.IPTVPlayerMario.laola1tv_myip1))
    optionList.append(getConfigListEntry(_("Alternative geolocation IP 2:"), config.plugins.IPTVPlayerMario.laola1tv_myip2))
    return optionList
###################################################
...
class Laola1TV(CBaseHostClass):
    ...
    def getMainUrl(self):
        return (self.MAIN_URL + '{language}-{portal}/').format(language=config.plugins.IPTVPlayerMario.laola1tv_language.value, portal=config.plugins.IPTVPlayerMario.laola1tv_portal.value)
    ...
    def getLinksForVideo(self, cItem):
        ...
        for streamaccess in streamaccessTab:
            for myip in ['', config.plugins.IPTVPlayerMario.laola1tv_myip1.value, config.plugins.IPTVPlayerMario.laola1tv_myip2.value]:
                if '' != myip:
        ...
        if 0 < len(urlTab):
            max_bitrate = int(config.plugins.IPTVPlayerMario.laola1tv_defquality.value)
            ...
            if config.plugins.IPTVPlayerMario.laola1tv_onelink.value:
                urlTab = [urlTab[0]]
        else:
