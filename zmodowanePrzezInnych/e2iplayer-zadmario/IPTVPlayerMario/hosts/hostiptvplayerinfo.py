...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, byteify, GetIPTVPlayerComitStamp
from Plugins.Extensions.IPTVPlayerMario.components.configbase import COLORS_DEFINITONS
###################################################
...
###################################################
config.plugins.IPTVPlayerMario.iptvplayerinfo_currversion_color = ConfigSelection(default="#008000", choices=COLORS_DEFINITONS)


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_("The color of the current version"), config.plugins.IPTVPlayerMario.iptvplayerinfo_currversion_color))
    return optionList
###################################################
...
class IPTVPlayerInfo(CBaseHostClass):
    ...
    def listCommits(self, cItem, nextCategory):
        ...
        url = self.getFullUrl('/%s/e2iplayer/-/commits/master?limit=%d&offset=%d' % (config.plugins.IPTVPlayerMario.gitlab_repo.value, ITEMS_PER_PAGE, page * ITEMS_PER_PAGE))
            ...
            for item in data:
                ...
                for it in item:
                    ...
                    if currCommitStamp != '' and currCommitStamp == stamp:
                        params['text_color'] = config.plugins.IPTVPlayerMario.iptvplayerinfo_currversion_color.value
