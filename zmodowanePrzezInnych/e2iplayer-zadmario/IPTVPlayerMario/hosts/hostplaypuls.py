...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import CSelOneLink, printDBG, printExc, byteify
...
from Plugins.Extensions.IPTVPlayerMario.p2p3.manipulateStrings import iterDictItems
###################################################
# FOREIGN import
###################################################
...
###################################################
config.plugins.IPTVPlayerMario.playpuls_defaultformat = ConfigSelection(default="999999", choices=[("0", "najgorsza"), ("600", "średnia"), ("800", "dobra"), ("999999", "najlepsza")])
config.plugins.IPTVPlayerMario.playpuls_usedf = ConfigYesNo(default=False)
config.plugins.IPTVPlayerMario.playpuls_defaultproto = ConfigSelection(default="hls", choices=[("rtmp", "dash (mpd)"), ("hls", "hls (m3u8)")])
config.plugins.IPTVPlayerMario.playpuls_proxy = ConfigYesNo(default=False)


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry("Preferowany protokół:", config.plugins.IPTVPlayerMario.playpuls_defaultproto))
    optionList.append(getConfigListEntry("Domyślny jakość video:", config.plugins.IPTVPlayerMario.playpuls_defaultformat))
    optionList.append(getConfigListEntry("Używaj domyślnej jakości video:", config.plugins.IPTVPlayerMario.playpuls_usedf))
    optionList.append(getConfigListEntry("PlayPuls korzystaj z proxy?", config.plugins.IPTVPlayerMario.playpuls_proxy))
    return optionList
###################################################
...
class Playpuls(CBaseHostClass):
    def __init__(self):
        ...
        CBaseHostClass.__init__(self, {'history': 'Playpuls', 'proxyURL': config.plugins.IPTVPlayerMario.proxyurl.value, 'useProxy': config.plugins.IPTVPlayerMario.playpuls_proxy.value, 'cookie': 'playpuls.cookie'})
        self.cacheMenuTree = []
    ...
    def getLinksForVideo(self, cItem):
        ...
        sources = []
        proto = config.plugins.IPTVPlayerMario.playpuls_defaultproto.value
        ...
        if 0 < len(videoUrls):
            max_bitrate = int(config.plugins.IPTVPlayerMario.playpuls_defaultformat.value)

            def __getLinkQuality(itemLink):
                return int(itemLink['bitrate'])
            videoUrls = CSelOneLink(videoUrls, __getLinkQuality, max_bitrate).getSortedLinks()
            if config.plugins.IPTVPlayerMario.playpuls_usedf.value:
