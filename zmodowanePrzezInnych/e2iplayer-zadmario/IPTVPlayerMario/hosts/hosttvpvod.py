...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import CSelOneLink, printDBG, printExc, MergeDicts, readCFG
from Plugins.Extensions.IPTVPlayerMario.libs.urlparserhelper import getDirectM3U8Playlist, getMPDLinksWithMeta
...
###################################################
config.plugins.IPTVPlayerMario.tvpvod_premium = ConfigYesNo(default=False)
config.plugins.IPTVPlayerMario.tvpvod_login = ConfigText(default=readCFG('tvpvod_login', ""), fixed_size=False)
config.plugins.IPTVPlayerMario.tvpvod_password = ConfigText(default=readCFG('tvpvod_password', ""), fixed_size=False)

config.plugins.IPTVPlayerMario.tvpVodProxyEnable = ConfigYesNo(default=False)
config.plugins.IPTVPlayerMario.tvpVodDefaultformat = ConfigSelection(default="590000", choices=[("360000", "320x180"),
                                                                                               ("590000", "398x224"),
                                                                                               ("820000", "480x270"),
                                                                                               ("1250000", "640x360"),
                                                                                               ("1750000", "800x450"),
                                                                                               ...
                                                                                               ("6500000", "1600x900"),
                                                                                               ("9100000", "1920x1080")])
config.plugins.IPTVPlayerMario.tvpVodUseDF = ConfigYesNo(default=True)
config.plugins.IPTVPlayerMario.tvpVodPreferedformat = ConfigSelection(default="mp4", choices=[("mp4", "MP4"), ("m3u8", "HLS/m3u8"), ("mpd", "MPD")])
...
def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry("Strefa Widza", config.plugins.IPTVPlayerMario.tvpvod_premium))
    if config.plugins.IPTVPlayerMario.tvpvod_premium.value:
        optionList.append(getConfigListEntry("  email:", config.plugins.IPTVPlayerMario.tvpvod_login))
        optionList.append(getConfigListEntry("  hasło:", config.plugins.IPTVPlayerMario.tvpvod_password))
    optionList.append(getConfigListEntry("Peferowany format wideo", config.plugins.IPTVPlayerMario.tvpVodPreferedformat))
    optionList.append(getConfigListEntry("Domyślna jakość wideo", config.plugins.IPTVPlayerMario.tvpVodDefaultformat))
    optionList.append(getConfigListEntry("Używaj domyślnej jakości wideo:", config.plugins.IPTVPlayerMario.tvpVodUseDF))
    optionList.append(getConfigListEntry("Korzystaj z proxy?", config.plugins.IPTVPlayerMario.tvpVodProxyEnable))
    return optionList
###################################################
...
class TvpVod(CBaseHostClass, CaptchaHelper):
    ...
    def __init__(self):
        ...
        CBaseHostClass.__init__(self, {'history': 'TvpVod', 'cookie': 'tvpvod.cookie', 'proxyURL': config.plugins.IPTVPlayerMario.proxyurl.value, 'useProxy': config.plugins.IPTVPlayerMario.tvpVodProxyEnable.value})
    ...
    def tryTologin(self):
        self.loginMessage = ''
        email = config.plugins.IPTVPlayerMario.tvpvod_login.value
        password = config.plugins.IPTVPlayerMario.tvpvod_password.value
...
#            printDBG("TvpVod.getLinksForVideo data [%s]" % data)
            ...
            if '' != hlsUrl:
                ...
                if 1 < len(videoTab):
                    max_bitrate = int(config.plugins.IPTVPlayerMario.tvpVodDefaultformat.value)

                    oneLink = CSelOneLink(videoTab, __getLinkQuality, max_bitrate)
                    if config.plugins.IPTVPlayerMario.tvpVodUseDF.value:
...
#            printDBG("getLinksForVideo data [%s]" % data)
            ...
            if config.plugins.IPTVPlayerMario.tvpVodPreferedformat.value == 'm3u8':
                hlsUrl = self.cm.ph.getSearchGroups(data, '''['"](http[^'^"]*?\.m3u8[^'^"]*?)['"]''')[0].replace('\/', '/')
                if '' != hlsUrl:
                    videoTab = getDirectM3U8Playlist(hlsUrl, checkExt=False, variantCheck=False)
                    if 1 < len(videoTab):
                    ...
                    if config.plugins.IPTVPlayerMario.tvpVodUseDF.value:
                        ...
                        videoTab = oneLink.getSortedLinks()
            if config.plugins.IPTVPlayerMario.tvpVodPreferedformat.value == 'mpd':
                ...
                if '' != mpdLink:
                    ...
                    if config.plugins.IPTVPlayerMario.tvpVodUseDF.value:
                        ...
                        videoTab = oneLink.getSortedLinks()
            if config.plugins.IPTVPlayerMario.tvpVodPreferedformat.value == 'mp4':
                    ...
                    for item in data['formats']:
                        if item['mimeType'] in FORMATS.keys():
                            ...
                            if 'm3u8' == formatType:
                                ...
                                if config.plugins.IPTVPlayerMario.tvpVodProxyEnable.value:
                                    meta['http_proxy'] = config.plugins.IPTVPlayerMario.proxyurl.value
...
#            sts, data = self.cm.getPage( 'https://apivod.tvp.pl/tv/video/%s/default/default?device=android' % asset_id, params)
        ...
        if config.plugins.IPTVPlayerMario.tvpVodUseDF.value and len(videoTab):
            videoTab = [videoTab[0]]
    ...
    def getLinksForFavourite(self, fav_data):
        if None == self.loggedIn:
            premium = config.plugins.IPTVPlayerMario.tvpvod_premium.value
            if premium:
