...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, GetCookieDir, MergeDicts, ReadTextFile, WriteTextFile, GetTmpDir, rm
...
config.plugins.IPTVPlayerMario.serienstreamto_langpreference = ConfigSelection(default="de,de_sub,en", choices=[("de,de_sub,en", "de,sub,en"),
                                                                                                               ("de,en,de_sub", "de,en,sub"),
                                                                                                               ("de_sub,de,en", "sub,de,en"),
                                                                                                               ("de_sub,en,de", "sub,en,de"),
                                                                                                               ("en,de_sub,de", "en,sub,de"),
                                                                                                               ("en,de,de_sub", "en,de,sub")])
config.plugins.IPTVPlayerMario.serienstreamto_login = ConfigText(default="", fixed_size=False)
config.plugins.IPTVPlayerMario.serienstreamto_password = ConfigText(default="", fixed_size=False)
config.plugins.IPTVPlayerMario.serienstreamto_host = ConfigText(default=DEFAULTHOST, fixed_size=False)


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_("Your language preference:"), config.plugins.IPTVPlayerMario.serienstreamto_langpreference))

    optionList.append(getConfigListEntry(_("e-mail") + ":", config.plugins.IPTVPlayerMario.serienstreamto_login))
    optionList.append(getConfigListEntry(_("password") + ":", config.plugins.IPTVPlayerMario.serienstreamto_password))
    optionList.append(getConfigListEntry(_("host") + ":", config.plugins.IPTVPlayerMario.serienstreamto_host))
    return optionList
###################################################


def gettytul():
    return config.plugins.IPTVPlayerMario.serienstreamto_host.value
...
class SerienStreamTo(CBaseHostClass, CaptchaHelper):
    ...
    def getLinksForVideo(self, cItem):
        ...
        # 1 - de, 2 - en, 3 - de_sub
        langPreference = config.plugins.IPTVPlayerMario.serienstreamto_langpreference.value.replace('de_sub', '3').replace('de', '1').replace('en', '2').split(',')
    ...
    def tryTologin(self):
        ...
        if self.login == config.plugins.IPTVPlayerMario.serienstreamto_login.value and \
           self.password == config.plugins.IPTVPlayerMario.serienstreamto_password.value:
           return

        self.cm.clearCookie(self.COOKIE_FILE, ['__cfduid', 'cf_clearance'])
        self.login = config.plugins.IPTVPlayerMario.serienstreamto_login.value
        self.password = config.plugins.IPTVPlayerMario.serienstreamto_password.value
    ...
    def tryTologin(self):
        ...
        if None == self.loggedIn or self.login != config.plugins.IPTVPlayerMario.serienstreamto_login.value or\
            self.password != config.plugins.IPTVPlayerMario.serienstreamto_password.value:
