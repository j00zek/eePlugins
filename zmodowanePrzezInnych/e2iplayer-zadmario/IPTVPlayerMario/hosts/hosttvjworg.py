...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, GetDefaultLang, CSelOneLink, GetLogoDir, byteify
...
###################################################
config.plugins.IPTVPlayerMario.tvjworg_language = ConfigSelection(default="default", choices=[("default", _("Default")), ("P", _("Polish")), ("E", _("English"))])
config.plugins.IPTVPlayerMario.tvjworg_icontype = ConfigSelection(default="vertical", choices=[("vertical", _('vertical')), ("horizontal", _('horizontal'))])
config.plugins.IPTVPlayerMario.tvjworg_default_format = ConfigSelection(default="720", choices=[("0", _("the worst")),
                                                                                               ("240", "240p"),
                                                                                               ("360", "360p"),
                                                                                               ("480", "480p"),
                                                                                               ("720", "720p"),
                                                                                               ("99999999", "the best")])
config.plugins.IPTVPlayerMario.tvjworg_use_df = ConfigYesNo(default=True)


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_("Language"), config.plugins.IPTVPlayerMario.tvjworg_language))
    optionList.append(getConfigListEntry(_("Default video quality"), config.plugins.IPTVPlayerMario.tvjworg_default_format))
    optionList.append(getConfigListEntry(_("Use default video quality"), config.plugins.IPTVPlayerMario.tvjworg_use_df))
    optionList.append(getConfigListEntry(_("Icon type"), config.plugins.IPTVPlayerMario.tvjworg_icontype))
    return optionList
###################################################
...
class TVJWORG(CBaseHostClass):
    ...
    def _getIcon(self, iconItem):
        icon = ''
        try:
            imageTypes = self.ICONS_TYPES.get(config.plugins.IPTVPlayerMario.tvjworg_icontype.value, [])
    ...
    def _getLangCode(self):
        langCode = 'E'
        if config.plugins.IPTVPlayerMario.tvjworg_language.value == 'default':
            ...
            langCode = config.plugins.IPTVPlayerMario.tvjworg_language.value
        return langCode
    ...
    def getLinksForVideo(self, cItem):
        ...
        try:
            ...
            if 1 < len(urlTab):
                error = False
                max_bitrate = int(config.plugins.IPTVPlayerMario.tvjworg_default_format.value)
                ...
                if not error and config.plugins.IPTVPlayerMario.tvjworg_use_df.value:
