...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, byteify, rm, GetPluginDir
...
###################################################
config.plugins.IPTVPlayerMario.putlockertv_proxy = ConfigSelection(default="None", choices=[("None", _("None")),
                                                                                           ("proxy_1", _("Alternative proxy server (1)")),
                                                                                           ("proxy_2", _("Alternative proxy server (2)"))])
config.plugins.IPTVPlayerMario.putlockertv_alt_domain = ConfigText(default="", fixed_size=False)


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_("Use proxy server:"), config.plugins.IPTVPlayerMario.putlockertv_proxy))
    if config.plugins.IPTVPlayerMario.putlockertv_proxy.value == 'None':
        optionList.append(getConfigListEntry(_("Alternative domain:"), config.plugins.IPTVPlayerMario.putlockertv_alt_domain))
    return optionList
###################################################
...
class PutlockerTvTo(CBaseHostClass):
    ...
    def getPage(self, url, addParams={}, post_data=None):
        ...
        proxy = config.plugins.IPTVPlayerMario.putlockertv_proxy.value
        if proxy != 'None':
            if proxy == 'proxy_1':
    ...
    def selectDomain(self):
        ...
        domain = config.plugins.IPTVPlayerMario.putlockertv_alt_domain.value.strip()
