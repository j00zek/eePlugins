...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, byteify
...
###################################################
config.plugins.IPTVPlayerMario.ardmediathek_iconquality = ConfigSelection(default="medium", choices=[("large", _("high")), ("medium", _("medium")), ("small", _("low"))])
config.plugins.IPTVPlayerMario.ardmediathek_prefformat = ConfigSelection(default="mp4,m3u8", choices=[
("mp4,m3u8", "mp4,m3u8"), ("m3u8,mp4", "m3u8,mp4")])
config.plugins.IPTVPlayerMario.ardmediathek_prefquality = ConfigSelection(default="4", choices=[("0", _("low")), ("1", _("medium")), ("2", _("high")), ("3", _("very high")), ("4", _("hd"))])
config.plugins.IPTVPlayerMario.ardmediathek_prefmoreimportant = ConfigSelection(default="quality", choices=[("quality", _("quality")), ("format", _("format"))])
config.plugins.IPTVPlayerMario.ardmediathek_onelinkmode = ConfigYesNo(default=True)


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_("Icons quality"), config.plugins.IPTVPlayerMario.ardmediathek_iconquality))
    optionList.append(getConfigListEntry(_("Prefered format"), config.plugins.IPTVPlayerMario.ardmediathek_prefformat))
    optionList.append(getConfigListEntry(_("Prefered quality"), config.plugins.IPTVPlayerMario.ardmediathek_prefquality))
    optionList.append(getConfigListEntry(_("More important"), config.plugins.IPTVPlayerMario.ardmediathek_prefmoreimportant))
    optionList.append(getConfigListEntry(_("One link mode"), config.plugins.IPTVPlayerMario.ardmediathek_onelinkmode))
    return optionList
###################################################
...
class ARDmediathek(CBaseHostClass):
    ...
    def getIconUrl(self, url):
        marker = '##width##'
        if marker in url:
            iconQuality = config.plugins.IPTVPlayerMario.ardmediathek_iconquality.value
    ...
    def getLinksForVideo(self, cItem):
        ...
        preferedQuality = int(config.plugins.IPTVPlayerMario.ardmediathek_prefquality.value)
        preferedFormat = config.plugins.IPTVPlayerMario.ardmediathek_prefformat.value
        ...
        try:
            ...
            def _cmpLinks(it1, it2):
                prefmoreimportantly = config.plugins.IPTVPlayerMario.ardmediathek_prefmoreimportant.value
            ...
            onelinkmode = config.plugins.IPTVPlayerMario.ardmediathek_onelinkmode.value
