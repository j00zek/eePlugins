...
# LOCAL import
###################################################
from Plugins.Extensions.IPTVPlayerMario.components.iptvplayerinit import TranslateTXT as _, SetIPTVPlayerLastHostError
from Plugins.Extensions.IPTVPlayerMario.components.ihost import CHostBase, CBaseHostClass
from Plugins.Extensions.IPTVPlayerMario.components.captcha_helper import CaptchaHelper
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, GetDefaultLang, byteify, rm, GetCacheSubDir, ReadTextFile, WriteTextFile
from Plugins.Extensions.IPTVPlayerMario.tools.e2ijs import js_execute
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtypes import strwithmeta
###################################################
from Plugins.Extensions.IPTVPlayerMario.p2p3.UrlLib import urllib_urlencode, urllib_quote_plus
...
from Plugins.Extensions.IPTVPlayerMario.libs.recaptcha_v2 import UnCaptchaReCaptcha as UnCaptchaReCaptcha_fallback
###################################################
...
###################################################
config.plugins.IPTVPlayerMario.mrpiracy_login = ConfigText(default="", fixed_size=False)
config.plugins.IPTVPlayerMario.mrpiracy_password = ConfigText(default="", fixed_size=False)
...
config.plugins.IPTVPlayerMario.mrpiracy_linkcache = ConfigYesNo(default=True)
config.plugins.IPTVPlayerMario.mrpiracy_bypassrecaptcha = ConfigSelection(default="None", choices=[("None", _("None")),
                                                                                                 ("9kw.eu", "https://9kw.eu/"),
...
def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_("e-mail"), config.plugins.IPTVPlayerMario.mrpiracy_login))
    optionList.append(getConfigListEntry(_("Password"), config.plugins.IPTVPlayerMario.mrpiracy_password))

    optionList.append(getConfigListEntry(_("Use links cache"), config.plugins.IPTVPlayerMario.mrpiracy_linkcache))
    optionList.append(getConfigListEntry(_("Captcha solving service"), config.plugins.IPTVPlayerMario.mrpiracy_bypassrecaptcha))
    if config.plugins.IPTVPlayerMario.mrpiracy_bypassrecaptcha.value == '9kw.eu':
        optionList.append(getConfigListEntry(_("%s API KEY") % '    ', config.plugins.IPTVPlayerMario.api_key_9kweu))
    elif config.plugins.IPTVPlayerMario.mrpiracy_bypassrecaptcha.value == '2captcha.com':
        optionList.append(getConfigListEntry(_("%s API KEY") % '    ', config.plugins.IPTVPlayerMario.api_key_2captcha))
    return optionList
###################################################
...
class MRPiracyGQ(CBaseHostClass, CaptchaHelper):
    ...
    def getVideoLinks(self, videoUrl):
        ...
        if hostUrl == '' and config.plugins.IPTVPlayerMario.mrpiracy_linkcache.value:
        ...
        if hostUrl == '':
            try:
                ...
                if 'sitekey' in playerData:
                    token, errorMsgTab = self.processCaptcha(playerData['sitekey'], playerData['ref_url'], config.plugins.IPTVPlayerMario.mrpiracy_bypassrecaptcha.value)
        ...
        if self.cm.isValidUrl(hostUrl):
            if linkId != '':
                if config.plugins.IPTVPlayerMario.mrpiracy_linkcache.value and linkId not in MRPiracyGQ.LINKS_CACHE:
    ...
    def tryTologin(self):
        ...
        login = config.plugins.IPTVPlayerMario.mrpiracy_login.value
        passwd = config.plugins.IPTVPlayerMario.mrpiracy_password.value
    ...
    def handleService(self, index, refresh=0, searchPattern='', searchType=''):
        ...
        if (self.login != config.plugins.IPTVPlayerMario.mrpiracy_login.value or
             self.password != config.plugins.IPTVPlayerMario.mrpiracy_password.value) and \
            '' != config.plugins.IPTVPlayerMario.mrpiracy_login.value.strip() and \
            '' != config.plugins.IPTVPlayerMario.mrpiracy_password.value.strip():

            self.loggedIn, msg = self.tryTologin()
            if not self.loggedIn:
                userName = config.plugins.IPTVPlayerMario.mrpiracy_login.value
                self.sessionEx.open(MessageBox, 'Login failed for user "%s".' % userName, type=MessageBox.TYPE_INFO, timeout=10)
            else:
                self.loogin = config.plugins.IPTVPlayerMario.mrpiracy_login.value
                self.password = config.plugins.IPTVPlayerMario.mrpiracy_password.value
        elif ('' == config.plugins.IPTVPlayerMario.mrpiracy_login.value.strip() or
              '' == config.plugins.IPTVPlayerMario.mrpiracy_password.value.strip()):
    ...
    #MAIN MENU
        if name == None:
            if (config.plugins.IPTVPlayerMario.mrpiracy_login.value == '' or config.plugins.IPTVPlayerMario.mrpiracy_password.value == ''):
                rm(self.COOKIE_FILE)
            self.cacheLinks = {}
