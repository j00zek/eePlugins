...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, rm, GetTmpDir
...
###################################################
config.plugins.IPTVPlayerMario.freediscpl_login = ConfigText(default="", fixed_size=False)
config.plugins.IPTVPlayerMario.freediscpl_password = ConfigText(default="", fixed_size=False)


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_("e-mail") + ":", config.plugins.IPTVPlayerMario.freediscpl_login))
    optionList.append(getConfigListEntry(_("password") + ":", config.plugins.IPTVPlayerMario.freediscpl_password))
    return optionList
###################################################
...
class FreeDiscPL(CBaseHostClass):
    ...
    def tryTologin(self):
        ...
        errMsg = []
        if None == self.loggedIn or self.login != config.plugins.IPTVPlayerMario.freediscpl_login.value or\
            self.password != config.plugins.IPTVPlayerMario.freediscpl_password.value:

            self.login = config.plugins.IPTVPlayerMario.freediscpl_login.value
            self.password = config.plugins.IPTVPlayerMario.freediscpl_password.value
