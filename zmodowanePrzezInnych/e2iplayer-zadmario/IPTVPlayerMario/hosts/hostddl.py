...
# LOCAL import
###################################################
from Plugins.Extensions.IPTVPlayerMario.components.iptvplayerinit import TranslateTXT as _, GetIPTVSleep
...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, byteify, MergeDicts, GetDefaultLang
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtypes import strwithmeta
###################################################
from Plugins.Extensions.IPTVPlayerMario.p2p3.UrlLib import urllib_unquote, urllib_quote, urllib_quote_plus
...
###################################################
config.plugins.IPTVPlayerMario.ddlme_proxy = ConfigSelection(default="None", choices=[("None", _("None")),
                                                                                     ("webproxy", _("Web proxy")),
                                                                                     ("proxy_1", _("Alternative proxy server (1)")),
                                                                                     ("proxy_2", _("Alternative proxy server (2)"))])
config.plugins.IPTVPlayerMario.ddlme_lang = ConfigSelection(default="", choices=[("", _("default")),
                                                                                ("de", "de"),
                                                                                ("en", "en")])
...
def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_("Language"), config.plugins.IPTVPlayerMario.ddlme_lang))
    optionList.append(getConfigListEntry(_("Use proxy"), config.plugins.IPTVPlayerMario.ddlme_proxy))
    return optionList
###################################################
...
class DDLMe(CBaseHostClass):
    ...
    def getRealUrl(self, url):
        if config.plugins.IPTVPlayerMario.ddlme_proxy.value == 'webproxy' and url != None and 'browse.php?u=' in url:
    ...
    def getMainUrl(self):
        lang = config.plugins.IPTVPlayerMario.ddlme_lang.value
        if lang == '':
            lang = GetDefaultLang()
    ...
    def getPage(self, baseUrl, addParams={}, post_data=None):
        ...
        proxy = config.plugins.IPTVPlayerMario.ddlme_proxy.value
        if proxy == 'webproxy':
            ...
            if proxy == 'proxy_1':
                proxy = config.plugins.IPTVPlayerMario.alternative_proxy1.value
            else:
                proxy = config.plugins.IPTVPlayerMario.alternative_proxy2.value
            addParams = dict(addParams)
        ...
        while tries < 3:
            ...
            if sts:
                if config.plugins.IPTVPlayerMario.ddlme_proxy.value == 'webproxy' and 'sslagree' in data:
    ...
    def listItems(self, cItem, nextCategory):
        ...
        if config.plugins.IPTVPlayerMario.ddlme_proxy.value == 'webproxy':
            nextPage = '_{0}%2'
        else:
            nextPage = '_{0}/'
