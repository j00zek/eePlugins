...
###################################################
config.plugins.IPTVPlayerMario.akoam_proxy = ConfigSelection(default="None", choices=[("None", _("None")),
...
def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_("Use proxy server:"), config.plugins.IPTVPlayerMario.akoam_proxy))
    return optionList
###################################################
...
class AkoAm(CBaseHostClass):
    ...
    def getProxy(self):
        proxy = config.plugins.IPTVPlayerMario.akoam_proxy.value
        if proxy != 'None':
            if proxy == 'proxy_1':
