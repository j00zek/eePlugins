...
###################################################
config.plugins.IPTVPlayerMario.cda_searchsort = ConfigSelection(default="best", choices=[("best", "Najtrafniejsze"), ("date", "Najnowsze"), ("rate", "Najlepiej oceniane"), ("alf", "Alfabetycznie")])
config.plugins.IPTVPlayerMario.cda_login = ConfigText(default=readCFG('cda_login'), fixed_size=False)
config.plugins.IPTVPlayerMario.cda_password = ConfigText('', fixed_size=False)
config.plugins.IPTVPlayerMario.cda_acc_token = ConfigText('', fixed_size=False)
config.plugins.IPTVPlayerMario.cda_refr_token = ConfigText('', fixed_size=False)
config.plugins.IPTVPlayerMario.cda_login_id = ConfigText('', fixed_size=False)


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry("Login:", config.plugins.IPTVPlayerMario.cda_login))
    optionList.append(getConfigListEntry("Hasło:", config.plugins.IPTVPlayerMario.cda_password))
    optionList.append(getConfigListEntry("Sortuj wyniki wyszukiwania po:", config.plugins.IPTVPlayerMario.cda_searchsort))
    return optionList
###################################################
...
class cda(CBaseHostClass, CaptchaHelper):
    ...
    def listSearchResult(self, cItem, searchPattern, searchType):
        ...
        searchsort = config.plugins.IPTVPlayerMario.cda_searchsort.value
    ...
    def tryTologin(self):
        ...
        if None == self.loggedIn or self.login != config.plugins.IPTVPlayerMario.cda_login.value or\
            self.password != config.plugins.IPTVPlayerMario.cda_password.value:

            loginCookie = GetCookieDir('cda.pl.login')
            self.login = ensure_str(config.plugins.IPTVPlayerMario.cda_login.value)
            self.password = ensure_str(config.plugins.IPTVPlayerMario.cda_password.value)
