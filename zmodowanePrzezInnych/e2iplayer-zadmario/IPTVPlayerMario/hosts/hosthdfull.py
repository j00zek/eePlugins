...
from Plugins.Extensions.IPTVPlayerMario.tools.e2ijs import js_execute_ext, is_js_cached
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, MergeDicts, rm, GetCookieDir, ReadTextFile, WriteTextFile
...
###################################################
config.plugins.IPTVPlayerMario.hdfull_language = ConfigSelection(default="es", choices=[("es", _("Spanish")), ("en", _("English"))])
config.plugins.IPTVPlayerMario.hdfull_login = ConfigText(default="", fixed_size=False)
config.plugins.IPTVPlayerMario.hdfull_password = ConfigText(default="", fixed_size=False)


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_("Language"), config.plugins.IPTVPlayerMario.hdfull_language))
    optionList.append(getConfigListEntry(_("login"), config.plugins.IPTVPlayerMario.hdfull_login))
    optionList.append(getConfigListEntry(_("password"), config.plugins.IPTVPlayerMario.hdfull_password))
    return optionList
###################################################
...
class SuggestionsProvider:
    ...
    def __init__(self):
        self.cm = common()
        self.lang = config.plugins.IPTVPlayerMario.hdfull_language.value
...
class HDFull(CBaseHostClass, CaptchaHelper):
    ...
    def __init__(self):
        ...
        language = config.plugins.IPTVPlayerMario.hdfull_language.value
    ...
    def listEpisodes2(self, cItem):
        ...
        lang = config.plugins.IPTVPlayerMario.hdfull_language.value
    ...
    def tryTologin(self):
        ...
        if None == self.loggedIn or self.login != config.plugins.IPTVPlayerMario.hdfull_login.value or \
            self.password != config.plugins.IPTVPlayerMario.hdfull_password.value:

            self.cm.clearCookie(self.COOKIE_FILE, removeNames=['language'])

            loginCookie = GetCookieDir('hdfull.me.login')
            self.login = config.plugins.IPTVPlayerMario.hdfull_login.value
            self.password = config.plugins.IPTVPlayerMario.hdfull_password.value
