...
###################################################
config.plugins.IPTVPlayerMario.filman_login = ConfigText(default="", fixed_size=False)
config.plugins.IPTVPlayerMario.filman_password = ConfigText(default="", fixed_size=False)


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry("Filman login:", config.plugins.IPTVPlayerMario.filman_login))
    optionList.append(getConfigListEntry("Filman hasło:", config.plugins.IPTVPlayerMario.filman_password))
    return optionList
###################################################
...
#        itemsList.append((_('Genres'), self.cleanHtmlStr(self.cm.ph.getDataBeetwenMarkers(data, '<ul class="genres">', '</ul>', True)[1])))
    ...
    def tryTologin(self):
        ...
        if None == self.loggedIn or self.login != config.plugins.IPTVPlayerMario.filman_login.value or\
            self.password != config.plugins.IPTVPlayerMario.filman_password.value:
            ...
            if sts and '/wylogowanie' not in data:
                self.login = config.plugins.IPTVPlayerMario.filman_login.value
                self.password = config.plugins.IPTVPlayerMario.filman_password.value
