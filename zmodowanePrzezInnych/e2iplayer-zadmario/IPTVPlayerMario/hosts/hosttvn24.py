...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, GetLogoDir
...
###################################################
config.plugins.IPTVPlayerMario.TVN24httpType = ConfigSelection(default="http://", choices=[("http://", "http://"), ("https://", "https://")])


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry("Typ połączenia:", config.plugins.IPTVPlayerMario.TVN24httpType))
    return optionList
###################################################
...
class Tvn24(CBaseHostClass):
    ...
    def converUrl(self, url):
        if "http://" == config.plugins.IPTVPlayerMario.TVN24httpType.value and url.startswith("https://"):
            return "http" + url[5:]
        return url
