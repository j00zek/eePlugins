...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, GetDefaultLang, rm, byteify
...
###################################################
from Plugins.Extensions.IPTVPlayerMario.p2p3.UrlParse import urljoin
...
###################################################
config.plugins.IPTVPlayerMario.ekinotv_login = ConfigText(default="", fixed_size=False)
config.plugins.IPTVPlayerMario.ekinotv_password = ConfigText(default="", fixed_size=False)

config.plugins.IPTVPlayerMario.ekinotv_sortby = ConfigSelection(default="data-dodania", choices=[("alfabetycznie", "nazwy"), ("ocena", "oceny"), ("odslony", "ilości odsłon"), ("data-dodania", "daty dodania"), ("data-premiery", "daty premiery"), ('data-aktualizacji', 'daty aktualizacji')])
config.plugins.IPTVPlayerMario.ekinotv_sortorder = ConfigSelection(default="desc", choices=[("desc", "malejąco"), ("asc", "rosnąco")])


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry("Ekino TV login:", config.plugins.IPTVPlayerMario.ekinotv_login))
    optionList.append(getConfigListEntry("Ekino TV hasło:", config.plugins.IPTVPlayerMario.ekinotv_password))
    optionList.append(getConfigListEntry("Sortuj według:", config.plugins.IPTVPlayerMario.ekinotv_sortby))
    optionList.append(getConfigListEntry("Kolejność wyświetlania:", config.plugins.IPTVPlayerMario.ekinotv_sortorder))
    return optionList
###################################################
...
class EkinoTv(CBaseHostClass, CaptchaHelper):
    ...
    def listMovies(self, cItem):
        ...
        # prepare url
        sortby = config.plugins.IPTVPlayerMario.ekinotv_sortby.value
        sortorder = config.plugins.IPTVPlayerMario.ekinotv_sortorder.value
...
#                name = self.cm.ph.getSearchGroups(data, '''\stype=['"]([^'^"]+?)['"]''')[0]
    ...
    def tryTologin(self):
        ...
        if None == self.loggedIn or self.login != config.plugins.IPTVPlayerMario.ekinotv_login.value or\
            self.password != config.plugins.IPTVPlayerMario.ekinotv_password.value:
            ...
            if sts and 'user/logout' not in data:
                self.login = config.plugins.IPTVPlayerMario.ekinotv_login.value
                self.password = config.plugins.IPTVPlayerMario.ekinotv_password.value
