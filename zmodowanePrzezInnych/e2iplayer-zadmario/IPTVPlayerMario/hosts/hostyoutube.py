...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, IsExecutable, printExc, byteify
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtypes import strwithmeta
from Plugins.Extensions.IPTVPlayerMario.tools.iptvfilehost import IPTVFileHost
...
###################################################
config.plugins.IPTVPlayerMario.Sciezkaurllist = ConfigDirectory(default="/hdd/")


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_("Sort by:"), config.plugins.IPTVPlayerMario.ytSortBy))
    optionList.append(getConfigListEntry(_("Path to ytlist.txt, urllist.txt"), config.plugins.IPTVPlayerMario.Sciezkaurllist))
    optionList.append(getConfigListEntry(_("Video format:"), config.plugins.IPTVPlayerMario.ytformat))
    optionList.append(getConfigListEntry(_("Default video quality:"), config.plugins.IPTVPlayerMario.ytDefaultformat))
    optionList.append(getConfigListEntry(_("Use default video quality:"), config.plugins.IPTVPlayerMario.ytUseDF))
    optionList.append(getConfigListEntry(_("Age-gate bypass:"), config.plugins.IPTVPlayerMario.ytAgeGate))
    ...
    if IsExecutable('ffmpeg'):
        optionList.append(getConfigListEntry(_("Allow dash format:"), config.plugins.IPTVPlayerMario.ytShowDash))
        if config.plugins.IPTVPlayerMario.ytShowDash.value != 'false':
            optionList.append(getConfigListEntry(_("Allow VP9 codec:"), config.plugins.IPTVPlayerMario.ytVP9))
    return optionList
###################################################
###################################################
...
class Youtube(CBaseHostClass):
    ...
    def listCategory(self, cItem, searchMode=False):
        ...
        sortList = True
        filespath = config.plugins.IPTVPlayerMario.Sciezkaurllist.value
        groupList = True
    ...
    def listSearchResult(self, cItem, pattern, searchType):
        ...
        if url:
            ...
            tmpList = self.ytp.getSearchResult(urllib_quote_plus(pattern), searchType, page, 'search', config.plugins.IPTVPlayerMario.ytSortBy.value, url)
        else:
            tmpList = self.ytp.getSearchResult(urllib_quote_plus(pattern), searchType, page, 'search', config.plugins.IPTVPlayerMario.ytSortBy.value)
    ...
    def getLinksForVideo(self, cItem):
        ...
        if config.plugins.IPTVPlayerMario.ytUseDF.value and 0 < len(urlTab):
            return [urlTab[0]]
        return urlTab
