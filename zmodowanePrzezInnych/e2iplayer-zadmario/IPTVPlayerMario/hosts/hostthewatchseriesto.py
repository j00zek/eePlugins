...
###################################################
config.plugins.IPTVPlayerMario.swatchseries_web_proxy_gateway = ConfigSelection(default="auto", choices=[("auto", _("Auto")), ("always", _("Always")), ("never", _("Never"))])


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_("Use web proxy gateway"), config.plugins.IPTVPlayerMario.swatchseries_web_proxy_gateway))
    return optionList
###################################################
...
class TheWatchseriesTo(CBaseHostClass):
    ...
    def isNeedProxy(self):
        val = config.plugins.IPTVPlayerMario.swatchseries_web_proxy_gateway.value
        if val == 'always':
            return True
        elif val == 'never':
            return False
