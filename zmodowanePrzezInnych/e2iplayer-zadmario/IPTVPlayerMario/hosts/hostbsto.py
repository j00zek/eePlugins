...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, byteify, rm, GetPluginDir, GetCacheSubDir, ReadTextFile, WriteTextFile
...
###################################################
from Plugins.Extensions.IPTVPlayerMario.p2p3.manipulateStrings import ensure_binary
###################################################
# FOREIGN import
###################################################
import base64
...
config.plugins.IPTVPlayerMario.bsto_linkcache = ConfigYesNo(default=True)
config.plugins.IPTVPlayerMario.bsto_bypassrecaptcha = ConfigSelection(default="None", choices=[("None", _("None")),
                                                                                              ("9kw.eu", "https://9kw.eu/"),
...
def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_("Use links cache"), config.plugins.IPTVPlayerMario.bsto_linkcache))
    optionList.append(getConfigListEntry(_("Captcha solving service"), config.plugins.IPTVPlayerMario.bsto_bypassrecaptcha))
    if config.plugins.IPTVPlayerMario.bsto_bypassrecaptcha.value == '9kw.eu':
        optionList.append(getConfigListEntry(_("%s API KEY") % '    ', config.plugins.IPTVPlayerMario.api_key_9kweu))
    elif config.plugins.IPTVPlayerMario.bsto_bypassrecaptcha.value == '2captcha.com':
...
class BSTO(CBaseHostClass, CaptchaHelper):
    ...
    def getVideoLinks(self, videoUrl):
        ...
        if hostUrl == '' and config.plugins.IPTVPlayerMario.bsto_linkcache.value:
        ...
        if hostUrl == '':
            ...
            if url == prevUrl:
                ...
                if sitekey != '' and 'bitte das Captcha' in data:
                    token, errorMsgTab = self.processCaptcha(sitekey, self.cm.meta['url'], config.plugins.IPTVPlayerMario.bsto_bypassrecaptcha.value)
                    if token != '':
        ...
        if 1 != self.up.checkHostSupport(hostUrl):
            ...
            if config.plugins.IPTVPlayerMario.bsto_linkcache.value:
