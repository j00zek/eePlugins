...
from Plugins.Extensions.IPTVPlayerMario.components.ihost import CHostBase, CBaseHostClass, RetHost, CUrlItem
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, GetLogoDir, GetCookieDir, GetHostsOrderList
...
from Plugins.Extensions.IPTVPlayerMario.libs.urlparserhelper import getDirectM3U8Playlist, getF4MLinksWithMeta, getMPDLinksWithMeta
from Plugins.Extensions.IPTVPlayerMario.libs.youtube_dl.utils import clean_html
from Plugins.Extensions.IPTVPlayerMario.libs.teledunet import TeledunetParser
from Plugins.Extensions.IPTVPlayerMario.libs.urlparser import urlparser
from Plugins.Extensions.IPTVPlayerMario.libs.filmonapi import FilmOnComApi, GetConfigList as FilmOn_GetConfigList
from Plugins.Extensions.IPTVPlayerMario.libs.videostar import VideoStarApi, GetConfigList as VideoStar_GetConfigList
from Plugins.Extensions.IPTVPlayerMario.libs.webcamera import WebCameraApi
from Plugins.Extensions.IPTVPlayerMario.libs.bilasportpw import BilaSportPwApi, GetConfigList as BilaSportPw_GetConfigList
from Plugins.Extensions.IPTVPlayerMario.libs.canlitvliveio import CanlitvliveIoApi
from Plugins.Extensions.IPTVPlayerMario.libs.weebtv import WeebTvApi, GetConfigList as WeebTv_GetConfigList
from Plugins.Extensions.IPTVPlayerMario.libs.ustvnow import UstvnowApi, GetConfigList as Ustvnow_GetConfigList
from Plugins.Extensions.IPTVPlayerMario.libs.meteopl import MeteoPLApi, GetConfigList as MeteoPL_GetConfigList
from Plugins.Extensions.IPTVPlayerMario.libs.skylinewebcamscom import WkylinewebcamsComApi, GetConfigList as WkylinewebcamsCom_GetConfigList
from Plugins.Extensions.IPTVPlayerMario.libs.livespottingtv import LivespottingTvApi
from Plugins.Extensions.IPTVPlayerMario.libs.sport365live import Sport365LiveApi
from Plugins.Extensions.IPTVPlayerMario.libs.karwantv import KarwanTvApi
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtypes import strwithmeta
from Plugins.Extensions.IPTVPlayerMario.libs.djingcom import DjingComApi
from Plugins.Extensions.IPTVPlayerMario.libs.mlbstreamtv import MLBStreamTVApi, GetConfigList as MLBStreamTV_GetConfigList
from Plugins.Extensions.IPTVPlayerMario.libs.wiziwig1 import Wiziwig1Api
###################################################
...
from Plugins.Extensions.IPTVPlayerMario.p2p3.UrlParse import urlsplit, urlunsplit
...
# E2 GUI COMMPONENTS
###################################################
from Plugins.Extensions.IPTVPlayerMario.components.asynccall import MainSessionWrapper
###################################################

###################################################
# Config options for HOST
###################################################
config.plugins.IPTVPlayerMario.SortowanieWebstream = ConfigYesNo(default=False)
config.plugins.IPTVPlayerMario.weatherbymatzgprohibitbuffering = ConfigYesNo(default=True)
config.plugins.IPTVPlayerMario.weather_useproxy = ConfigYesNo(default=False)
config.plugins.IPTVPlayerMario.fake_separator = ConfigSelection(default=" ", choices=[(" ", " ")])


def GetConfigList():
    optionList = []

    optionList.append(getConfigListEntry("----------------pilot.wp.pl-----------------", config.plugins.IPTVPlayerMario.fake_separator))
    try:
        optionList.extend(VideoStar_GetConfigList())
    except Exception:
        printExc()

    optionList.append(getConfigListEntry("------------------meteo.pl------------------", config.plugins.IPTVPlayerMario.fake_separator))
    try:
        optionList.extend(MeteoPL_GetConfigList())
    except Exception:
        printExc()

    optionList.append(getConfigListEntry("-------------------WeebTV-------------------", config.plugins.IPTVPlayerMario.fake_separator))
    try:
        optionList.extend(WeebTv_GetConfigList())
    except Exception:
        printExc()

    optionList.append(getConfigListEntry("-----------------FilmOn TV------------------", config.plugins.IPTVPlayerMario.fake_separator))
    try:
        optionList.extend(FilmOn_GetConfigList())
    except Exception:
        printExc()

    optionList.append(getConfigListEntry("----------------ustvnow.com-----------------", config.plugins.IPTVPlayerMario.fake_separator))
    try:
        optionList.extend(Ustvnow_GetConfigList())
    except Exception:
        printExc()

    optionList.append(getConfigListEntry("-------------SkyLineWebCams.com-------------", config.plugins.IPTVPlayerMario.fake_separator))
    try:
        optionList.extend(WkylinewebcamsCom_GetConfigList())
    except Exception:
        printExc()

    optionList.append(getConfigListEntry(_("----------Other----------"), config.plugins.IPTVPlayerMario.fake_separator))
    optionList.append(getConfigListEntry(_("Turn off buffering for http://prognoza.pogody.tv/"), config.plugins.IPTVPlayerMario.weatherbymatzgprohibitbuffering))
    optionList.append(getConfigListEntry(_("Use Polish proxy for http://prognoza.pogody.tv/"), config.plugins.IPTVPlayerMario.weather_useproxy))

    optionList.append(getConfigListEntry("----------------bilasport.pw-------------------", config.plugins.IPTVPlayerMario.fake_separator))
    try:
        optionList.extend(BilaSportPw_GetConfigList())
    except Exception:
        printExc()

    optionList.append(getConfigListEntry("-----------------mlbstream.tv------------------", config.plugins.IPTVPlayerMario.fake_separator))
    try:
...
class HasBahCa(CBaseHostClass):
    ...
    def __init__(self):
        ...
        #Login data
        self.sort = config.plugins.IPTVPlayerMario.SortowanieWebstream.value
    ...
    def prognozaPogodyList(self, url):
        ...
        if config.plugins.IPTVPlayerMario.weather_useproxy.value:
            params = {'http_proxy': config.plugins.IPTVPlayerMario.proxyurl.value}
        else:
            params = {}
    ...
    def prognozaPogodyLink(self, url):
        ...
        urlMeta = {}
        if config.plugins.IPTVPlayerMario.weatherbymatzgprohibitbuffering.value:
            urlMeta['iptv_buffering'] = 'forbidden'
        if config.plugins.IPTVPlayerMario.weather_useproxy.value:
            urlMeta['http_proxy'] = config.plugins.IPTVPlayerMario.proxyurl.value
