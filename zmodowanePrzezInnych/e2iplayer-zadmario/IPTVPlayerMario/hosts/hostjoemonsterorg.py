...
###################################################
from Plugins.Extensions.IPTVPlayerMario.p2p3.UrlLib import urllib_unquote
###################################################
# FOREIGN import
###################################################
import re
...
config.plugins.IPTVPlayerMario.joemonsterorg_login = ConfigText(default="", fixed_size=False)
config.plugins.IPTVPlayerMario.joemonsterorg_password = ConfigText(default="", fixed_size=False)


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry("Login:", config.plugins.IPTVPlayerMario.joemonsterorg_login))
    optionList.append(getConfigListEntry("Hasło:", config.plugins.IPTVPlayerMario.joemonsterorg_password))
    return optionList
###################################################
...
#                elif 'video/youtube' == type:
    ...
    def handleService(self, index, refresh=0, searchPattern='', searchType=''):
        ...
        if self.login != config.plugins.IPTVPlayerMario.joemonsterorg_login.value and \
           self.password != config.plugins.IPTVPlayerMario.joemonsterorg_password.value and \
           '' != config.plugins.IPTVPlayerMario.joemonsterorg_login.value.strip() and \
           '' != config.plugins.IPTVPlayerMario.joemonsterorg_password.value.strip():
            loggedIn, msg = self.tryTologin(config.plugins.IPTVPlayerMario.joemonsterorg_login.value, config.plugins.IPTVPlayerMario.joemonsterorg_password.value)
            if not loggedIn:
                self.sessionEx.open(MessageBox, 'Problem z zalogowaniem użytkownika "%s".' % config.plugins.IPTVPlayerMario.joemonsterorg_login.value, type=MessageBox.TYPE_INFO, timeout=10)
            else:
                self.login = config.plugins.IPTVPlayerMario.joemonsterorg_login.value
                self.password = config.plugins.IPTVPlayerMario.joemonsterorg_password.value
