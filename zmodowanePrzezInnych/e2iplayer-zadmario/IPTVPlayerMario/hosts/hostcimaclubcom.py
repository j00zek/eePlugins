...
###################################################
config.plugins.IPTVPlayerMario.cimaclub_proxy = ConfigSelection(default="None", choices=[("None", _("None")),
...
def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_("Use proxy server:"), config.plugins.IPTVPlayerMario.cimaclub_proxy))
    return optionList
###################################################
...
class CimaClubCom(CBaseHostClass):
    ...
    def getProxy(self):
        proxy = config.plugins.IPTVPlayerMario.cimaclub_proxy.value
        if proxy != 'None':
            if proxy == 'proxy_1':
