...
###################################################
config.plugins.IPTVPlayerMario.filmstreamvk_proxy = ConfigSelection(default="None", choices=[("None", _("None")),
                                                                                            ("proxy_1", _("Alternative proxy server (1)")),
                                                                                            ("proxy_2", _("Alternative proxy server (2)"))])
config.plugins.IPTVPlayerMario.filmstreamvk_alt_domain = ConfigText(default="", fixed_size=False)


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_("Use proxy server:"), config.plugins.IPTVPlayerMario.filmstreamvk_proxy))
    if config.plugins.IPTVPlayerMario.filmstreamvk_proxy.value == 'None':
        optionList.append(getConfigListEntry(_("Alternative domain:"), config.plugins.IPTVPlayerMario.filmstreamvk_alt_domain))
    return optionList
###################################################
...
class FilmstreamvkCom(CBaseHostClass):
    ...
    def getPage(self, baseUrl, addParams={}, post_data=None):
        ...
        proxy = config.plugins.IPTVPlayerMario.filmstreamvk_proxy.value
        if proxy != 'None':
            if proxy == 'proxy_1':
    ...
    def selectDomain(self):
        if self.MAIN_URL == None:
            ...
            domain = config.plugins.IPTVPlayerMario.filmstreamvk_alt_domain.value.strip()
    ...
    def listSearchResult(self, cItem, searchPattern, searchType):
        ...
        self.selectDomain()
        cItem = dict(cItem)
        cItem['url'] = self.getFullUrl('/?s=') + urllib_quote(searchPattern)
