...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, byteify, rm, CSelOneLink
...
###################################################
from Plugins.Extensions.IPTVPlayerMario.p2p3.UrlLib import urllib_quote_plus
###################################################
# FOREIGN import
###################################################
import re
...
###################################################
config.plugins.IPTVPlayerMario.tvplayercom_login = ConfigText(default="", fixed_size=False)
config.plugins.IPTVPlayerMario.tvplayercom_password = ConfigText(default="", fixed_size=False)
config.plugins.IPTVPlayerMario.tvplayercom_password = ConfigText(default="", fixed_size=False)
config.plugins.IPTVPlayerMario.tvplayercom_drmbypass = ConfigYesNo(default=False)
config.plugins.IPTVPlayerMario.tvplayercom_preferredbitrate = ConfigSelection(default="99999999", choices=[("99999999", _("highest")),
                                                                                               ("2564000", "2564k"),
                                                                                               ("1864000", "1864k"),
                                                                                               ("1064000", "1064k"),
                                                                                               ("564000", "564k"),
                                                                                               ("214000", "214k"),
                                                                                               ("0", _("lowest"))])
config.plugins.IPTVPlayerMario.tvplayercom_usepreferredbitrate = ConfigYesNo(default=False)


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_("Preferred bitrate") + ":", config.plugins.IPTVPlayerMario.tvplayercom_preferredbitrate))
    optionList.append(getConfigListEntry(_("Use preferred bitrate") + ":", config.plugins.IPTVPlayerMario.tvplayercom_usepreferredbitrate))
    optionList.append(getConfigListEntry(_("email") + ":", config.plugins.IPTVPlayerMario.tvplayercom_login))
    optionList.append(getConfigListEntry(_("password") + ":", config.plugins.IPTVPlayerMario.tvplayercom_password))
    optionList.append(getConfigListEntry(_("Try to bypass DRM (it may be illegal)") + ":", config.plugins.IPTVPlayerMario.tvplayercom_drmbypass))
    return optionList
###################################################
...
class TVPlayer(CBaseHostClass):
    ...
    def getLinksForVideo(self, cItem):
        ...
        try:
            ...
            if 'error' in data:
                ...
                if not config.plugins.IPTVPlayerMario.tvplayercom_drmbypass.value or cItem.get('next_try', False):
                    return []
            ...
            if len(retTab):
                ...
                retTab = CSelOneLink(retTab, __getLinkQuality, int(int(config.plugins.IPTVPlayerMario.tvplayercom_preferredbitrate.value) * 1.2)).getSortedLinks()
                if len(retTab) and config.plugins.IPTVPlayerMario.tvplayercom_usepreferredbitrate.value:
                    retTab = [retTab[0]]
                printDBG(retTab)
    ...
    def tryTologin(self):
        ...
        if None == self.loggedIn or self.login != config.plugins.IPTVPlayerMario.tvplayercom_login.value or\
            self.password != config.plugins.IPTVPlayerMario.tvplayercom_password.value:

            self.login = config.plugins.IPTVPlayerMario.tvplayercom_login.value
            self.password = config.plugins.IPTVPlayerMario.tvplayercom_password.value
