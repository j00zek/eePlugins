...
###################################################
config.plugins.IPTVPlayerMario.onetvodDefaultformat = ConfigSelection(default="9999", choices=[("0", "bitrate: najgorszy"), ("200", "bitrate: 200p"), ("450", "bitrate: 450p"), ("900", "bitrate: 900"), ("1800", "bitrate: 1800"), ("9999", "bitrate: najlepszy")])
config.plugins.IPTVPlayerMario.onetvodUseDF = ConfigYesNo(default=True)
config.plugins.IPTVPlayerMario.proxyOnet = ConfigYesNo(default=False)


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry("Domyślny format video:", config.plugins.IPTVPlayerMario.onetvodDefaultformat))
    optionList.append(getConfigListEntry("Używaj domyślnego format video:", config.plugins.IPTVPlayerMario.onetvodUseDF))
    optionList.append(getConfigListEntry("Korzystaj z proxy?", config.plugins.IPTVPlayerMario.proxyOnet))
    return optionList
###################################################
...
class VODPL(CBaseHostClass):
    ...
    def __init__(self):
        proxyUrl = config.plugins.IPTVPlayerMario.proxyurl.value
        useProxy = config.plugins.IPTVPlayerMario.proxyOnet.value
    ...
    def getLinksForVideo(self, cItem):
        ...
        maxRes = int(config.plugins.IPTVPlayerMario.onetvodDefaultformat.value) * 1.1
        tab = CSelOneLink(tab, __getLinkQuality, maxRes).getSortedLinks()
        if config.plugins.IPTVPlayerMario.onetvodUseDF.value and len(tab) > 0:
            tab = [tab[0]]
