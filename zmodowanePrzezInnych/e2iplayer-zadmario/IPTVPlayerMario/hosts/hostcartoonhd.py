...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, rm, MergeDicts, GetJSScriptFile
...
import time
import re
from Plugins.Extensions.IPTVPlayerMario.p2p3.UrlLib import urllib_unquote, urllib_quote
...
###################################################
config.plugins.IPTVPlayerMario.movieshdco_sortby = ConfigSelection(default="date", choices=[("date", _("Lastest")), ("views", _("Most viewed")), ("duree", _("Longest")), ("rate", _("Top rated")), ("random", _("Random"))])
config.plugins.IPTVPlayerMario.cartoonhd_login = ConfigText(default="", fixed_size=False)
config.plugins.IPTVPlayerMario.cartoonhd_password = ConfigText(default="", fixed_size=False)


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_("login") + ":", config.plugins.IPTVPlayerMario.cartoonhd_login))
    optionList.append(getConfigListEntry(_("password") + ":", config.plugins.IPTVPlayerMario.cartoonhd_password))
    return optionList
###################################################
...
class CartoonHD(CBaseHostClass):
    ...
    def tryTologin(self):
        ...
        login = config.plugins.IPTVPlayerMario.cartoonhd_login.value
        password = config.plugins.IPTVPlayerMario.cartoonhd_password.value
