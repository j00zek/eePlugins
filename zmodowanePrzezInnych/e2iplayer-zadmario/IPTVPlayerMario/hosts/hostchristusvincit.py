...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, MergeDicts, rm, formatBytes, CSelOneLink
...
config.plugins.IPTVPlayerMario.christusvincit_preferred_bitrate = ConfigSelection(default="99999999", choices=[("0", _("the lowest")),
                                                                                                    ("360000", "360000"),
                                                                                                    ("590000", "590000"),
                                                                                                    ("820000", "820000"),
                                                                                                    ("1250000", "1250000"),
...
config.plugins.IPTVPlayerMario.christusvincit_use_preferred_bitrate = ConfigYesNo(default=True)
...
def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry("Preferred video bitrate", config.plugins.IPTVPlayerMario.christusvincit_preferred_bitrate))
    optionList.append(getConfigListEntry("Use preferred video bitrate", config.plugins.IPTVPlayerMario.christusvincit_use_preferred_bitrate))
    return optionList
###################################################
    ...
    def getLinksForVideo(self, cItem):
        ...
        if 'url' in cItem and cItem['url'].startswith('rtmp://'):
            ...
            try:
                ...
                printExc()
        if len(urlsTab):
            max_bitrate = int(config.plugins.IPTVPlayerMario.christusvincit_preferred_bitrate.value)
            urlsTab = CSelOneLink(urlsTab, lambda x: int(x['bitrate']), max_bitrate).getSortedLinks()
            if config.plugins.IPTVPlayerMario.christusvincit_use_preferred_bitrate.value:
                urlsTab = [urlsTab[0]]
        return urlsTab
