...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, CSelOneLink, byteify
...
###################################################
config.plugins.IPTVPlayerMario.itv_use_x_forwarded_for = ConfigYesNo(default=False)


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_("Bypass geo-blocking for VODs (it may be illegal):"), config.plugins.IPTVPlayerMario.itv_use_x_forwarded_for))
    return optionList
###################################################
...
class ITV(CBaseHostClass):
    ...
    def getRandomGBIP(self):
        if not config.plugins.IPTVPlayerMario.itv_use_x_forwarded_for.value:
            return ''
        if self.forwardedIP == '':
