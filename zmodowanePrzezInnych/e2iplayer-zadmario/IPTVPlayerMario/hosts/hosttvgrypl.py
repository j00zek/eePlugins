...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import CSelOneLink, printDBG, printExc, byteify, rm
...
###################################################
#config.plugins.IPTVPlayerMario.tvgrypl_default_quality = ConfigSelection(default="SD", choices=[("MOB", "MOB: niska"), ("SD", "SD: standardowa"), ("HD", "HD: wysoka")]) #, ("FHD", "FHD: bardzo wysoka")
#config.plugins.IPTVPlayerMario.tvgrypl_use_dq = ConfigYesNo(default=True)
config.plugins.IPTVPlayerMario.tvgrypl_date_of_birth = ConfigText(default="2017-01-31", fixed_size=False)


def GetConfigList():
    optionList = []
#    optionList.append(getConfigListEntry("Domyślna jakość wideo:", config.plugins.IPTVPlayerMario.tvgrypl_default_quality))
#    optionList.append(getConfigListEntry("Używaj domyślnej jakości wideo:", config.plugins.IPTVPlayerMario.tvgrypl_use_dq))
    optionList.append(getConfigListEntry("Wprowadź datę urodzenia [RRRRR-MM-DD]:", config.plugins.IPTVPlayerMario.tvgrypl_date_of_birth))
    return optionList
###################################################
...
class TvGryPL(CBaseHostClass):
    ...
    def getLinksForVideo(self, cItem):
        ...
        if ageMarker in data:
            tmp = self.cm.ph.getSearchGroups(config.plugins.IPTVPlayerMario.tvgrypl_date_of_birth.value, '''([0-9]{4})[-]?([0-9][0-9]?)[-]?([0-9][0-9]?)''', 3)
