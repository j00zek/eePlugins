...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, byteify, rm, GetTmpDir
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtypes import strwithmeta
###################################################
from Plugins.Extensions.IPTVPlayerMario.p2p3.UrlParse import urljoin, urlparse
from Plugins.Extensions.IPTVPlayerMario.p2p3.UrlLib import urllib_quote, urllib_urlencode, urllib_unquote, urllib_quote_plus
###################################################
# FOREIGN import
###################################################
import re
...
###################################################
config.plugins.IPTVPlayerMario.filmezzeu_login = ConfigText(default="", fixed_size=False)
config.plugins.IPTVPlayerMario.filmezzeu_password = ConfigText(default="", fixed_size=False)


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry(_("login") + ":", config.plugins.IPTVPlayerMario.filmezzeu_login))
    optionList.append(getConfigListEntry(_("password") + ":", config.plugins.IPTVPlayerMario.filmezzeu_password))
    return optionList
###################################################
...
class FilmezzEU(CBaseHostClass):
            ...
            if self.up.getDomain(self.getMainUrl()) in videoUrl:
                ...
                if 'google.com/recaptcha/' in data and 'sitekey' in data:
                    ...
                    if self.cm.isValidUrl(imgUrl):
                        ...
                        params = dict(self.defaultParams)
                        params.update({'maintype': 'image', 'subtypes': ['jpeg', 'png'], 'check_first_bytes': [b'\xFF\xD8', b'\xFF\xD9', b'\x89\x50\x4E\x47'], 'header': params['header']})
    ...
    def tryTologin(self):
        ...
        self.login = config.plugins.IPTVPlayerMario.filmezzeu_login.value
        self.password = config.plugins.IPTVPlayerMario.filmezzeu_password.value
    ...
    def handleService(self, index, refresh=0, searchPattern='', searchType=''):
        ...
        if None == self.loggedIn or self.login != config.plugins.IPTVPlayerMario.filmezzeu_login.value or\
            self.password != config.plugins.IPTVPlayerMario.filmezzeu_password.value:
