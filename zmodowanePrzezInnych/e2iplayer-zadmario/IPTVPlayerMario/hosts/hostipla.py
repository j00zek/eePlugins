...
from Plugins.Extensions.IPTVPlayerMario.tools.iptvtools import printDBG, printExc, CSelOneLink, GetLogoDir, byteify
...
###################################################
config.plugins.IPTVPlayerMario.iplacachexml = ConfigSelection(default="12", choices=[("0", "nigdy"), ("6", "przez 6 godzin"), ("12", "przez 12 godzin"), ("24", "przez dzień")])
config.plugins.IPTVPlayerMario.iplaDefaultformat = ConfigSelection(default="1900", choices=[("200", "bitrate: 200"), ("400", "bitrate: 400"), ("900", "bitrate: 900"), ("1900", "bitrate: 1900")])
config.plugins.IPTVPlayerMario.iplaUseDF = ConfigYesNo(default=True)


def GetConfigList():
    optionList = []
    optionList.append(getConfigListEntry("Używaj danych z pamięci podręcznej:", config.plugins.IPTVPlayerMario.iplacachexml))
    optionList.append(getConfigListEntry("Domyślny format video:", config.plugins.IPTVPlayerMario.iplaDefaultformat))
    optionList.append(getConfigListEntry("Używaj domyślnego format video:", config.plugins.IPTVPlayerMario.iplaUseDF))
    return optionList
###################################################
...
class Ipla(CBaseHostClass):
    ...
    def __init__(self):
        ...
        self.cacheFilePath = os_path.join(config.plugins.IPTVPlayerMario.SciezkaCache.value, "iplaxml.cache")
    ...
    def _getVideoUrls(self, vodData):
        ...
        max_bitrate = int(config.plugins.IPTVPlayerMario.iplaDefaultformat.value)
        ...
        try:
            ...
            for link in links:
                ...
                drm = attrib['drmtype']
                if drm == '0':
                    if config.plugins.IPTVPlayerMario.ZablokujWMV.value and attrib['format'] == '0':
                        continue
        ...
        if config.plugins.IPTVPlayerMario.iplaUseDF.value:
            urls = [urls[0]]
        return urls
    ...
    def __writeCategoryCache(self, data):
        ...
        try:
            if "0" == config.plugins.IPTVPlayerMario.iplacachexml.value:
                return
    ...
    def __readCategoryCache(self):
        ...
        try:
            ...
            if (currTimestamp - saveTimestamp) / 3600 < int(config.plugins.IPTVPlayerMario.iplacachexml.value):
                data = data["data"]
