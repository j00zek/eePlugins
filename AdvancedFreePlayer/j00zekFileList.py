    ...
    def changeDir(self, directory, select = None):
        ...
        if directory is None and self.showMountpoints: # present available mountpoints
            for p in harddiskmanager.getMountedPartitions():
                ...
                if path not in self.inhibitMounts and not self.inParentDirs(path, self.inhibitDirs):
                    self.ItemsList.append(FileEntryComponent(name = p.description, absolute = path, isDir = True, currDir = self.current_directory))
            files = [ ]
            directories = [ ]
        ...
        if directory is not None and self.showDirectories and not self.isTop:
            if directory == self.current_mountpoint and self.showMountpoints:
                self.ItemsList.append(FileEntryComponent(name = "<" +_("List of Storage Devices") + ">", absolute = None, isDir = True))
            elif (directory != "/") and not (self.inhibitMounts and self.getMountpoint(directory) in self.inhibitMounts):
                #self.ItemsList.append(FileEntryComponent(name = "<" +_("Parent Directory") + ">", absolute = '/'.join(directory.split('/')[:-2]) + '/', isDir = True))
                self.ItemsList.append(FileEntryComponent(name = _("Parent Directory"), absolute = '/'.join(directory.split('/')[:-2]) + '/', isDir = True, goBack = True, DimText0 = self.DimText0, DimText1=self.DimText1, DimPIC=self.DimPIC))
            ...
            for x in directories:
                if not (self.inhibitMounts and self.getMountpoint(x) in self.inhibitMounts) and not self.inParentDirs(x, self.inhibitDirs):
                    ...
                    if config.plugins.AdvancedFreePlayer.FileNameFilter.value != '' and not config.plugins.AdvancedFreePlayer.FileNameFilter.value in name:
                        ...
                        self.ItemsList.append(FileEntryComponent(name = name, absolute = x, isDir = True, DimText0 = self.DimText0, DimText1=self.DimText1, DimPIC=self.DimPIC))
        ...
        if self.showFiles:
            for x in files:
                ...
                if (self.matchingPattern is None) or re_compile(self.matchingPattern).search(path):
                    #self.ItemsList.append(FileEntryComponent(name = name, absolute = x , isDir = False))
                    self.ItemsList.append(FileEntryComponent(name = name, absolute = x , isDir = False, currDir = self.current_directory, DimText0 = self.DimText0, DimText1=self.DimText1, DimPIC=self.DimPIC))

        if self.showMountpoints and len(self.ItemsList) == 0:
            self.ItemsList.append(FileEntryComponent(name = _("nothing connected"), absolute = None, isDir = False, currDir = self.current_directory, DimText0 = self.DimText0, DimText1=self.DimText1, DimPIC=self.DimPIC))

        self.l.setList(self.ItemsList)
