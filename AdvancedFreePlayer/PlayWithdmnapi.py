...
from Plugins.Extensions.AdvancedFreePlayer.cueSheetHelper import setCut, CUT_TYPE_LAST
...
class AdvancedFreePlayer(Screen):
    ...
    def resumeLastPlaybackNext(self):
        if self.resume_point > 0:
            ...
            if cue is None:
                printDEBUG("PlayWithDmnapi|resumeLastPlayback: cue is None")
                return
            ...
            for (pts, what) in cut_list:
                if what == CUT_TYPE_LAST:
                    last = pts
                    printDEBUG("PlayWithDmnapi|resumeLastPlayback: last position found at %d" % last)
                    break
            if last is None:
                printDEBUG("PlayWithDmnapi|resumeLastPlayback: last position is None")
                return
            
        # only resume if at least 10 seconds ahead, or <10 seconds before the end.
        seekable = self.__getSeekable()
        if seekable is None:
            printDEBUG("PlayWithDmnapi|resumeLastPlayback: seekable is None")
            return  # Should not happen?
        length = seekable.getLength() or (None, 0)
        printDEBUG("PlayWithDmnapi|resumeLastPlayback: seekable.getLength() returns: %d" % length[1])
    ...
    def playLastCB(self, answer):
        if answer == True:
            try:
                printDEBUG("PlayWithDmnapi|playLastCB resuming from %d" % (self.resume_point/90000))
                self.doSeek(self.resume_point)
            except Exception as e:
                printDEBUG("PlayWithDmnapi|playLastCB() EXCEPTION: %s" % str(e))
#        self.hideAfterResume()
    ...
    def loadconfig(self):
        try:
            ...
            printDEBUG("EXCEPTION in loadconfig(): %s" % str(e))
            return
...
########################################################################################### START functions
    ...
    def ExitPlayer(self):
        ...
        if self.URLlinkName == '' and not access(self.openmovie, W_OK):
                    ...
                    if self.URLlinkName == '':
                        if myConfig.MoveToTrash == True:
                            try:
                                ...
                                printDEBUG("Error executing system>move files engine: %s" % str(e))
                        else:
                            ...
                            DeleteFile(self.openmovie + ".cuts2")
                            try:
            ...
            if path.exists(path.dirname(self.openmovie) + '/.neverDelete'):
                printDEBUG("Nothing to delete, folder has '.neverDelete' Attribute")
            elif '/hdd/pictures' in myDir:
                printDEBUG("Nothing to delete, movie located in pictures folder")
            elif path.exists(self.openmovie + ".neverDelete") or path.exists(myDir + '/' + myFile + ".neverDelete"):
                printDEBUG("Nothing to delete, movie has '.neverDelete' Attribute")
            else:
