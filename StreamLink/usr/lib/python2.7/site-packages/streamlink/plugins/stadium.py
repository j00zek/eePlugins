            ...
            "sources": [
                {
                    ...
                    validate.optional("ext_x_version"): validate.text,
                    "type": validate.text,
                }
            ],
        },
    ...
    def _get_streams(self):
        ...
        for tag in itertags(res.text, "video"):
            if tag.attributes.get("id") == "brightcove_video_player":
                ...
                policy_key = self._policy_key_re.search(res.content)
                if not policy_key:
                    log.warning("Cannot extract policy key")
                    policy_key = ""
                else:
                    policy_key = policy_key.group(1)
