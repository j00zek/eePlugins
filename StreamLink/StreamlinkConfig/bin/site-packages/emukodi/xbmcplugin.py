...
def log(txt, txt2 = ''):
    f = open("/tmp/emukodi.log", "a")
    f.write(txt)
    if txt2 != '':
        f.write(' ')
        f.write(txt2)
    if not txt.endswith('\n') and not txt2.endswith('\n'):
        f.write('\n')
    f.close()
...
def addDirectoryItem(handle, url, listitem, totalItems=40, isFolder=True):
        ...
        log('xbmcplugin.addDirectoryItem(handle=%s, url=%s, listitem=%s)' % (handle,url,listitem))
        ...
        url = url.replace("=", "ExQ")
        data = "&url=" + url + "\n"
        with open("/tmp/emukodi.plugindata", "a") as f:
            f.write("&" + getdata(listitem) + data)
            f.close()
...
def iPlayer(url):
    log("xbmcplugin iPlayer url =", url)
